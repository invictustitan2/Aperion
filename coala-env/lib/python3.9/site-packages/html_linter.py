...
class TrailingWhitespaceMessage(Message):
    ...
    description = "Trailing white spaces are unnecessary and can complicate " + "diffs"
...
class VoidElementMessage(Message):
    ...
    def __init__(self, line, column, tag, trailing_chars=None):
        ...
        if trailing_chars is None:
            self.message = (
                "Remove the closing %s tag, it is a huge "
                + "conceptual error to close it"
            ) % tag
        else:
...
class TypeAttributeMessage(Message):
    ...
    def __init__(self, line, column, tag):
        ...
        self.description = (
            'The default type for %s tags is "%s" so it can ' + "be safely omitted"
        ) % (tag, default_type)
...
class ConcernsSeparationMessage(Message):
    ...
    def __init__(self, line, column, tag, attribute=None):
        ...
        if tag == "script":
            ...
            self.description = "Javascript should be defined in its own file"
            self.message = (
                'Move the contents of the "href" attribute to its' + " own JS file"
            )
        elif attribute == "style":
            self.description = "CSS should be defined in its own file"
            self.message = (
                'Move the contents of the "style" attribute to ' + "its own CSS file"
            )
        elif attribute.startswith("on"):
            self.description = "Javascript should be defined in its own file"
            self.message = (
                'Register the handler for "%s" with events in a ' + "JS file"
            ) % attribute
...
class NameMessage(Message):
    ...
    def __init__(self, line, column, attribute, value):
        Message.__init__(self, line, column)
        self.description = (
            "The %s names should be lowercase and with "
...
class IndentationMessage(Message):
    ...
    def __init__(self, line, column, indent, min_indent=0, max_indent=0):
        Message.__init__(self, line, column)
        self.message = (
            "Was expecting between %d and %d (in multiples of 2) " + "spaces but got %d"
        ) % (min_indent, max_indent, indent)
...
class BooleanAttributeMessage(Message):
    ...
    description = (
        "Boolean attributes define their value based on the "
    ...
    def __init__(self, line, column, attribute, value):
        Message.__init__(self, line, column)
        self.message = "Change '%s=\"%s\"' to just '%s'" % (attribute, value, attribute)
...
class InvalidAttributeMessage(Message):
    ...
    def __init__(self, line, column, attribute):
        ...
        if attribute == "charset":
            self.description = (
                'The attribute "charset" applies only to ' + "external resources"
            )
            ...
            self.description = (
                'The attribute "language" was deprecated more' + " than 10 years ago"
            )
            self.message = 'Remove the "language" attribute'
            self.url = "http://perfectionkills.com/optimizing-html/#7_script_language_javascript"
        elif attribute == "name":
            self.description = (
                'The attribute "name" is no longer required ' + "for anchors"
            )
            self.message = (
                'Remove the "name" attribute and replace it with '
                + "and id if required"
            )
...
class InvalidHandlerMessage(Message):
    ...
    def __init__(self, line, column, attribute):
        Message.__init__(self, line, column)
        self.message = (
            'Remove the "javascript:" prefix from the "%s" ' + "attribute"
        ) % attribute
...
class HTTPEquivMessage(Message):
    ...
    def __init__(self, line, column, http_equiv):
        ...
        if http_equiv == "content-language":
            ...
            self.message += (
                "http://www.w3.org/International/questions/qa-http-and-lang.en#answer"
            )
        elif http_equiv == "content-type":
            self.message = 'Replace this by <meta charset="utf-8">'
        elif http_equiv == "set-cookie":
            self.message = (
                'The http-equiv "%s" directive is non conformant,' + " avoid it"
            ) % http_equiv
            ...
            self.message = (
                'HTML5 does not allow the http-equiv "%s" ' + "directive"
            ) % http_equiv
...
VOID_ZERO_PATTERN = re.compile(
    r"^javascript:\s*void\s*\(\s*0\s*\)\s*;?$", flags=re.IGNORECASE
)
...
VALID_HTTP_EQUIV = frozenset(("refresh", "default-style", "x-ua-compatible", ""))
...
class HTML5Linter(HTMLParser.HTMLParser):
    ...
    def get_attribute_line_column(self, attribute):
        ...
        """
        return get_attribute_line_column(
            self.get_starttag_text(), self.getline(), self.getcolumn(), attribute
        )
    ...
    def handle_entityref(self, name):
        if name not in VALID_ENTITIES:
            entity = "&%s;" % name
            self._messages.append(
                EntityReferenceMessage(
                    line=self.getline(), column=self.getcolumn(), entity=entity
                )
            )
    ...
    def handle_data(self, data):
        ...
        for match in TRAILING_WHITESPACE_PATTERN.finditer(data):
            ...
            self._messages.append(
                TrailingWhitespaceMessage(
                    line=line, column=column, whitespace=match.group(1)
                )
            )
    ...
    def _handle_meta_starttag(self, attrs):
        ...
        if attrs.get("http-equiv", "").lower() not in VALID_HTTP_EQUIV:
            ...
            self._messages.append(
                HTTPEquivMessage(
                    line=line, column=column, http_equiv=attrs["http-equiv"]
                )
            )
        ...
        if attrs["charset"] != "utf-8":
            ...
            self._messages.append(
                CharsetMessage(line=line, column=column, charset=attrs["charset"])
            )
    ...
    def _handle_script_starttag(self, attrs):
        ...
        if "src" not in attrs:
            self._messages.append(
                ConcernsSeparationMessage(
                    line=self.getline(), column=self.getcolumn(), tag="script"
                )
            )
            if "charset" in attrs:
                ...
                self._messages.append(
                    InvalidAttributeMessage(
                        line=line, column=column, attribute="charset"
                    )
                )
        if "language" in attrs:
            line, column = self.get_attribute_line_column("language")
            self._messages.append(
                InvalidAttributeMessage(line=line, column=column, attribute="language")
            )

    def _handle_style_starttag(self, attrs):
        self._messages.append(
            ConcernsSeparationMessage(
                line=self.getline(), column=self.getcolumn(), tag="style"
            )
        )
        if attrs.get("type") == "text/css":
            line, column = self.get_attribute_line_column("type")
    ...
    def _handle_a_starttag(self, attrs):
        if VOID_ZERO_PATTERN.match(attrs.get("href", "").strip()):
            ...
            self._messages.append(VoidZeroMessage(line=line, column=column))
        elif attrs.get("href", "").strip().lower().startswith("javascript:"):
            line, column = self.get_value_line_column("href")
            self._messages.append(
                ConcernsSeparationMessage(
                    line=line, column=column, tag="a", attribute="href"
                )
            )
        if "name" in attrs:
            line, column = self.get_attribute_line_column("name")
            self._messages.append(
                InvalidAttributeMessage(line=line, column=column, attribute="name")
            )
    ...
    def _handle_style_attribute(self, tag, attrs):
        if "style" in attrs:
            ...
            self._messages.append(
                ConcernsSeparationMessage(
                    line=line, column=column, tag=tag, attribute="style"
                )
            )
    ...
    def _handle_on_attributes(self, tag, attrs):
        for attr in attrs:
            if attr.lower().startswith("on"):
                ...
                self._messages.append(
                    ConcernsSeparationMessage(
                        line=line, column=column, tag=tag, attribute=attr
                    )
                )
                ...
                if attrs[attr].strip().lower().startswith("javascript:"):
                    ...
                    self._messages.append(
                        InvalidHandlerMessage(line=line, column=column, attribute=attr)
                    )
    ...
    def _handle_href_src_attributes(self, unused_tag, attrs):
        ...
        line, column = self.get_value_line_column(attr)
        self._messages.append(
            ProtocolMessage(line=line, column=column, protocol=protocol)
        )
    ...
    def _handle_id_attribute(self, unused_tag, attrs):
        ...
        if not re.match(r"^[a-z0-9-]*$", name):
            ...
            self._messages.append(
                NameMessage(line=line, column=column, attribute="id", value=name)
            )
    ...
    def _handle_class_attribute(self, unused_tag, attrs):
        ...
        if not re.match(r"^[a-z0-9 -]*$", name):
            ...
            self._messages.append(
                NameMessage(line=line, column=column, attribute="class", value=name)
            )
    ...
    def _check_optional_opening_tag(self, tag, attrs):
        if tag in OPTIONAL_OPENING_TAGS and not attrs:
            self._messages.append(
                OptionalTagMessage(
                    line=self.getline(), column=self.getcolumn(), tag=tag, opening=True
                )
            )
    ...
    def _check_starttag_capitalization(self, tag):
        ...
        if original_tag != original_tag.lower():
            self._messages.append(
                CapitalizationMessage(
                    line=self.getline(), column=self.getcolumn() + 1, tag=original_tag
                )
            )

    def _check_attributes_case_quotation_entities(self, tag, unused_attrs):
        original_def = self.get_starttag_text()
        for match in HTMLParser.attrfind_tolerant.finditer(original_def, len(tag) + 1):
            # We do not use islower() due to http://bugs.python.org/issue13822.
            if match.group(1) != match.group(1).lower():
                line, column = self.get_attribute_line_column(match.group(1).lower())
                self._messages.append(
                    CapitalizationMessage(
                        line=line, column=column, tag=tag, attribute=match.group(1)
                    )
                )
            if not match.group(3):
                continue
            if not match.group(3).startswith('"'):
                line, column = get_line_column(
                    original_def, self.getline(), self.getcolumn(), match.start(3)
                )
                quotation = ""
                if match.group(3).startswith("'"):
                    quotation = "'"
                self._messages.append(
                    QuotationMessage(line=line, column=column, quotation=quotation)
                )
            ...
            for entity_match in HTMLParser.entityref.finditer(match.group(3)):
                ...
                self.line, self.offset = get_line_column(
                    original_def, line, column, entity_match.start(0) + match.start(3)
                )
                self.offset -= 1
    ...
    def handle_endtag(self, tag):
        ...
        if tag in VOID_TAGS:
            ...
            self._messages.append(
                VoidElementMessage(
                    line=line, column=column, tag=tag, trailing_chars=trailing_chars
                )
            )
        elif tag in OPTIONAL_CLOSING_TAGS:
            self._messages.append(
                OptionalTagMessage(
                    line=self.getline(), column=self.getcolumn(), tag=tag
                )
            )
    ...
    def close(self):
        if not self._has_charset and (
            ...
            line, column = self._first_meta_line_col or self._after_head_line_col
    ...
    def _check_tags_in_newline(self, tag):
        if tag in NEWLINE_TAGS and self._get_indentation() is None:
            self._messages.append(
                FormattingMessage(line=self.getline(), column=self.getcolumn(), tag=tag)
            )
    ...
    def _check_boolean_attributes(self, attrs):
        for attr, value in attrs.items():
            if attr in BOOLEAN_ATTRIBUTES and value is not None:
                line, column = self.get_attribute_line_column(attr)
                self._messages.append(
                    BooleanAttributeMessage(
                        line=line, column=column, attribute=attr, value=value
                    )
                )

    def _check_whitespaces(self, opening):
        tag_pattern = re.compile(
            r"</?(?P<start>\s*)\w+(.*?)(\s*/|(?P<end>\s*))>", flags=re.DOTALL
        )
        ...
        for group_name in ("start", "end"):
            if match.group(group_name):
                ...
                self._messages.append(ExtraWhitespaceMessage(line=line, column=column))

        for attr_match in attribute_pattern.finditer(original_def, match.start(2)):
