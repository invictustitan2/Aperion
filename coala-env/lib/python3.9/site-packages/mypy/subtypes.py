...
# import mypy.solve
from mypy import experiments, messages, sametypes
...
def is_subtype(
    ...
    if (
        ...
        return any(
            is_subtype(left, item, type_parameter_checker) for item in right.items
        )
    else:
...
def is_equivalent(
    ...
    return is_subtype(a, b, type_parameter_checker) and is_subtype(
        b, a, type_parameter_checker
    )
...
class SubtypeVisitor(TypeVisitor[bool]):
    ...
    def __init__(
        self, right: Type, type_parameter_checker: TypeParameterChecker
    ) -> None:
        self.right = right
    ...
    def visit_none_type(self, left: NoneTyp) -> bool:
        if experiments.STRICT_OPTIONAL:
            return isinstance(self.right, NoneTyp) or is_named_instance(
    ...
    def visit_callable_type(self, left: CallableType) -> bool:
        ...
        if isinstance(right, CallableType):
            ...
            return all(
                is_subtype(left, item, self.check_type_parameter)
    ...
    def visit_tuple_type(self, left: TupleType) -> bool:
        ...
        if isinstance(right, Instance):
            if is_named_instance(right, "typing.Sized"):
                ...
                or is_named_instance(right, "typing.Iterable")
                or is_named_instance(right, "typing.Container")
    ...
    def visit_overloaded(self, left: Overloaded) -> bool:
        ...
        if isinstance(right, Instance):
            ...
            for i in range(len(left.items())):
                if not is_subtype(
                    left.items()[i], right.items()[i], self.check_type_parameter
                ):
                    return False
            return True
            ...
            # This is unsound, we don't check the __init__ signature.
            return left.is_type_obj() and is_subtype(
                left.items()[0].ret_type, right.item
            )
        else:
            return False
    ...
    def visit_union_type(self, left: UnionType) -> bool:
        return all(
            is_subtype(item, self.right, self.check_type_parameter)
            for item in left.items
        )
    ...
    def visit_type_type(self, left: TypeType) -> bool:
        ...
        if isinstance(right, Instance) and right.type.fullname() in (
            "builtins.type",
            "builtins.object",
        ):
...
def is_callable_subtype(
    left: CallableType, right: CallableType, ignore_return: bool = False
) -> bool:
...
def unify_generic_callable(
    type: CallableType, target: CallableType, ignore_return: bool
) -> CallableType:
    ...
    applied = mypy.applytype.apply_generic_arguments(
        type, inferred_vars, msg, context=target
    )
    if msg.is_errors():
        return None
    return applied
...
def restrict_subtype_away(t: Type, s: Type) -> Type:
    ...
    if isinstance(t, UnionType):
        new_items = [
            item
            for item in t.items
            if (not is_subtype(item, s) or isinstance(item, AnyType))
        ]
    ...
    if isinstance(t, Instance):
        if isinstance(s, Instance):
            ...
            return all(
                ...
                for ta, ra, tvar in zip(t.args, s.args, s.type.defn.type_vars)
            )
        return False
    else:
