...
import mypy.checker
from mypy import applytype, erasetype, experiments, join, messages, nodes, types
from mypy.checkmember import analyze_member_access, type_object_type
from mypy.checkstrformat import StringFormatterChecker
from mypy.constraints import get_actual_type
from mypy.erasetype import replace_meta_vars
from mypy.expandtype import expand_type
from mypy.infer import infer_function_type_arguments, infer_type_arguments
...
ArgChecker = Callable[
    [Type, Type, int, Type, int, int, CallableType, Context, MessageBuilder], None
]
    ...
    while expr.kind == MODULE_REF or expr.fullname is not None:
        ...
        if isinstance(expr, NameExpr):
            ...
            if isinstance(expr.node, TypeInfo):
                ...
                expr.fullname is not None
                and "." in expr.fullname
                and not is_suppressed_import
            ):
...
class ExpressionChecker:
    ...
    def __init__(self, chk: "mypy.checker.TypeChecker", msg: MessageBuilder) -> None:
    ...
    def try_infer_partial_type(self, e: CallExpr) -> None:
        if isinstance(e.callee, MemberExpr) and isinstance(e.callee.expr, RefExpr):
            ...
            if partial_types is not None and not self.chk.current_node_deferred:
                ...
                if (
                    partial_type is None
                    or not isinstance(partial_type, PartialType)
                    or partial_type.type is None
                ):
                    ...
                    if isinstance(arg_type, Instance):
                        ...
                        if arg_typename in self.container_args[typename][methodname]:
                            full_item_types = [
                                ...
                                for item_type, prev_type in zip(
                                    arg_type.args, partial_type.inner_types
                                )
                            ]
                            if all(
                                ...
                                var.type = self.chk.named_generic_type(
                                    typename, list(full_item_types)
                                )
                                del partial_types[var]

    def check_call_expr_with_callee_type(self, callee_type: Type, e: CallExpr) -> Type:
        ...
        return self.check_call(
            callee_type, e.args, e.arg_kinds, e, e.arg_names, callable_node=e.callee
        )[0]
    ...
    def check_call(
        ...
        if isinstance(callee, CallableType):
            ...
            target = self.overload_call_target(
                arg_types, arg_kinds, arg_names, callee, context, messages=arg_messages
            )
            return self.check_call(
                target, args, arg_kinds, context, arg_names, arg_messages=arg_messages
            )
            ...
            self.msg.disable_type_names -= 1
            return (
                UnionType.make_simplified_union([res[0] for res in results]),
                callee,
            )
            ...
            return self.check_call(
                item, args, arg_kinds, context, arg_names, callable_node, arg_messages
            )
        else:
    ...
    def analyze_type_type_callee(self, item: Type, context: Context) -> Type:
        ...
        if isinstance(item, UnionType):
            return UnionType(
                [self.analyze_type_type_callee(item, context) for item in item.items],
                item.line,
            )
    ...
    def infer_arg_types_in_context(
        self, callee: Optional[CallableType], args: List[Expression]
    ) -> List[Type]:
    ...
    def infer_function_type_arguments_using_context(
        ...
        if isinstance(ret_type, TypeVarType):
            if ret_type.values or (not isinstance(ctx, Instance) or not ctx.args):
        ...
        if self.chk.in_checked_function():
            ...
            if (
                ...
                if isinstance(inferred_args[0], (NoneTyp, UninhabitedType)):
                    ...
                    self.msg.fail(
                        messages.KEYWORD_ARGUMENT_REQUIRES_STR_KEY_TYPE, context
                    )
        else:
    ...
    def check_argument_count(
        ...
        for i, kind in enumerate(actual_kinds):
            if i not in all_actuals and (
                kind != nodes.ARG_STAR or not is_empty_tuple(actual_types[i])
            ):
        ...
        for i, kind in enumerate(formal_kinds):
            if kind == nodes.ARG_POS and (
                not formal_to_actual[i] and not is_unexpected_arg_error
            ):
                ...
                nodes.ARG_OPT,
                nodes.ARG_NAMED,
            ] and is_duplicate_mapping(formal_to_actual[i], actual_kinds):
                if self.chk.in_checked_function() or isinstance(
                    actual_types[formal_to_actual[i][0]], TupleType
                ):
                    if messages:
    ...
    def check_argument_types(
        ...
        for i, actuals in enumerate(formal_to_actual):
            for actual in actuals:
                ...
                if arg_kinds[actual] == nodes.ARG_STAR and not self.is_valid_var_arg(
                    arg_type
                ):
                    messages.invalid_var_arg(arg_type, context)
                if arg_kinds[
                    actual
                ] == nodes.ARG_STAR2 and not self.is_valid_keyword_var_arg(arg_type):
                ...
                actual_type = get_actual_type(
                    arg_type, arg_kinds[actual], tuple_counter
                )
                check_arg(
                    actual_type,
                    arg_type,
                ...
                if (
                    callee.arg_kinds[i] == nodes.ARG_STAR
                    and arg_kinds[actual] == nodes.ARG_STAR
                    and isinstance(tuplet, TupleType)
                ):
    ...
    def check_arg(
        ...
        if self.chk.is_unusable_type(caller_type):
            ...
            messages.incompatible_argument(
                n, m, callee, original_caller_type, caller_kind, context
            )
    ...
    def overload_call_target(
        ...
        for typ in overload.items():
            similarity = self.erased_signature_similarity(
                arg_types, arg_kinds, arg_names, typ, context=context
            )
            if similarity > 0 and similarity >= best_match:
                if (
                    match
                    and not is_same_type(match[-1].ret_type, typ.ret_type)
                    and not mypy.checker.is_more_precise_signature(match[-1], typ)
                ):
    ...
    def erased_signature_similarity(
        ...
        if not self.check_argument_count(
            callee, arg_types, arg_kinds, arg_names, formal_to_actual, None, None
        ):
        ...
        def check_arg(
            ...
            nonlocal similarity
            similarity = min(
                similarity, overload_arg_similarity(caller_type, callee_type)
            )
            if similarity == 0:
    ...
    def apply_generic_arguments(
        self, callable: CallableType, types: List[Type], context: Context
    ) -> CallableType:
    ...
    def visit_op_expr(self, e: OpExpr) -> Type:
        ...
        if e.op in nodes.op_methods:
            ...
            result, method_type = self.check_op(
                method, left_type, e.right, e, allow_reverse=True
            )
    ...
    def visit_comparison_expr(self, e: ComparisonExpr) -> Type:
        ...
        for left, right, operator in zip(e.operands, e.operands[1:], e.operators):
            ...
            if operator == "in" or operator == "not in":
                ...
                sub_result, method_type = self.check_op_local(
                    "__contains__", right_type, left, e, local_errors
                )
                if isinstance(right_type, PartialType):
                    # We don't really know if this is an error or not, so just shut up.
                    pass
                elif (
                    local_errors.is_errors()
                    and
    ...
    def _check_op_for_errors(
        self, method: str, base_type: Type, arg: Expression, context: Context
        ...
        local_errors = self.msg.copy()
        local_errors.disable_count = 0
        result = self.check_op_local(method, base_type, arg, context, local_errors)
    ...
    def check_op_local(
        ...
        return self.check_call(
            method_type, [arg], [nodes.ARG_POS], context, arg_messages=local_errors
        )
    ...
    def check_op(
        ...
        if success or not allow_reverse or isinstance(base_type, AnyType):
            ...
            first_error = (
                None
            )  # type: Optional[Tuple[Tuple[Type, Type], MessageBuilder]]
            if self.has_member(arg_type, rmethod):
                result, local_errors = self._check_op_for_errors(
                    rmethod, arg_type, base_arg_node, context
                )
            ...
            if (
                self.chk.options.python_version[0] == 2
                and method in nodes.ops_falling_back_to_cmp
            ):
                ...
                if self.has_member(base_type, cmp_method):
                    ...
                    result, local_errors = self._check_op_for_errors(
                        cmp_method, base_type, arg, context
                    )
                    if not local_errors.is_errors():
                        return result
                    first_error = first_error or (result, local_errors)
                if self.has_member(arg_type, cmp_method):
                    ...
                    result, local_errors = self._check_op_for_errors(
                        cmp_method, arg_type, base_arg_node, context
                    )
                    if not local_errors.is_errors():
                        return result
                    first_error = first_error or (result, local_errors)
            if first_error:
                ...
                return self.check_op_local(method, base_type, arg, context, self.msg)
    ...
    def visit_tuple_slice_helper(self, left_type: TupleType, slic: SliceExpr) -> Type:
        ...
        if slic.begin_index:
            ...
            if begin is None:
                self.chk.fail(
                    messages.TUPLE_SLICE_MUST_BE_AN_INT_LITERAL, slic.begin_index
                )
                return AnyType()
        ...
        if slic.end_index:
            ...
            if end is None:
                self.chk.fail(
                    messages.TUPLE_SLICE_MUST_BE_AN_INT_LITERAL, slic.end_index
                )
                return AnyType()
    ...
    def visit_cast_expr(self, expr: CastExpr) -> Type:
        ...
        if self.chk.options.warn_redundant_casts and is_same_type(
    ...
    def is_valid_cast(self, source_type: Type, target_type: Type) -> bool:
        ...
        return isinstance(target_type, AnyType) or (
            not isinstance(source_type, Void) and not isinstance(target_type, Void)
        )
        ...
        if isinstance(tp, CallableType):
            ...
            if len(tp.variables) != len(tapp.types):
                self.msg.incompatible_type_application(
                    len(tp.variables), len(tapp.types), tapp
                )
                return AnyType()
            ...
            for item in tp.items():
                if len(item.variables) != len(tapp.types):
                    self.msg.incompatible_type_application(
                        len(item.variables), len(tapp.types), tapp
                    )
                    return AnyType()
            return Overloaded(
                [
                    self.apply_generic_arguments(item, tapp.types, tapp)
                    for item in tp.items()
                ]
            )
        return AnyType()
    ...
    def visit_type_alias_expr(self, alias: TypeAliasExpr) -> Type:
        ...
        if isinstance(tp, CallableType):
            if len(tp.variables) != len(item.args):
                self.msg.incompatible_type_application(
                    len(tp.variables), len(item.args), item
                )
                return AnyType()
            ...
            for it in tp.items():
                if len(it.variables) != len(item.args):
                    self.msg.incompatible_type_application(
                        len(it.variables), len(item.args), item
                    )
                    return AnyType()
            return Overloaded(
                [self.apply_generic_arguments(it, item.args, item) for it in tp.items()]
            )
        return AnyType()
    ...
    def check_lst_expr(
        self, items: List[Expression], fullname: str, tag: str, context: Context
    ) -> Type:
    ...
    def visit_func_expr(self, e: FuncExpr) -> Type:
        ...
        if not inferred_type:
            ...
            if not e.arguments:
                ...
                return CallableType(
                    [], [], [], ret_type, self.named_type("builtins.function")
                )
            else:
    ...
    def analyze_super(self, e: SuperExpr, is_lvalue: bool) -> Type:
        ...
        if e.info and e.info.bases:
            ...
            if len(e.info.mro) < 2:
                self.chk.fail(
                    "Internal error: unexpected mro for {}: {}".format(
                        e.info.name(), e.info.mro
                    ),
                    e,
                )
                return AnyType()
            for base in e.info.mro[1:]:
                if e.name in base.names or base == e.info.mro[-1]:
                    ...
                    if not args:
                        self.chk.fail(
                            "super() requires at least one positional argument", e
                        )
                        return AnyType()
    ...
    def visit_generator_expr(self, e: GeneratorExpr) -> Type:
        return self.check_generator_or_comprehension(
            e, "typing.Iterator", "<generator>"
        )

    def check_generator_or_comprehension(
        self, gen: GeneratorExpr, type_name: str, id_for_messages: str
    ) -> Type:
    ...
    def check_for_comp(self, e: Union[GeneratorExpr, DictionaryComprehension]) -> None:
        ...
        """
        for index, sequence, conditions in zip(e.indices, e.sequences, e.condlists):
    ...
    def is_valid_var_arg(self, typ: Type) -> bool:
        ...
        return (
            ...
            or is_subtype(
                typ, self.chk.named_generic_type("typing.Iterable", [AnyType()])
            )
    ...
    def is_valid_keyword_var_arg(self, typ: Type) -> bool:
        ...
        if self.chk.options.python_version[0] >= 3:
            ...
            return is_subtype(
                ...
                typ,
                self.chk.named_generic_type(
                    "builtins.dict", [self.named_type("builtins.unicode"), AnyType()]
                ),
            )
...
def map_actuals_to_formals(
    ...
    for i, kind in enumerate(caller_kinds):
        if kind == nodes.ARG_POS:
            if j < ncallee:
                if callee_kinds[j] in [nodes.ARG_POS, nodes.ARG_OPT, nodes.ARG_NAMED]:
                    map[j].append(i)
                    j += 1
            ...
            for j in range(ncallee):
                ...
                if (callee_names[j] and no_certain_match) or callee_kinds[
                    j
                ] == nodes.ARG_STAR2:
                    map[j].append(i)
    return map
...
def overload_arg_similarity(actual: Type, formal: Type) -> int:
    ...
    if (
        isinstance(actual, UninhabitedType)
        or isinstance(actual, AnyType)
        or isinstance(formal, AnyType)
        or isinstance(formal, CallableType)
        or (isinstance(actual, Instance) and actual.type.fallback_to_any)
    ):
    ...
    if isinstance(actual, NoneTyp):
        if not experiments.STRICT_OPTIONAL:
            ...
            if (
                isinstance(formal, Instance)
                and formal.type.fullname() == "builtins.object"
            ):
                return 2
    if isinstance(actual, UnionType):
        return max(overload_arg_similarity(item, formal) for item in actual.items)
    if isinstance(formal, UnionType):
        return max(overload_arg_similarity(actual, item) for item in formal.items)
    ...
    return (
        2
        if is_same_type(erasetype.erase_type(actual), erasetype.erase_type(formal))
        else 0
    )
...
def freshen_generic_callable(callee: CallableType) -> CallableType:
    ...
    return cast(CallableType, expand_type(callee, tvmap)).copy_modified(
        variables=tvdefs
    )
