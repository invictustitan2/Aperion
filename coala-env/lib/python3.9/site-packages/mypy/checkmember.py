...
from mypy import messages, subtypes
...
def analyze_var(
    ...
    if typ:
        ...
        if (
            ...
            and isinstance(t, FunctionLike)
            and not t.is_type_obj()
        ):
            if is_lvalue:
                if var.is_property:
...
def handle_partial_attribute_type(
    typ: PartialType, is_lvalue: bool, msg: MessageBuilder, context: Context
) -> Type:
    if typ.type is None:
...
def analyze_class_attribute_access(
    ...
    if is_decorated:
        ...
        return function_type(
            cast(FuncBase, node.node), builtin_type("builtins.function")
        )
...
def add_class_tvars(
    ...
    if isinstance(t, CallableType):
        ...
        tvars = [
            TypeVarDef(n, i + 1, None, builtin_type("builtins.object"), tv.variance)
        ...
        return Overloaded(
            [
                cast(
                    CallableType,
                    add_class_tvars(
                        item, itype, is_classmethod, builtin_type, original_type
                    ),
                )
                for item in t.items()
            ]
...
def type_object_type_from_function(
    init_or_new: FuncBase, info: TypeInfo, fallback: Instance
) -> FunctionLike:
    ...
    signature = cast(
        FunctionLike, map_type_from_supertype(signature, info, init_or_new.info)
    )
...
def bind_self(method: F, original_type: Type = None) -> F:
    ...
    if func.variables and (
        isinstance(self_param_type, TypeVarType)
        or (
            isinstance(self_param_type, TypeType)
            and isinstance(self_param_type.item, TypeVarType)
        )
    ):
        ...
        typearg = infer_type_arguments(
            [x.id for x in func.variables], self_param_type, original_type
        )[0]
