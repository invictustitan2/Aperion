...
import re
from typing import Any, Callable, Dict, List, Match, Pattern, Set, Tuple, Union

from mypy import defaults
from mypy.util import find_python_encoding, short_type
...
class Lexer:
    ...
    def __init__(
        ...
        for seq, method in [
            ("ABCDEFGHIJKLMNOPQRSTUVWXYZ", self.lex_name),
    ...
    def lex(self, text: Union[str, bytes], first_line: int) -> None:
        ...
        # of input.
        if len(self.tok) > 0 and (
            not isinstance(self.tok[-1], Break) and not isinstance(self.tok[-1], Dedent)
        ):
    ...
    def report_unknown_encoding(self, encoding_line: int) -> None:
        ...
        self.add_token(LexError("", DECODE_ERROR, "Unknown encoding %r" % self.enc))
        self.add_token(Break(""))
        self.add_token(Eof(""))
    ...
    number_exp2 = re.compile(r"[0-9]*\.[0-9]*([eE][-+]?[0-9]+)?|[0-9]+[eE][-+]?[0-9]+")
    ...
    def lex_number(self) -> None:
        ...
        if (
            self.name_char_exp.match(self.s[self.i + maxlen : self.i + maxlen + 1])
            is not None
        ):
    ...
    str_exp_single = re.compile(r"[a-zA-Z]*'([^'\\\r\n]|\\[^\r\n])*('|\\(\n|\r\n?))")
    # Non-initial part of a multiline single-quoted literal, e.g. foo'
    str_exp_single_multi = re.compile(r"([^'\\\r\n]|\\[^\r\n])*('|\\(\n|\r\n?))")
    ...
    str_exp_raw_single_multi = re.compile(r"([^'\r\n]|'')*('|\\(\n|\r\n?))")
    ...
    str_exp_double = re.compile(r'[a-z]*"([^"\\\r\n]|\\[^\r\n])*("|\\(\n|\r\n?))')
    str_exp_double_multi = re.compile(r'([^"\\\r\n]|\\[^\r\n])*("|\\(\n|\r\n?))')
    str_exp_raw_double = re.compile(
        r'[a-z]*"([^"\r\n\\]|\\"|\\[^\n\r])*("|\\(\n|\r\n?))'
    )
    str_exp_raw_double_multi = re.compile(r'([^"\r\n]|"")*("|\\(\n|\r\n?))')
    ...
    def lex_str_single(self) -> None:
        ...
        self.lex_str(
            self.str_exp_single,
            self.str_exp_single_multi,
            self.str_exp_single3,
            self.str_exp_single3end,
        )
    ...
    def lex_str_double(self) -> None:
        ...
        self.lex_str(
            self.str_exp_double,
            self.str_exp_double_multi,
            self.str_exp_double3,
            self.str_exp_double3end,
        )
    ...
    def lex_prefixed_str(self, prefix: str) -> None:
        ...
        if s.endswith("'"):
            ...
            self.lex_str(
                re1, re2, self.str_exp_single3, self.str_exp_single3end, prefix
            )
        else:
            ...
            self.lex_str(
                re1, re2, self.str_exp_double3, self.str_exp_double3end, prefix
            )
    ...
    def lex_multiline_string_literal(self, re_end: Pattern[str], prefix: str) -> None:
