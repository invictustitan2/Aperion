...
def submodule_revision(dir: str, submodule: str) -> bytes:
    ...
    output = subprocess.check_output(
        ["git", "ls-files", "-s", submodule], cwd=dir
    ).strip()
...
def warn_no_git_executable() -> None:
    print(
        "Warning: Couldn't check git integrity. " "git executable not in path.",
        file=sys.stderr,
    )
...
def warn_dirty(dir: str) -> None:
    print(
        "Warning: git module '{}' has uncommitted changes.".format(dir), file=sys.stderr
    )
...
def warn_extra_files(dir: str) -> None:
    print("Warning: git module '{}' has untracked files.".format(dir), file=sys.stderr)
    print("Go to the directory", file=sys.stderr)
    print("  {}".format(dir), file=sys.stderr)
...
def error_submodule_not_initialized(name: str, dir: str) -> None:
    ...
    print(
        "  {}git submodule update --init {}".format(chdir_prefix(dir), name),
        file=sys.stderr,
    )
...
def error_submodule_not_updated(name: str, dir: str) -> None:
    ...
    print("Please run:", file=sys.stderr)
    print(
        "  {}git submodule update {}".format(chdir_prefix(dir), name), file=sys.stderr
    )
    print(
        "(If you got this message because you updated {} yourself".format(name),
        file=sys.stderr,
    )
