...
def generate_stub_for_c_module(
    module_name, target, add_header=True, sigs={}, class_sigs={}
):
    ...
    for name, obj in items:
        ...
        if is_c_type(obj):
            generate_c_type_stub(
                module, name, obj, types, sigs=sigs, class_sigs=class_sigs
            )
            done.add(name)
    variables = []
    for name, obj in items:
...
def is_c_method(obj):
    return type(obj) in (type(str.index), type(str.__add__), type(str.__new__))
...
def generate_c_function_stub(
    module, name, obj, output, self_var=None, sigs={}, class_name=None, class_sigs={}
):
    if self_var:
        self_arg = "%s, " % self_var
    else:
