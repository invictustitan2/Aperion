...
class TypeVarId:
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, TypeVarId):
            return self.raw_id == other.raw_id and self.meta_level == other.meta_level
        else:
            return False
...
class TypeVarDef(mypy.nodes.Context):
    ...
    def serialize(self) -> JsonDict:
        ...
        return {
            ...
            "id": self.id.raw_id,
            "values": (
                None if self.values is None else [v.serialize() for v in self.values]
            ),
...
class UnboundType(Type):
    ...
    def deserialize(cls, data: JsonDict) -> "UnboundType":
        ...
        return UnboundType(data["name"], [Type.deserialize(a) for a in data["args"]])
...
class AnyType(Type):
    ...
    def __init__(
        self, implicit: bool = False, line: int = -1, column: int = -1
    ) -> None:
        super().__init__(line, column)
...
class NoneTyp(Type):
    ...
    def __init__(
        self, is_ret_type: bool = False, line: int = -1, column: int = -1
    ) -> None:
        super().__init__(line, column)
        self.is_ret_type = is_ret_type
...
class CallableType(FunctionLike):
    ...
    def is_type_obj(self) -> bool:
        return (
            self.fallback.type is not None
            and self.fallback.type.fullname() == "builtins.type"
        )
    ...
    def serialize(self) -> JsonDict:
        ...
        return {
            ".class": "CallableType",
            "arg_types": [
                (None if t is None else t.serialize()) for t in self.arg_types
            ],
    ...
    def deserialize(cls, data: JsonDict) -> "CallableType":
        ...
        return CallableType(
            [(None if t is None else Type.deserialize(t)) for t in data["arg_types"]],
            data["arg_kinds"],
            data["arg_names"],
...
class TupleType(Type):
    ...
    def deserialize(cls, data: JsonDict) -> "TupleType":
        ...
        return TupleType(
            [Type.deserialize(t) for t in data["items"]],
            Instance.deserialize(data["fallback"]),
            implicit=data["implicit"],
        )

    def copy_modified(
        self, *, fallback: Instance = None, items: List[Type] = None
    ) -> "TupleType":
        if fallback is None:
...
class UnionType(Type):
    ...
    def make_simplified_union(
        items: List[Type], line: int = -1, column: int = -1
    ) -> Type:
...
class TypeTranslator(TypeVisitor[Type]):
    ...
    def translate_variables(self, variables: List[TypeVarDef]) -> List[TypeVarDef]:
        return variables
    ...
    def visit_overloaded(self, t: Overloaded) -> Type:
        ...
        for item in t.items():
            ...
            if isinstance(new, CallableType):
                ...
                raise RuntimeError(
                    "CallableType expectected, but got {}".format(type(new))
                )
...
def strip_type(typ: Type) -> Type:
    ...
    if isinstance(typ, CallableType):
        ...
        return Overloaded(
            [cast(CallableType, strip_type(item)) for item in typ.items()]
        )
    else:
        return typ
...
def is_named_instance(t: Type, fullname: str) -> bool:
    return (
        isinstance(t, Instance) and t.type is not None and t.type.fullname() == fullname
    )
...
def set_typ_args(
    tp: Type, new_args: List[Type], line: int = -1, column: int = -1
) -> Type:
    ...
    if isinstance(tp, CallableType):
        return tp.copy_modified(
            arg_types=new_args[:-1], ret_type=new_args[-1], line=line, column=column
        )
    return tp
