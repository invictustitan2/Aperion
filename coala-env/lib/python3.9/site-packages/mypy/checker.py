...
from mypy import experiments, messages, nodes
from mypy.binder import ConditionalTypeBinder
from mypy.checkmember import bind_self, erase_to_bound, map_type_from_supertype
from mypy.erasetype import erase_typevars
from mypy.errors import Errors, report_internal_error
from mypy.expandtype import expand_type
from mypy.join import join_types
from mypy.maptype import map_instance_to_supertype
from mypy.meet import is_overlapping_types, meet_simple
...
from mypy.semanal import fill_typevars, refers_to_fullname, set_callable_name
...
DeferredNode = NamedTuple(
    ...
    [
        ...
        (
            "context_type_name",
            Optional[str],
        ),  # Name of the surrounding class (for error messages)
...
class TypeChecker(NodeVisitor[Type]):
    ...
    def __init__(
        ...
        if options.strict_optional_whitelist is None:
            ...
            self.suppress_none_errors = not any(
                ...
                for pattern in options.strict_optional_whitelist
            )
    ...
    def check_first_pass(self) -> None:
        ...
        if all_ is not None and all_.type is not None:
            ...
            if not is_subtype(all_.type, seq_str):
                ...
                self.fail(
                    messages.ALL_MUST_BE_SEQ_STR.format(str_seq_s, all_s), all_.node
                )
    ...
    def accept(
        self, node: Union[Expression, Statement, FuncItem], type_context: Type = None
    ) -> Type:
    ...
    def accept_loop(
        ...
        *,
        exit_condition: Expression = None
    ) -> None:
    ...
    def is_generator_return_type(self, typ: Type, is_coroutine: bool) -> bool:
        ...
        if is_coroutine:
            ...
            gt = self.named_generic_type(
                "typing.Generator", [AnyType(), AnyType(), AnyType()]
            )
        ...
        if defn.original_def:
            ...
            if isinstance(defn.original_def, FuncDef):
                ...
                if orig_type is None:
                    ...
                    self.msg.note(
                        "Internal mypy error checking function redefinition.", defn
                    )
                    return None
    ...
    def check_func_item(
        self, defn: FuncItem, type_override: CallableType = None, name: str = None
    ) -> None:
        ...
        for item, typ in self.expand_typevars(defn, typ):
            ...
            with self.binder.top_frame_context():
                ...
                if fdef:
                    ...
                    if (
                        fdef.info
                        and fdef.name() == "__init__"
                        and not isinstance(typ.ret_type, (Void, NoneTyp))
                        and not self.dynamic_funcs[-1]
                    ):
                        self.fail(messages.INIT_MUST_HAVE_NONE_RETURN_TYPE, item.type)

                    show_untyped = (
                        not self.is_typeshed_stub or self.options.warn_incomplete_stub
                    )
                ...
                if defn.is_generator:
                    ...
                    if (
                        ...
                        if not isinstance(
                            typ.ret_type.args[2], (Void, NoneTyp, AnyType)
                        ):
                    ...
                    if (
                        ...
                        if not is_subtype_ignoring_tvars(ref_type, erased):
                            self.fail(
                                ...
                                "is not a supertype of its class '{}'".format(
                                    erased, ref_type
                                ),
                                defn,
                            )
                        ...
                        if arg_type.variance == COVARIANT:
                            self.fail(
                                messages.FUNCTION_PARAMETER_CANNOT_BE_COVARIANT,
                                arg_type,
                            )
    ...
    def is_trivial_body(self, block: Block) -> bool:
        ...
        # Skip a docstring
        if isinstance(body[0], ExpressionStmt) and isinstance(
            body[0].expr, (StrExpr, UnicodeExpr)
        ):
            body = block.body[1:]
    ...
    def check_getattr_method(self, typ: CallableType, context: Context) -> None:
        method_type = CallableType(
            [AnyType(), self.named_type("builtins.str")],
    ...
    def check_method_or_accessor_override_for_base(
        self, defn: FuncBase, base: TypeInfo
    ) -> None:
        ...
        if base_attr:
            ...
            if isinstance(original_type, FunctionLike):
                original = map_type_from_supertype(
                    bind_self(original_type, self.scope.active_class()), defn.info, base
                )
    ...
    def check_override(
        ...
        if not is_subtype(override, original):
            ...
            not isinstance(original, Overloaded)
            and isinstance(override, Overloaded)
            and name in nodes.reverse_op_methods.keys()
        ):
    ...
    def check_compatibility(
        ...
        if isinstance(first_type, FunctionLike) and isinstance(
    ...
    def check_import(self, node: ImportBase) -> None:
        for assign in node.assignments:
            ...
            if lvalue_type is None:
                ...
                lvalue_type = AnyType()
            message = '{} "{}"'.format(
                messages.INCOMPATIBLE_IMPORT_OF, cast(NameExpr, assign.rvalue).name
            )
    ...
    def check_assignment(
        ...
        if isinstance(lvalue, TupleExpr) or isinstance(lvalue, ListExpr):
            self.check_assignment_to_multiple_lvalues(
                lvalue.items, rvalue, lvalue, infer_lvalue_type
            )
        else:
            ...
            if inferred:
                self.infer_variable_type(inferred, lvalue, self.accept(rvalue), rvalue)
        ...
        if isinstance(rvalue_type, AnyType):
            for lv in lvalues:
                ...
                self.check_assignment(
                    lv, self.temp_node(AnyType(), context), infer_lvalue_type
                )
    ...
    def check_multi_assignment_from_tuple(
        ...
        if self.check_rvalue_count_in_assignment(
            lvalues, len(rvalue_type.items), context
        ):
            star_index = next(
                (i for i, lv in enumerate(lvalues) if isinstance(lv, StarExpr)),
                len(lvalues),
    ...
    def type_is_iterable(self, type: Type) -> bool:
        return is_subtype(
            type, self.named_generic_type("typing.Iterable", [AnyType()])
        ) and isinstance(type, Instance)
    ...
    def check_multi_assignment_from_iterable(
        ...
        if self.type_is_iterable(rvalue_type):
            ...
            for lv in lvalues:
                if isinstance(lv, StarExpr):
                    self.check_assignment(
                        lv.expr, self.temp_node(rvalue_type, context), infer_lvalue_type
                    )
                else:
                    self.check_assignment(
    ...
    def infer_partial_type(self, name: Var, lvalue: Lvalue, init_type: Type) -> bool:
        if isinstance(init_type, (NoneTyp, UninhabitedType)):
            ...
            if (
                ...
                and all(
                    isinstance(t, (NoneTyp, UninhabitedType)) for t in init_type.args
                )
            ):
    ...
    def set_inference_error_fallback_type(
        self, var: Var, lvalue: Lvalue, type: Type, context: Context
    ) -> None:
    ...
    def check_indexed_assignment(
        ...
        self.expr_checker.check_call(
            method_type, [lvalue.index, rvalue], [nodes.ARG_POS, nodes.ARG_POS], context
        )
        ...
        if isinstance(lvalue.base, RefExpr) and isinstance(lvalue.base.node, Var):
            ...
            if isinstance(var.type, PartialType):
                ...
                if typename == "builtins.dict":
                    ...
                    if is_valid_inferred_type(full_key_type) and is_valid_inferred_type(
                        full_value_type
                    ):
                        if not self.current_node_deferred:
                            var.type = self.named_generic_type(
                                "builtins.dict", [full_key_type, full_value_type]
                            )
                        del partial_types[var]
        ...
        if defn is not None:
            ...
            if s.expr:
                ...
                if self.is_unusable_type(return_type):
                    ...
                    if isinstance(self.scope.top_function(), FuncExpr) or isinstance(
                        typ, NoneTyp
                    ):
                        return
    ...
    def visit_while_stmt(self, s: WhileStmt) -> Type:
        ...
        self.accept_loop(
            IfStmt([s.expr], [s.body], None), s.else_body, exit_condition=s.expr
        )
        return None

    def visit_operator_assignment_stmt(self, s: OperatorAssignmentStmt) -> Type:
                    ...
                    with self.binder.frame_context(can_skip=True, fall_through=4):
                        if s.types[i]:
                            ...
                            if s.vars[i]:
                                ...
                                self.current_node_deferred = False
                                self.check_assignment(
                                    s.vars[i], self.temp_node(t, s.vars[i])
                                )
    ...
    def analyze_async_iterable_item_type(self, expr: Expression) -> Type:
        ...
        self.check_subtype(
            iterable,
            self.named_generic_type("typing.AsyncIterable", [AnyType()]),
            expr,
            messages.ASYNC_ITERABLE_EXPECTED,
        )
        ...
        return self.check_awaitable_expr(
            awaitable, expr, messages.INCOMPATIBLE_TYPES_IN_ASYNC_FOR
        )
    ...
    def analyze_iterable_item_type(self, expr: Expression) -> Type:
        ...
        if isinstance(iterable, TupleType):
            ...
            if self.options.python_version[0] >= 3:
                ...
                nextmethod = "next"
            method = echk.analyze_external_member_access(nextmethod, iterator, expr)
            return echk.check_call(method, [], [], expr)[0]

    def analyze_index_variables(
        self, index: Expression, item_type: Type, context: Context
    ) -> None:
    ...
    def visit_decorator(self, e: Decorator) -> Type:
        ...
        for i in range(len(e.decorators)):
            ...
            sig, t2 = self.expr_checker.check_call(dec, [temp], [nodes.ARG_POS], e)
        ...
        if not e.var.is_settable_property and e.func.info is not None:
            ...
            for base in e.func.info.mro[1:]:
                ...
                if (
                    ...
                    and base_attr.node.items[0].var.is_settable_property
                ):
    ...
    def visit_yield_from_expr(self, e: YieldFromExpr) -> Type:
        ...
        if isinstance(subexpr_type, AnyType):
            iter_type = AnyType()
        elif isinstance(subexpr_type, Instance) and is_subtype(
            subexpr_type, self.named_type("typing.Iterable")
        ):
            if self.is_async_def(subexpr_type) and not self.has_coroutine_decorator(
                return_type
            ):
                self.msg.yield_from_invalid_operand_type(subexpr_type, e)
            iter_method_type = self.expr_checker.analyze_external_member_access(
                "__iter__", subexpr_type, AnyType()
            )

            generic_generator_type = self.named_generic_type(
                "typing.Generator", [AnyType(), AnyType(), AnyType()]
            )
            iter_type, _ = self.expr_checker.check_call(
                iter_method_type, [], [], context=generic_generator_type
            )
        else:
            if not (
                self.is_async_def(subexpr_type)
                and self.has_coroutine_decorator(return_type)
            ):
                self.msg.yield_from_invalid_operand_type(subexpr_type, e)
                iter_type = AnyType()
            else:
                iter_type = self.check_awaitable_expr(
                    subexpr_type, e, messages.INCOMPATIBLE_TYPES_IN_YIELD_FROM
                )
    ...
    def has_coroutine_decorator(self, t: Type) -> bool:
        ...
        return (
            isinstance(t, Instance) and t.type.fullname() == "typing.AwaitableGenerator"
        )
    ...
    def visit_yield_expr(self, e: YieldExpr) -> Type:
        ...
        if e.expr is None:
            if (
                not isinstance(expected_item_type, (Void, NoneTyp, AnyType))
    ...
    def visit_await_expr(self, e: AwaitExpr) -> Type:
        ...
        if isinstance(actual_type, AnyType):
            return AnyType()
        return self.check_awaitable_expr(
            actual_type, e, messages.INCOMPATIBLE_TYPES_IN_AWAIT
        )
        ...
        if is_subtype(subtype, supertype):
            ...
            if self.is_unusable_type(subtype):
                ...
                if subtype_label is not None or supertype_label is not None:
                    subtype_str, supertype_str = self.msg.format_distinctly(
                        subtype, supertype
                    )
    ...
    def contains_none(self, t: Type):
        return (
            isinstance(t, NoneTyp)
            or (
                isinstance(t, UnionType)
                and any(self.contains_none(ut) for ut in t.items)
            )
            or (
                isinstance(t, TupleType)
                and any(self.contains_none(tt) for tt in t.items)
            )
            or (
                isinstance(t, Instance)
                and t.args
    ...
    def should_suppress_optional_error(self, related_types: List[Type]) -> bool:
        return self.suppress_none_errors and any(
            self.contains_none(t) for t in related_types
        )
    ...
    def lookup_qualified(self, name: str) -> SymbolTableNode:
        if "." not in name:
            ...
            if last in n.names:
                ...
                raise KeyError(
                    "Could not find builtin symbol '{}'. (Are you running a "
    ...
    def find_isinstance_check(
        self, n: Expression
    ) -> Tuple[Optional[Dict[Expression, Type]], Optional[Dict[Expression, Type]]]:
...
def remove_optional(typ: Type) -> Type:
    if isinstance(typ, UnionType):
        return UnionType.make_union(
            [t for t in typ.items if not isinstance(t, NoneTyp)]
        )
    else:
        return typ
...
def is_same_arg_prefix(t: CallableType, s: CallableType) -> bool:
    # TODO check argument kinds
    return all(is_same_type(argt, args) for argt, args in zip(t.arg_types, s.arg_types))
