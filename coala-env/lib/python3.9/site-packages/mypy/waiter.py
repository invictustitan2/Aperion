...
class LazySubprocess:
    ...
    def __init__(
        self, name: str, args: List[str], *, cwd: str = None, env: Dict[str, str] = None
    ) -> None:
        self.name = name
        self.args = args
        self.cwd = cwd
    ...
    def start(self) -> None:
        ...
        self.process = Popen(
            self.args, cwd=self.cwd, env=self.env, stdout=self.outfile, stderr=STDOUT
        )
...
class Waiter:
    ...
    def __init__(
        self, limit: int = 0, *, verbosity: int = 0, xfail: List[str] = []
    ) -> None:
    ...
    def _report_task_failure(
        self, fail_type: Optional[str], num: int, name: str, output: str
    ) -> None:
        if self.verbosity <= 0:
            sys.stdout.write("\n")
    ...
    def run(self) -> int:
        ...
        if all_failures:
            ...
            print(
                "SUMMARY  all %d tasks and %d tests passed"
...
def parse_test_stats_from_output(
    output: str, fail_type: Optional[str]
) -> Tuple[int, int]:
    ...
    if m:
        ...
        for part in m.group(1).split(
            ", "
        ):  # e.g., '3 failed, 32 passed, 345 deselected'
            count, key = part.split()
            counts[key] = int(count)
        return (
            sum(c for k, c in counts.items() if k != "deselected"),
            counts.get("failed", 0),
        )

    # myunit
    m = re.search(
        "^([0-9]+)/([0-9]+) test cases failed(, ([0-9]+) skipped)?.$",
        output,
        re.MULTILINE,
    )
    if m:
    ...
    m = re.search(
        "^([0-9]+) test cases run(, ([0-9]+) skipped)?, all passed.$",
        output,
        re.MULTILINE,
    )
    if m:
