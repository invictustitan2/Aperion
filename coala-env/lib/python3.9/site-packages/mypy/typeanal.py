...
from mypy import experiments, nodes
...
def analyze_type_alias(
    ...
    if isinstance(node, RefExpr):
        if node.kind == UNBOUND_TVAR or node.kind == BOUND_TVAR:
            fail_func(
                'Type variable "{}" is invalid as target for type alias'.format(
                    node.fullname
                ),
                node,
            )
        ...
        if not (
            ...
            or node.fullname == "typing.Any"
            or node.kind == TYPE_ALIAS
        ):
            return None
        ...
        if isinstance(base, RefExpr):
            if not (
                isinstance(base.node, TypeInfo)
                or base.fullname in type_constructors
...
class TypeAnalyser(TypeVisitor[Type]):
    ...
    def visit_unbound_type(self, t: UnboundType) -> Type:
        if t.optional:
            ...
            return UnionType.make_simplified_union(
                [self.visit_unbound_type(t), NoneTyp()]
            )
        ...
        if sym is not None:
            if sym.node is None:
                ...
                if sym.kind != UNBOUND_IMPORTED:
                    self.fail(
                        "Internal error (node is None, kind={})".format(sym.kind), t
                    )
                return AnyType()
            ...
            if sym.kind == BOUND_TVAR:
                if len(t.args) > 0:
                    self.fail(
                        'Type variable "{}" used with arguments'.format(t.name), t
                    )
                ...
                if len(t.args) == 2 and isinstance(t.args[1], EllipsisType):
                    ...
                    instance = self.builtin_type(
                        "builtins.tuple", [t.args[0].accept(self)]
                    )
                ...
                if exp_len > 0 and act_len == 0:
                    ...
                    return self.replace_alias_tvars(
                        override, all_vars, [AnyType()] * exp_len, t.line, t.column
                    )
                if exp_len == 0 and act_len == 0:
                    return override
                if act_len != exp_len:
                    ...
                    return t
                return self.replace_alias_tvars(
                    override, all_vars, an_args, t.line, t.column
                )
            ...
            if len(t.args) > 0 and info.fullname() == "builtins.tuple":
                return TupleType(
                    self.anal_array(t.args), Instance(info, [AnyType()], t.line), t.line
                )
            else:
    ...
    def replace_alias_tvars(
        self, tp: Type, vars: List[str], subs: List[Type], newline: int, newcolumn: int
    ) -> Type:
        ...
        for i, arg in enumerate(typ_args):
            ...
            if tvar and tvar in vars:
                ...
                new_args[i] = self.replace_alias_tvars(
                    arg, vars, subs, newline, newcolumn
                )
    ...
    def visit_tuple_type(self, t: TupleType) -> Type:
        ...
        fallback = (
            t.fallback
            if t.fallback
            else self.builtin_type("builtins.tuple", [AnyType()])
        )
    ...
    def analyze_callable_type(self, t: UnboundType) -> Type:
        ...
        if len(t.args) == 0:
            ...
            if isinstance(t.args[0], TypeList):
                ...
                self.fail(
                    'The first argument to Callable must be a list of types or "..."', t
                )
                return AnyType()
    ...
    def builtin_type(
        self, fully_qualified_name: str, args: List[Type] = None
    ) -> Instance:
    ...
    def tuple_type(self, items: List[Type]) -> TupleType:
        return TupleType(
            items, fallback=self.builtin_type("builtins.tuple", [AnyType()])
        )
        ...
        for actual in actuals:
            if not isinstance(actual, AnyType) and not any(
                ...
                if len(actuals) > 1 or not isinstance(actual, Instance):
                    self.fail(
                        'Invalid type argument value for "{}"'.format(type.name()),
                        context,
                    )
                else:
                    self.fail(
                        'Type argument {} of "{}" has incompatible value "{}"'.format(
                            arg_number, type.name(), actual.type.name()
                        ),
                        context,
                    )
