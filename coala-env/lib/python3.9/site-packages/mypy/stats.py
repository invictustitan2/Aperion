...
class StatisticsVisitor(TraverserVisitor):
    def __init__(
        self,
        inferred: bool,
        typemap: Dict[Expression, Type] = None,
                ...
                for item in items:
                    if isinstance(item, RefExpr) and item.is_def:
                        ...
                        if t:
                            ...
                            self.log("  !! No inferred type on line %d" % self.line)
                            self.record_line(self.line, TYPE_ANY)
    ...
    def record_line(self, line: int, precision: int) -> None:
        self.line_map[line] = max(precision, self.line_map.get(line, TYPE_PRECISE))
...
def generate_html_report(
    tree: MypyFile, path: str, type_map: Dict[Expression, Type], output_dir: str
) -> None:
    if is_special_module(path):
        return
...
<pre>"""
    ...
    with open(path) as input_file:
        for i, line in enumerate(input_file):
            ...
            append(
                '<span class="lineno">%4d</span>   ' % lineno
...
<body>"""
    ...
    for source_path, target_path, num_lines, num_imprecise in sorted(html_files):
        ...
        # TODO: Windows paths.
        if source_path.startswith("stubs/") or "/stubs/" in source_path:
            continue
        ...
        if percent >= 20:
            ...
            style = 'class="yellow"'
        append(
            '<tr %s><td><a href="%s">%s</a><td>%.1f%% imprecise<td>%d LOC\n'
