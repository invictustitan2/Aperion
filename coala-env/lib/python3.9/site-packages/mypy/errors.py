...
class Errors:
    ...
    def __init__(
        self, hide_error_context: bool = False, show_column_numbers: bool = False
    ) -> None:
        self.error_info = []
        self.import_ctx = []
        self.type_name = [None]
    ...
    def add_error_info(self, info: ErrorInfo) -> None:
        if (
            info.file in self.ignored_lines
            and info.line in self.ignored_lines[info.file]
            ...
            if self.hide_error_context:
                ...
                if e.function_or_member is None:
                    if e.type is None:
                        ...
                        result.append(
                            (file, -1, -1, "note", 'In class "{}":'.format(e.type))
                        )
                else:
                    if e.type is None:
                        result.append(
    ...
    def remove_duplicates(
        self, errors: List[Tuple[str, int, int, str, str]]
    ) -> List[Tuple[str, int, int, str, str]]:
        ...
        while i < len(errors):
            ...
            while (
                ...
                if (
                    errors[j][3] == errors[i][3] and errors[j][4] == errors[i][4]
                ):  # ignore column
                    dup = True
                    break
                j -= 1
...
def report_internal_error(
    err: Exception, file: str, line: int, errors: Errors, options: Options
) -> None:
