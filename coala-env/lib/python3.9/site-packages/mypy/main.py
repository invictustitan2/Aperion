...
from mypy import build, defaults, experiments, git, util
from mypy.build import PYTHON_EXTENSIONS, BuildResult, BuildSource
...
def type_check_only(
    sources: List[BuildSource], bin_dir: str, options: Options
) -> BuildResult:
    # Type-check the program and dependencies and translate to Python.
    return build.build(sources=sources, bin_dir=bin_dir, options=options)
...
def process_options(
    ...
    parser.add_argument(
        "--python-version", type=parse_version, metavar="x.y", help="use Python x.y"
    )
    parser.add_argument(
        "--platform",
        action="store",
    ...
    parser.add_argument(
        "--fast-parser", action="store_true", help="enable experimental fast parser"
    )
    parser.add_argument(
        "-i",
        "--incremental",
    ...
    for report_type in sorted(reporter_classes):
        report_group.add_argument(
            "--%s-report" % report_type.replace("_", "-"),
            metavar="DIR",
    ...
    if special_opts.modules:
        ...
        if (
            os.sep in special_opts.package
            or os.altsep
            and os.altsep in special_opts.package
        ):
            fail(
                "Package name '{}' cannot have a slash in it.".format(
                    special_opts.package
                )
            )
        options.build_type = BuildType.MODULE
        ...
        if name.startswith("mypy-"):
            ...
            if set(updates) - Options.PER_MODULE_OPTIONS:
                print(
                    ...
                    % (
                        prefix,
                        ", ".join(sorted(set(updates) - Options.PER_MODULE_OPTIONS)),
                    ),
                    file=sys.stderr,
                )
                updates = {
                    k: v for k, v in updates.items() if k in Options.PER_MODULE_OPTIONS
                }
            globs = name[5:]
...
def parse_section(
    prefix: str, template: Options, section: Mapping[str, str]
    ...
    for key in section:
            ...
            if dv is None:
                if key.endswith("_report"):
                    ...
                    if report_type in reporter_classes:
                        ...
                        print(
                            "%s: Unrecognized report type: %s" % (prefix, key),
                            file=sys.stderr,
                        )
                    continue
                print(
                    "%s: Unrecognized option: %s = %s" % (prefix, key, section[key]),
                    file=sys.stderr,
                )
                continue
            ct = type(dv)
        ...
        try:
            if ct is bool:
                ...
                print(
                    "%s: Don't know what type %s should have" % (prefix, key),
                    file=sys.stderr,
                )
                continue
