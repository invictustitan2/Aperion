...
from mypy import defaults, experiments
...
try:
    ...
    if sys.version_info.minor > 2:
        print(
            "You must install the typed_ast package before you can run mypy"
            " with `--fast-parser`.\n"
        ...
        print(
            "The typed_ast package required by --fast-parser is only compatible with"
...
class ASTConverter(ast35.NodeTransformer):
    ...
    def do_func_def(
        ...
        if n.type_comment is not None:
            try:
                ...
                raise TypeCommentParseError(
                    TYPE_COMMENT_SYNTAX_ERROR, n.lineno, n.col_offset
                )
            ...
            if len(func_type_ast.argtypes) == 1 and isinstance(
                ...
                arg_types = [
                    a.type_annotation if a.type_annotation is not None else AnyType()
                    for a in args
                ]
            else:
                translated_args = TypeConverter(line=n.lineno).translate_expr_list(
                    func_type_ast.argtypes
                )
                arg_types = [a if a is not None else AnyType() for a in translated_args]
        ...
        if any(arg_types) or return_type:
            if len(arg_types) > len(arg_kinds):
                raise FastParserError(
                    "Type signature has too many arguments", n.lineno, offset=0
                )
            if len(arg_types) < len(arg_kinds):
                raise FastParserError(
                    "Type signature has too few arguments", n.lineno, offset=0
                )
            func_type = CallableType(
                [a if a is not None else AnyType() for a in arg_types],
                arg_kinds,
                arg_names,
        ...
        func_def = FuncDef(n.name, args, self.as_block(n.body, n.lineno), func_type)
        if is_coroutine:
    ...
    def transform_args(self, args: ast35.arguments, line: int) -> List[Argument]:
        def make_argument(
            arg: ast35.arg, default: Optional[ast35.expr], kind: int
        ) -> Argument:
    ...
    def visit_Lambda(self, n: ast35.Lambda) -> FuncExpr:
        ...
        return FuncExpr(
            self.transform_args(n.args, n.lineno), self.as_block([body], n.lineno)
        )
    ...
    def visit_Dict(self, n: ast35.Dict) -> DictExpr:
        return DictExpr(
            list(
                zip(
                    self.translate_expr_list(n.keys), self.translate_expr_list(n.values)
                )
            )
        )
    ...
    def visit_DictComp(self, n: ast35.DictComp) -> DictionaryComprehension:
        ...
        return DictionaryComprehension(
            self.visit(n.key), self.visit(n.value), targets, iters, ifs_list
        )
    ...
    def visit_Call(self, n: ast35.Call) -> CallExpr:
        ...
        arg_kinds = [
            ARG_STAR if isinstance(a, ast35.Starred) else ARG_POS for a in n.args
    ...
    def visit_Num(self, n: ast35.Num) -> Union[IntExpr, FloatExpr, ComplexExpr]:
        if getattr(n, "contains_underscores", None) and self.pyversion < (3, 6):
            raise FastParserError(
                "Underscores in numeric literals are only " "supported in Python 3.6",
                n.lineno,
                n.col_offset,
            )
    ...
    def visit_Attribute(self, n: ast35.Attribute) -> Union[MemberExpr, SuperExpr]:
        if (
            isinstance(n.value, ast35.Call)
            and isinstance(n.value.func, ast35.Name)
...
class TypeConverter(ast35.NodeTransformer):
    ...
    def generic_visit(self, node: ast35.AST) -> None:
        raise TypeCommentParseError(
            TYPE_COMMENT_AST_ERROR, self.line, getattr(node, "col_offset", -1)
        )
    ...
    def visit_Subscript(self, n: ast35.Subscript) -> Type:
        if not isinstance(n.slice, ast35.Index):
            raise TypeCommentParseError(
                TYPE_COMMENT_SYNTAX_ERROR, self.line, getattr(n, "col_offset", -1)
            )
        ...
        return UnboundType(
            value.name, params, line=self.line, empty_tuple_index=empty_tuple_index
        )

    def visit_Tuple(self, n: ast35.Tuple) -> Type:
        return TupleType(
            self.translate_expr_list(n.elts), None, implicit=True, line=self.line
        )
