...
INVALID_RETURN_TYPE_FOR_GENERATOR = 'The return type of a generator function should be "Generator" or one of its supertypes'
INVALID_GENERATOR_RETURN_ITEM_TYPE = "The return type of a generator function must be None in its third type parameter in Python 2"
...
class MessageBuilder:
    ...
    def report(
        self, msg: str, context: Context, severity: str, file: str = None
    ) -> None:
    ...
    def format_simple(self, typ: Type, verbosity: int = 0) -> str:
        ...
        if isinstance(typ, Instance):
            ...
            print_as_optional = (
                len(typ.items) - sum(isinstance(t, NoneTyp) for t in typ.items) == 1
            )
    ...
    def has_no_attr(self, typ: Type, member: str, context: Context) -> Type:
        ...
        """
        if isinstance(typ, Instance) and typ.type.has_readable_member(member):
            self.fail('Member "{}" is not assignable'.format(member), context)
        elif self.check_unusable_type(typ, context):
            pass
        elif member == "__contains__":
            self.fail(
                "Unsupported right operand type for in ({})".format(self.format(typ)),
                context,
            )
            ...
            self.fail(
                "Unsupported operand type for unary - ({})".format(self.format(typ)),
                context,
            )
        elif member == "__pos__":
            self.fail(
            ...
            self.fail(
                "Unsupported operand type for ~ ({})".format(self.format(typ)), context
            )
        elif member == "__getitem__":
            # Indexed get.
            self.fail(
                "Value of type {} is not indexable".format(self.format(typ)), context
            )
            ...
            if isinstance(typ, Instance) and (
                typ.type.fullname() == "builtins.function"
            ):
            ...
            if not self.disable_type_names:
                ...
                if isinstance(typ, Instance) and typ.type.names:
                    ...
                    matches = [
                        m for m in COMMON_MISTAKES.get(member, []) if m in alternatives
                    ]
                    matches.extend(best_matches(member, alternatives)[:3])
                    if matches:
                        self.fail(
                            '{} has no attribute "{}"; maybe {}?'.format(
                                self.format(typ), member, pretty_or(matches)
                            ),
                            context,
                        )
                        failed = True
                if not failed:
                    self.fail(
                        '{} has no attribute "{}"'.format(self.format(typ), member),
                        context,
                    )
            else:
                self.fail(
                    'Some element of union has no attribute "{}"'.format(member),
                    context,
                )
        return AnyType()

    def unsupported_operand_types(
        self, op: str, left_type: Any, right_type: Any, context: Context
    ) -> None:
        """Report unsupported operand types for a binary operation.

        Types can be Type objects or strings.
        """
        if self.check_unusable_type(left_type, context) or self.check_unusable_type(
            right_type, context
        ):
            return
        left_str = ""
        ...
        if callee.name:
            ...
            if name.startswith('"__setitem__" of'):
                if n == 1:
                    ...
                    arg_type_str, callee_type_str = self.format_distinctly(
    ...
    def invalid_index_type(
        self, index_type: Type, base_str: str, context: Context
    ) -> None:
        self.fail(
            "Invalid index type {} for {}".format(self.format(index_type), base_str),
            context,
        )

    def too_few_arguments(
        self, callee: CallableType, context: Context, argument_names: List[str]
    ) -> None:
        if (
            argument_names is not None
            and not all(k is None for k in argument_names)
    ...
    def unexpected_keyword_argument(
        self, callee: CallableType, name: str, context: Context
    ) -> None:
        ...
        if callee.definition:
            ...
            if fullname is not None and "." in fullname:
                ...
                self.note(
                    "{} defined here".format(callee.name), callee.definition, file=path
                )
    ...
    def duplicate_argument_value(
        ...
        self.fail(
            '{} gets multiple values for keyword argument "{}"'.format(
                capitalize(callable_name(callee)), callee.arg_names[index]
            ),
            context,
        )
    ...
    def does_not_return_value(self, unusable_type: Type, context: Context) -> None:
        ...
        if isinstance(unusable_type, Void) and unusable_type.source is not None:
            self.fail(
                "{} does not return a value".format(
                    capitalize((cast(Void, unusable_type)).source)
                ),
                context,
            )
        else:
    ...
    def no_variant_matches_arguments(
        self, overload: Overloaded, arg_types: List[Type], context: Context
    ) -> None:
        if overload.name():
            self.fail(
                "No overload variant of {} matches argument types {}".format(
                    overload.name(), arg_types
                ),
                context,
            )
        else:
            self.fail(
                "No overload variant matches argument types {}".format(arg_types),
                context,
            )

    def invalid_cast(
        self, target_type: Type, source_type: Type, context: Context
    ) -> None:
        if not self.check_unusable_type(source_type, context):
            self.fail(
                "Cannot cast from {} to {}".format(
                    self.format(source_type), self.format(target_type)
                ),
                context,
            )
    ...
    def wrong_number_values_to_unpack(
        ...
        if provided < expected:
            if provided == 1:
                self.fail(
                    "Need more than 1 value to unpack ({} expected)".format(expected),
                    context,
                )
            else:
                self.fail(
                    "Need more than {} values to unpack ({} expected)".format(
                        provided, expected
                    ),
                    context,
                )
            ...
            self.fail(
                "Too many values to unpack ({} expected, {} provided)".format(
                    expected, provided
                ),
                context,
            )
    ...
    def signature_incompatible_with_supertype(
        ...
        self.fail(
            'Signature of "{}" incompatible with {}'.format(name, target), context
        )
    ...
    def argument_incompatible_with_supertype(
        ...
        self.fail(
            'Argument {} of "{}" incompatible with {}'.format(arg_num, name, target),
            context,
        )

    def return_type_incompatible_with_supertype(
        self, name: str, name_in_supertype: str, supertype: str, context: Context
    ) -> None:
        target = self.override_target(name, name_in_supertype, supertype)
        self.fail(
            'Return type of "{}" incompatible with {}'.format(name, target), context
        )
    ...
    def incompatible_type_application(
        ...
        if expected_arg_count == 0:
            ...
            self.fail(
                "Type application has too many types ({} expected)".format(
                    expected_arg_count
                ),
                context,
            )
            ...
            self.fail(
                "Type application has too few types ({} expected)".format(
                    expected_arg_count
                ),
                context,
            )
    ...
    def could_not_infer_type_arguments(
        ...
        if callee_type.name and n > 0:
            self.fail(
                "Cannot infer type argument {} of {}".format(n, callee_type.name),
                context,
            )
        else:
    ...
    def invalid_keyword_var_arg(self, typ: Type, context: Context) -> None:
        if isinstance(typ, Instance) and (typ.type.fullname() == "builtins.dict"):
            ...
            self.fail("Argument after ** must be a dictionary", context)
    ...
    def check_unusable_type(self, typ: Type, context: Context) -> bool:
        ...
        """
        if isinstance(typ, Void) or (isinstance(typ, NoneTyp) and typ.is_ret_type):
    ...
    def cannot_determine_type_in_base(
        ...
        self.fail(
            "Cannot determine type of '%s' in base class '%s'" % (name, base), context
        )
    ...
    def read_only_property(self, name: str, type: TypeInfo, context: Context) -> None:
        self.fail(
            'Property "{}" defined in "{}" is read-only'.format(name, type.name()),
            context,
        )

    def incompatible_typevar_value(
        self, callee: CallableType, index: int, type: Type, context: Context
    ) -> None:
        self.fail(
            "Type argument {} of {} has incompatible value {}".format(
                index, callable_name(callee), self.format(type)
            ),
            context,
        )
    ...
    def forward_operator_not_callable(
        ...
        self.fail(
            'Forward operator "{}" is not callable'.format(forward_method), context
        )
    ...
    def signatures_incompatible(
        ...
        self.fail(
            'Signatures of "{}" and "{}" are incompatible'.format(method, other_method),
            context,
        )
...
def best_matches(current: str, options: Iterable[str]) -> List[str]:
    ...
    return sorted(
        (o for o in options if ratios[o] > 0.75),
        reverse=True,
        key=lambda v: (ratios[v], v),
    )
