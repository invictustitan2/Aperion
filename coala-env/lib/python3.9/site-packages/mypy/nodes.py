...
reverse_type_aliases = dict(
    (name.replace("__builtins__", "builtins"), alias)
...
class Argument(Node):
    ...
    def __init__(
        ...
        kind: int,
        initialization_statement: Optional["AssignmentStmt"] = None,
    ) -> None:
...
class FuncDef(FuncItem, SymbolNode, Statement):
    ...
    original_def = (
        None
    )  # type: Union[None, FuncDef, Var]  # Original conditional definition
...
class Decorator(SymbolNode, Statement):
    ...
    decorators = (
        None
    )  # type: List[Expression] # Decorators, at least one  # XXX Not true
...
class Var(SymbolNode):
    ...
    def deserialize(cls, data: JsonDict) -> "Var":
        ...
        name = data["name"]
        type = (
            None if data["type"] is None else mypy.types.Type.deserialize(data["type"])
        )
        v = Var(name, type)
...
class ForStmt(Statement):
    ...
    def __init__(
        self, index: Lvalue, expr: Expression, body: Block, else_body: Block
    ) -> None:
        self.index = index
        self.expr = expr
        self.body = body
...
class IfStmt(Statement):
    ...
    def __init__(
        self, expr: List[Expression], body: List[Block], else_body: Block
    ) -> None:
        self.expr = expr
        self.body = body
        self.else_body = else_body
...
class WithStmt(Statement):
    ...
    def __init__(
        self, expr: List[Expression], target: List[Lvalue], body: Block
    ) -> None:
        self.expr = expr
        self.target = target
        self.body = body
...
class PrintStmt(Statement):
    ...
    def __init__(
        self, args: List[Expression], newline: bool, target: Expression = None
    ) -> None:
        self.args = args
...
class ExecStmt(Statement):
    ...
    def __init__(
        ...
        variables2: Optional[Expression],
    ) -> None:
        self.expr = expr
...
class NameExpr(RefExpr):
    ...
    def deserialize(cls, data: JsonDict) -> "NameExpr":
        ...
        ret.kind = data["kind"]
        ret.node = (
            None if data["node"] is None else SymbolNode.deserialize(data["node"])
        )
...
ops_falling_back_to_cmp = {"__ne__", "__eq__", "__lt__", "__le__", "__gt__", "__ge__"}
...
class ListExpr(Expression):
    ...
    def __init__(self, items: List[Expression]) -> None:
        ...
        if all(x.literal == LITERAL_YES for x in items):
            ...
            self.literal_hash = (cast(Any, "List"),) + tuple(
                x.literal_hash for x in items
            )
...
class TupleExpr(Expression):
    ...
    def __init__(self, items: List[Expression]) -> None:
        ...
        if all(x.literal == LITERAL_YES for x in items):
            self.literal = LITERAL_YES
            self.literal_hash = (cast(Any, "Tuple"),) + tuple(
                x.literal_hash for x in items
            )
...
class SetExpr(Expression):
    ...
    def __init__(self, items: List[Expression]) -> None:
        ...
        if all(x.literal == LITERAL_YES for x in items):
            self.literal = LITERAL_YES
            self.literal_hash = (cast(Any, "Set"),) + tuple(
                x.literal_hash for x in items
            )
...
class SymbolTableNode:
    ...
    def type(self) -> "mypy.types.Type":
        ...
        if self.type_override is not None:
            ...
            isinstance(node, Var) or isinstance(node, FuncDef)
    ...
    def serialize(self, prefix: str, name: str) -> JsonDict:
        ...
        if self.kind == MODULE_REF:
            assert self.node is not None, "Missing module cross ref in %s for %s" % (
                prefix,
                name,
            )
...
def linearize_hierarchy(info: TypeInfo) -> List[TypeInfo]:
    ...
    lin_bases = []
    for base in bases:
        assert base is not None, "Cannot linearize bases for %s %s" % (
            info.fullname(),
            bases,
        )
