...
reporter_classes = (
    {}
)  # type: Dict[str, Tuple[Callable[[Reports, str], AbstractReporter], bool]]
...
class MemoryXmlReporter(AbstractReporter):
    ...
    def on_file(self, tree: MypyFile, type_map: Dict[Expression, Type]) -> None:
        ...
        visitor = stats.StatisticsVisitor(
            inferred=True, typemap=type_map, all_nodes=True
        )
        tree.accept(visitor)
        ...
        transform_pi = etree.ProcessingInstruction(
            "xml-stylesheet", 'type="text/xsl" href="%s"' % cgi.escape(xslt_path, True)
        )
...
class CoberturaPackage(object):
    ...
    def as_xml(self) -> Any:
        package_element = etree.Element("package", name=self.name, complexity="1.0")
...
class CoberturaXmlReporter(AbstractReporter):
    ...
    def on_file(self, tree: MypyFile, type_map: Dict[Expression, Type]) -> None:
        ...
        class_element = etree.Element(
            "class", filename=path, complexity="1.0", name=class_name
        )
