...
from mypy.types import AnyType, Instance, TupleType, Type, UnionType
...
class ConversionSpecifier:
    def __init__(
        self, key: str, flags: str, width: str, precision: str, type: str
    ) -> None:
        self.key = key
        self.flags = flags
        self.width = width
...
class StringFormatterChecker:
    ...
    def check_str_interpolation(
        self, str: Union[StrExpr, BytesExpr, UnicodeExpr], replacements: Expression
    ) -> Type:
    ...
    def parse_conversion_specifiers(self, format: str) -> List[ConversionSpecifier]:
        key_regex = (
            r"(\(([^()]*)\))?"  # (optional) parenthesised sequence of characters
        )
        flags_regex = r"([#0\-+ ]*)"  # (optional) sequence of flags
        width_regex = (
            r"(\*|[1-9][0-9]*)?"  # (optional) minimum field width (* or numbers)
        )
        precision_regex = (
            r"(?:\.(\*|[0-9]+)?)?"  # (optional) . followed by * of numbers
        )
    ...
    def check_simple_str_interpolation(
        self, specifiers: List[ConversionSpecifier], replacements: Expression
    ) -> None:
    ...
    def build_replacement_checkers(
        self, specifiers: List[ConversionSpecifier], context: Context
    ) -> List[Tuple[Callable[[Expression], None], Callable[[Type], None]]]:
        checkers = (
            []
    ...
    ) -> Tuple[Callable[[Expression], None], Callable[[Type], None]]:
    ...
    def checkers_for_c_type(
        ...
        def check_expr(expr: Expression) -> None:
            ...
            type = self.accept(expr, expected_type)
            if (
                isinstance(expr, (StrExpr, BytesExpr))
                and len(cast(StrExpr, expr).value) != 1
            ):
