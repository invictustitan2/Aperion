...
from mypy import moduleinfo, util
from mypy.checker import TypeChecker
from mypy.errors import CompileError, DecodeError, Errors, report_internal_error
...
class BuildSource:
    def __init__(
        self, path: Optional[str], module: Optional[str], text: Optional[str]
    ) -> None:
        self.path = path
...
def build(
    ...
    try:
        ...
        manager.log(
            "Build finished in %.3f seconds with %d modules, %d types, and %d errors"
            % (
...
def default_lib_path(
    ...
    versions = [
        "%d.%d" % (pyversion[0], minor) for minor in reversed(range(pyversion[1] + 1))
    ]
...
class BuildManager:
    ...
    def maybe_swap_for_shadow_path(self, path: str) -> str:
        if self.options.shadow_file and os.path.samefile(
            self.options.shadow_file[0], path
        ):
    ...
    def module_not_found(self, path: str, line: int, id: str) -> None:
        ...
        if (
            self.options.python_version[0] == 2 and moduleinfo.is_py2_std_lib_module(id)
        ) or (
            ...
            self.errors.report(
                line,
                0,
                "No library stub file for standard library module '{}'".format(id),
            )
            self.errors.report(line, 0, stub_msg, severity="note", only_once=True)
        elif moduleinfo.is_third_party_module(id):
            self.errors.report(
                line, 0, "No library stub file for module '{}'".format(id)
            )
            self.errors.report(line, 0, stub_msg, severity="note", only_once=True)
        else:
...
def remove_cwd_prefix_from_path(p: str) -> str:
    ...
    # Compute root path.
    while p and (
        os.path.isfile(os.path.join(p, "__init__.py"))
    ...
    if module_path.endswith(("__init__.py", "__init__.pyi")):
        ...
        for item in sorted(os.listdir(os.path.dirname(module_path))):
            ...
            if os.path.isdir(abs_path) and (
                os.path.isfile(os.path.join(abs_path, "__init__.py"))
                or os.path.isfile(os.path.join(abs_path, "__init__.pyi"))
            ):
                hits.add(item)
                result += find_modules_recursive(module + "." + item, lib_path)
            elif (
                item != "__init__.py"
                and item != "__init__.pyi"
                and item.endswith((".py", ".pyi"))
            ):
                mod = item.split(".")[0]
                if mod not in hits:
                    hits.add(mod)
                    result += find_modules_recursive(module + "." + mod, lib_path)
    return result
...
def verify_module(id: str, path: str) -> bool:
    ...
    for i in range(id.count(".")):
        path = dirname(path)
        if not any(
            os.path.isfile(os.path.join(path, "__init__{}".format(extension)))
            for extension in PYTHON_EXTENSIONS
        ):
            return False
    return True
...
def get_cache_names(
    id: str, path: str, cache_dir: str, pyversion: Tuple[int, int]
) -> Tuple[str, str]:
...
def find_cache_meta(id: str, path: str, manager: BuildManager) -> Optional[CacheMeta]:
    ...
    if not os.path.exists(meta_json):
        manager.trace(
            "Could not load cache for {}: could not find {}".format(id, meta_json)
        )
        return None
    ...
    if not isinstance(meta, dict):
        manager.trace(
            "Could not load cache for {}: meta cache is not a dict".format(id)
        )
        return None
    ...
    if (
        m.version_id != manager.version_id
        or m.options is None
        or len(m.dependencies) != len(m.dep_prios)
    ):
        manager.trace(
            "Metadata abandoned for {}: new attributes are missing".format(id)
        )
        return None
...
def is_meta_fresh(
    meta: Optional[CacheMeta], id: str, path: str, manager: BuildManager
) -> bool:
    if meta is None:
        return False
...
class State:
    ...
    def __init__(
        ...
        if not path and source is None:
            ...
            if path:
                ...
                if (
                    self.options.silent_imports
                    and path.endswith(".py")
                    and (caller_state or ancestor_for)
                ):
                    # (Never silence builtins, even if it's a .py file;
                    # this can happen in tests!)
                    if id != "builtins" and not (
                        (
                            caller_state
                            and caller_state.tree
                            and caller_state.tree.is_stub
                        )
                    ):
                ...
                if caller_state:
                    suppress_message = (
                        self.options.silent_imports and not self.options.almost_silent
                    )
        ...
        if is_meta_fresh(self.meta, self.id, self.path, manager):
            ...
            self.priorities = {
                id: pri for id, pri in zip(self.meta.dependencies, self.meta.dep_prios)
            }
    ...
    def skipping_ancestor(self, id: str, path: str, ancestor_for: "State") -> None:
        ...
        manager.errors.report(
            -1,
            -1,
            "Ancestor package '%s' silently ignored" % (id,),
            severity="note",
            only_once=True,
        )
        manager.errors.report(
            -1,
            -1,
            "(Using --silent-imports, submodule passed on command line)",
            severity="note",
            only_once=True,
        )
        manager.errors.report(
            -1,
            -1,
            "(This note brought to you by --almost-silent)",
            severity="note",
            only_once=True,
        )
    ...
    def skipping_module(self, id: str, path: str) -> None:
        ...
        manager.errors.report(
            line, 0, "Import of '%s' silently ignored" % (id,), severity="note"
        )
        manager.errors.report(
            line,
            0,
        ...
        manager.errors.report(
            line,
            0,
            "(This note courtesy of --almost-silent)",
            severity="note",
            only_once=True,
        )
        manager.errors.set_import_context(save_import_context)
    ...
    def has_new_submodules(self) -> bool:
        ...
        return self.meta is not None and self.child_modules != set(
            self.meta.child_modules
        )
        ...
        with self.wrap_context():
            ...
            if self.path and source is None:
                try:
                    ...
                    source = read_with_python_encoding(
                    ...
                    raise CompileError(
                        [
                            "mypy: can't decode file '{}': {}".format(
                                self.path, str(decodeerr)
                            )
                        ]
                    )
    ...
    def semantic_analysis(self) -> None:
        with self.wrap_context():
            self.manager.semantic_analyzer.visit_file(
                self.tree, self.xpath, self.options
            )
    ...
    def type_check_first_pass(self) -> None:
        ...
        with self.wrap_context():
            self.type_checker = TypeChecker(
                manager.errors, manager.modules, self.options, self.tree, self.xpath
            )
    ...
    def write_cache(self) -> None:
        if (
            ...
            and not self.manager.errors.is_errors()
        ):
            dep_prios = [
                self.priorities.get(dep, PRI_HIGH) for dep in self.dependencies
            ]
            ...
            if new_interface_hash == self.interface_hash:
                ...
                self.manager.log(
                    "Cached module {} has changed interface".format(self.id)
                )
...
def process_graph(graph: Graph, manager: BuildManager) -> None:
    ...
    for ascc in sccs:
        ...
        if fresh:
            ...
            newest_in_deps = (
                0 if not viable else max(graph[dep].meta.data_mtime for dep in viable)
            )
            if manager.options.verbosity >= 3:  # Dump all mtimes for extreme debugging.
                all_ids = sorted(
                    ascc | viable, key=lambda id: graph[id].meta.data_mtime
                )
                for id in all_ids:
                    if id in scc:
            ...
            if oldest_in_scc < newest_in_deps:
                fresh = False
                fresh_msg = "out of date by %.0f seconds" % (
                    newest_in_deps - oldest_in_scc
                )
            else:
                fresh_msg = "fresh"
        elif undeps:
            fresh_msg = "stale due to changed suppression (%s)" % " ".join(
                sorted(undeps)
            )
        elif stale_scc:
        ...
        if fresh:
            ...
            if len(fresh_scc_queue) > 0:
                manager.log(
                    "Processing the last {} queued SCCs".format(len(fresh_scc_queue))
                )
            ...
            if size == 1:
                manager.log(
                    "Processing SCC singleton (%s) as %s" % (scc_str, fresh_msg)
                )
            else:
                manager.log(
    ...
    if sccs_left:
        manager.log(
            "{} fresh SCCs left in queue (and will remain unprocessed)".format(
                sccs_left
            )
        )
...
def sorted_components(
    graph: Graph, vertices: Optional[AbstractSet[str]] = None, pri_max: int = PRI_ALL
