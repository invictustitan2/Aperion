...
class SemanticAnalyzer(NodeVisitor):
            ...
            if defn.is_coroutine and isinstance(defn.type, CallableType):
                ...
                defn.type = defn.type.copy_modified(
                    ret_type=self.named_type_or_none(
                        "typing.Awaitable", [defn.type.ret_type]
                    )
                )
            self.errors.pop_function()
    ...
    def infer_type_variables(self, type: CallableType) -> List[Tuple[str, TypeVarExpr]]:
    ...
    def visit_overloaded_func_def(self, defn: OverloadedFuncDef) -> None:
        ...
        if self.is_class_scope():
            self.type.names[defn.name()] = SymbolTableNode(MDEF, defn, typ=defn.type)
            defn.info = self.type
        elif self.is_func_scope():
            self.add_local(defn, defn)
    ...
    def analyze_function(self, defn: FuncItem) -> None:
        ...
        next_function_tvar_id = min(
            [self.next_function_tvar_id()]
        ...
        self.postpone_nested_functions_stack.append(
    ...
    def bind_type_var(
        self, fullname: str, tvar_def: TypeVarDef, context: Context
    ) -> SymbolTableNode:
        ...
        if sym.node.fullname() == "typing.Generic":
            ...
            for arg in unbound.args:
                ...
                if tvar:
                    ...
                    self.fail(
                        "Free type variable expected in %s[...]" % sym.node.name(), t
                    )
            return tvars
        return None
    ...
    def check_namedtuple_classdef(self, defn: ClassDef) -> Tuple[List[str], List[Type]]:
        ...
        for stmt in defn.defs.body:
            if not isinstance(stmt, AssignmentStmt):
                ...
                items.append(name)
                types.append(
                    AnyType() if stmt.type is None else self.anal_type(stmt.type)
                )
                # ...despite possible minor failures that allow further analyzis.
                if name.startswith("_"):
                    self.fail(
                        "NamedTuple field name cannot start with an underscore: {}".format(
                            name
                        ),
                        stmt,
                    )
                if (
                    ...
                    self.fail(
                        "Right hand side values are not supported in NamedTuple", stmt
                    )
        return items, types
    ...
    def analyze_base_classes(self, defn: ClassDef) -> None:
        ...
        for base_expr in defn.base_type_exprs:
            ...
            if isinstance(base, TupleType):
                if info.tuple_type:
                    self.fail(
                        "Class has two incompatible bases derived from tuple", defn
                    )
                ...
                if self.options.disallow_subclassing_any:
                    if isinstance(base_expr, (NameExpr, MemberExpr)):
                        msg = "Class cannot subclass '{}' (has type 'Any')".format(
                            base_expr.name
                        )
                    else:
    ...
    def verify_base_classes(self, defn: ClassDef) -> bool:
        ...
        for base in info.bases:
            ...
            if baseinfo.fullname() == "builtins.bool":
                self.fail(
                    "'%s' is not a valid base class" % baseinfo.name(),
                    defn,
                    blocker=True,
                )
                return False
    ...
    def named_type_or_none(
        self, qualified_name: str, args: List[Type] = None
    ) -> Instance:
        ...
        for id, as_id in i.ids:
            if as_id is not None:
                ...
                self.add_module_symbol(
                    base, base, module_public=module_public, context=i
                )
    ...
    def add_submodules_to_parent_modules(self, id: str, module_public: bool) -> None:
        ...
        while "." in id:
            ...
            if parent_mod and child not in parent_mod.names:
                ...
                if child_mod:
                    sym = SymbolTableNode(
                        MODULE_REF, child_mod, parent, module_public=module_public
                    )
                    parent_mod.names[child] = sym
            id = parent

    def add_module_symbol(
        self, id: str, as_id: str, module_public: bool, context: Context
    ) -> None:
        if id in self.modules:
            ...
            self.add_symbol(
                as_id,
                SymbolTableNode(
                    MODULE_REF, m, self.cur_mod_id, module_public=module_public
                ),
                context,
            )
        else:
    ...
    def process_import_over_existing_name(
        ...
        if existing_symbol.kind in (LDEF, GDEF, MDEF) and isinstance(
            existing_symbol.node, (Var, FuncDef, TypeInfo)
        ):
    ...
    def normalize_type_alias(
        self, node: SymbolTableNode, ctx: Context
    ) -> SymbolTableNode:
            ...
            for name, node in m.names.items():
                ...
                if not name.startswith("_") and node.module_public:
                    ...
                    self.add_symbol(
                        name,
                        SymbolTableNode(
                            node.kind, node.node, self.cur_mod_id, node.type_override
                        ),
                        i,
                    )
        else:
    ...
    def add_unknown_symbol(
        self, name: str, context: Context, is_import: bool = False
    ) -> None:
        var = Var(name)
        if self.type:
    ...
    def anal_type(
        self, t: Type, allow_tuple_literal: bool = False, aliasing: bool = False
    ) -> Type:
        if t:
            if allow_tuple_literal:
                ...
                if isinstance(t, TupleType):
                    ...
                    star_count = sum(
                        1 for item in t.items if isinstance(item, StarType)
                    )
                    if star_count > 1:
                        self.fail("At most one star type allowed in a tuple", t)
                        return TupleType(
                            [AnyType() for _ in t.items],
                            self.builtin_type("builtins.tuple"),
                            t.line,
                        )
                    items = [self.anal_type(item, True) for item in t.items]
            ...
            if (
                ...
                if res and (not isinstance(res, Instance) or res.args):
                    ...
                    if isinstance(s.rvalue, IndexExpr):
                        s.rvalue.analyzed = TypeAliasExpr(
                            res, fallback=self.alias_fallback(res)
                        )
        if s.type:
    ...
    def alias_fallback(self, tp: Type) -> Instance:
        ...
        kind = (
            ...
            if isinstance(tp, CallableType)
                ...
                else " to Union" if isinstance(tp, UnionType) else ""
            )
        )
    ...
    def analyze_lvalue(
        ...
        if isinstance(lval, NameExpr):
            ...
            nested_global = (
                not self.is_func_scope() and self.block_depth[-1] > 0 and not self.type
            )
            if (add_global or nested_global) and lval.name not in self.globals:
                ...
                self.globals[lval.name] = SymbolTableNode(GDEF, v, self.cur_mod_id)
            ...
            elif not self.is_func_scope() and (
                self.type and lval.name not in self.type.names
            ):
            ...
            if not add_global:
                lval.accept(self)
        elif isinstance(lval, TupleExpr) or isinstance(lval, ListExpr):
            items = lval.items
    ...
    def analyze_tuple_or_list_lvalue(
        ...
        if len(star_exprs) > 1:
            ...
            for i in items:
                self.analyze_lvalue(
                    i, nested=True, add_global=add_global, explicit_type=explicit_type
                )

    def analyze_member_lvalue(self, lval: MemberExpr) -> None:
        lval.accept(self)
        if self.is_self_member_ref(lval) and self.type.get(lval.name) is None:
    ...
    def check_lvalue_validity(
        self, node: Union[Expression, SymbolNode], ctx: Context
    ) -> None:
    ...
    def process_newtype_declaration(self, s: AssignmentStmt) -> None:
        ...
        if isinstance(old_type, TupleType):
            newtype_class_info = self.build_newtype_typeinfo(
                name, old_type, old_type.fallback
            )
    ...
    def build_newtype_typeinfo(
        self, name: str, old_type: Type, base_type: Instance
    ) -> TypeInfo:
        ...
        # Add __init__ method
        args = [
            Argument(Var("cls"), NoneTyp(), None, ARG_POS),
    ...
    def process_typevar_parameters(
        ...
        for param_value, param_name, param_kind in zip(args, names, kinds):
            ...
            if param_name == "covariant":
                ...
                self.fail(
                    "Unexpected argument to TypeVar(): {}".format(param_name), context
                )
                return None
    ...
    def parse_namedtuple_args(
        ...
        if not isinstance(args[1], (ListExpr, TupleExpr)):
            ...
            if fullname == "collections.namedtuple":
                ...
                items, types, ok = self.parse_namedtuple_fields_with_types(
                    listexpr.items, call
                )
        if not types:
    ...
    def parse_namedtuple_fields_with_types(
        ...
        for item in nodes:
            if isinstance(item, TupleExpr):
                if len(item.items) != 2:
                    return self.fail_namedtuple_arg(
                        "Invalid NamedTuple field definition", item
                    )
                ...
                if isinstance(name, (StrExpr, BytesExpr, UnicodeExpr)):
                    ...
                    return self.fail_namedtuple_arg(
                        "Invalid NamedTuple() field name", item
                    )
                try:
                ...
                return self.fail_namedtuple_arg(
                    "Tuple expected as NamedTuple() field", item
                )
        return items, types, True

    def fail_namedtuple_arg(
    ...
    def build_namedtuple_typeinfo(
        ...
        dictype = (
            self.named_type_or_none("builtins.dict", [strtype, AnyType()])
            or self.object_type()
        )
    ...
    def parse_typeddict_fields_with_types(
        self, dict_items: List[Tuple[Expression, Expression]], context: Context
    ) -> Tuple[List[str], List[Type], bool]:
        items = []  # type: List[str]
        types = []  # type: List[Type]
        for field_name_expr, field_type_expr in dict_items:
            if isinstance(field_name_expr, (StrExpr, BytesExpr, UnicodeExpr)):
                ...
                return self.fail_typeddict_arg(
                    "Invalid TypedDict() field name", field_name_expr
                )
            try:
    ...
    def visit_decorator(self, dec: Decorator) -> None:
        ...
        for i, d in enumerate(dec.decorators):
            ...
            if refers_to_fullname(d, "abc.abstractmethod"):
                ...
                d, "asyncio.coroutines.coroutine"
            ) or refers_to_fullname(d, "types.coroutine"):
                removed.append(i)
            ...
            elif refers_to_fullname(d, "builtins.property") or refers_to_fullname(
        ...
        if not dec.is_overload or dec.var.is_property:
            if self.is_func_scope():
                self.add_symbol(dec.var.name(), SymbolTableNode(LDEF, dec), dec)
            elif self.type:
                dec.var.info = self.type
                dec.var.is_initialized_in_class = True
                self.add_symbol(dec.var.name(), SymbolTableNode(MDEF, dec), dec)
    ...
    def visit_if_stmt(self, s: IfStmt) -> None:
        infer_reachability_of_if_statement(
            s, pyversion=self.options.python_version, platform=self.options.platform
        )
    ...
    def analyze_try_stmt(
        self, s: TryStmt, visitor: NodeVisitor, add_global: bool = False
    ) -> None:
    ...
    def translate_dict_call(self, call: CallExpr) -> Optional[DictExpr]:
        ...
        if not all(kind == ARG_NAMED for kind in call.arg_kinds):
            ...
            return None
        expr = DictExpr(
            [(StrExpr(key), value) for key, value in zip(call.arg_names, call.args)]
        )
        expr.set_line(call)
        expr.accept(self)
        return expr
    ...
    def visit_index_expr(self, expr: IndexExpr) -> None:
        ...
        if isinstance(expr.base, RefExpr) and expr.base.kind == TYPE_ALIAS:
            ...
            res = analyze_type_alias(
                expr, self.lookup_qualified, self.lookup_fully_qualified, self.fail
            )
            expr.analyzed = TypeAliasExpr(
                res, fallback=self.alias_fallback(res), in_runtime=True
            )
    ...
    def analyze_comp_for(
        self, expr: Union[GeneratorExpr, DictionaryComprehension]
    ) -> None:
    ...
    def check_for_obsolete_short_name(self, name: str, ctx: Context) -> None:
        ...
        if len(matches) == 1:
            self.note(
                "(Did you mean '{}'?)".format(obsolete_name_mapping[matches[0]]), ctx
            )
        ...
        if self.is_func_scope():
            ...
            if (
                ...
                if (
                    existing.type
                    and node.type
                    and is_same_type(existing.type, node.type)
                ):
                    ok = True
                if not ok:
                    self.name_already_defined(name, context)
    ...
    def add_local(
        self, node: Union[Var, FuncDef, OverloadedFuncDef], ctx: Context
    ) -> None:
        name = node.name()
        if name in self.locals[-1]:
    ...
    def check_no_global(
        self, n: str, ctx: Context, is_overloaded_func: bool = False
    ) -> None:
    ...
    def note(self, msg: str, ctx: Context) -> None:
        if (
            not self.options.check_untyped_defs
            and self.function_stack
            and self.function_stack[-1].is_dynamic()
        ):
            return
...
class FirstPass(NodeVisitor):
    ...
    def visit_overloaded_func_def(self, func: OverloadedFuncDef) -> None:
        ...
        if kind == GDEF:
            self.sem.globals[func.name()] = SymbolTableNode(
                kind, func, self.sem.cur_mod_id
            )
    ...
    def visit_class_def(self, cdef: ClassDef) -> None:
        ...
        cdef.info = info
        if kind == GDEF:
            self.sem.globals[cdef.name] = SymbolTableNode(
                kind, info, self.sem.cur_mod_id
            )
    ...
    def visit_import_from(self, node: ImportFrom) -> None:
        ...
        for name, as_name in node.names:
            ...
            if imported_name not in self.sem.globals:
                self.sem.add_symbol(
                    imported_name, SymbolTableNode(UNBOUND_IMPORTED, None), node
                )
    ...
    def visit_import(self, node: Import) -> None:
        ...
        for id, as_id in node.ids:
            ...
            if imported_id not in self.sem.globals:
                self.sem.add_symbol(
                    imported_id, SymbolTableNode(UNBOUND_IMPORTED, None), node
                )
            else:
    ...
    def visit_decorator(self, d: Decorator) -> None:
        ...
        self.sem.add_symbol(
            d.var.name(), SymbolTableNode(self.kind_by_scope(), d.var), d
        )

    def visit_if_stmt(self, s: IfStmt) -> None:
        infer_reachability_of_if_statement(
            s, pyversion=self.pyversion, platform=self.platform
        )
        for node in s.body:
            node.accept(self)
        if s.else_body:
    ...
    def analyze_lvalue(self, lvalue: Lvalue, explicit_type: bool = False) -> None:
        self.sem.analyze_lvalue(
            lvalue, add_global=self.sem.is_module_scope(), explicit_type=explicit_type
        )
...
class ThirdPass(TraverserVisitor):
    ...
    def visit_assignment_stmt(self, s: AssignmentStmt) -> None:
        self.analyze(s.type)
        if isinstance(s.rvalue, IndexExpr) and isinstance(
            s.rvalue.analyzed, TypeAliasExpr
        ):
...
def set_callable_name(sig: Type, fdef: FuncDef) -> Type:
    if isinstance(sig, FunctionLike):
        if fdef.info:
            return sig.with_name('"{}" of "{}"'.format(fdef.name(), fdef.info.name()))
        else:
...
def infer_reachability_of_if_statement(
    s: IfStmt, pyversion: Tuple[int, int], platform: str
) -> None:
    for i in range(len(s.expr)):
...
def infer_if_condition_value(
    expr: Expression, pyversion: Tuple[int, int], platform: str
) -> int:
