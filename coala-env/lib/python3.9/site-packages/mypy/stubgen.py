...
def generate_stub_for_module(
    ...
    result = find_module_path_and_all(
        module=module,
        pyversion=pyversion,
        no_import=no_import,
...
def load_python_module_info(
    ...
    if not module_path.endswith((".py", ".pyc", ".pyo")):
        raise SystemExit(
            "%s looks like a C module; they are not supported for Python 2" % module
        )
...
class StubGenerator(mypy.traverser.TraverserVisitor):
    ...
    def visit_decorator(self, o: Decorator) -> None:
        ...
        for decorator in o.decorators:
            if isinstance(decorator, NameExpr) and decorator.name in (
                "property",
                "staticmethod",
                "classmethod",
            ):
    ...
    def is_namedtuple(self, expr: Expression) -> bool:
        ...
        callee = expr.callee
        return (
            isinstance(callee, NameExpr) and callee.name.endswith("namedtuple")
        ) or (isinstance(callee, MemberExpr) and callee.name == "namedtuple")
    ...
    def visit_import_from(self, o: ImportFrom) -> None:
        ...
        if self._all_:
            ...
            names = [
                name for name, alias in o.names if name in self._all_ and alias is None
            ]
            ...
            if o.relative:
                sub_names = [name for name, alias in o.names if alias is None]
    ...
    def import_and_export_names(
        self, module_id: str, relative: int, names: Iterable[str]
    ) -> None:
    ...
    def visit_import(self, o: Import) -> None:
        for id, as_id in o.ids:
            ...
            if (
                self._all_
                and target_name in self._all_
                and (as_id is not None or "." not in id)
            ):
    ...
    def get_str_type_of_node(
        self, rvalue: Expression, can_infer_optional: bool = False
    ) -> str:
...
def walk_packages(packages: List[str]):
    for package_name in packages:
        ...
        for importer, qualified_name, ispkg in pkgutil.walk_packages(
            package.__path__, prefix=package.__name__ + ".", onerror=lambda r: None
        ):
...
def parse_options() -> Options:
    ...
    if not interpreter:
        interpreter = (
            sys.executable if pyversion[0] == 3 else default_python2_interpreter()
        )
    return Options(
        pyversion=pyversion,
        no_import=no_import,
