...
from mypy import defaults, experiments, lex, nodes
...
from mypy.parsetype import TypeParseError, parse_signature, parse_type, parse_types
from mypy.types import AnyType, CallableType, Type, UnboundType
...
op_comp = set([">", "<", "==", ">=", "<=", "<>", "!=", "is", "is", "in", "not"])

none = Token("")  # Empty token


def parse(
    source: Union[str, bytes], fnam: str, errors: Errors, options: Options
) -> MypyFile:
...
class Parser:
    ...
    def parse(self, s: Union[str, bytes]) -> MypyFile:
        self.tok, self.ignored_lines = lex.lex(
            s, pyversion=self.pyversion, is_stub_file=self.is_stub_file
        )
        self.ind = 0
        self.imports = []
        self.future_options = []
            ...
            while True:
                ...
                if "%s.%s" % (name, id) == self.custom_typing_module:
                    if targets or self.current_str() == ",":
                        self.fail(
                            "You cannot import any other modules when you "
    ...
    def parse_function(self, no_type_checks: bool = False) -> FuncDef:
        ...
        try:
            (name, args, typ, is_error, extra_stmts) = self.parse_function_header(
                no_type_checks
            )
            ...
            if comment_type:
                ...
                if sig.is_ellipsis_args:
                    ...
                    self.check_argument_kinds(
                        arg_kinds, sig.arg_kinds, def_tok.line, def_tok.column
                    )
    ...
    def check_argument_kinds(
        self, funckinds: List[int], sigkinds: List[int], line: int, column: int
    ) -> None:
        ...
        if len(funckinds) != len(sigkinds):
            ...
            return
        for kind, token in [(nodes.ARG_STAR, "*"), (nodes.ARG_STAR2, "**")]:
            if (funckinds.count(kind) != sigkinds.count(kind)) or (
                kind in funckinds and sigkinds.index(kind) != funckinds.index(kind)
            ):
                self.fail(
                    "Inconsistent use of '{}' in function " "signature".format(token),
                    line,
                    column,
                )

    def parse_function_header(
        self, no_type_checks: bool = False
    ) -> Tuple[str, List[Argument], CallableType, bool, List[AssignmentStmt]]:
    ...
    def parse_args(
        self, no_type_checks: bool = False
    ) -> Tuple[List[Argument], CallableType, List[AssignmentStmt]]:
    ...
    def build_func_annotation(
        ...
        if (ret_type and not is_default_ret) or arg_types != [None] * len(arg_types):
    ...
    def parse_arg_list(
        self, allow_signature: bool = True, no_type_checks: bool = False
    ) -> Tuple[List[Argument], List[AssignmentStmt]]:
    ...
    def check_duplicate_argument_names(self, names: List[str]) -> None:
        ...
        for name in names:
            if name in found:
                self.fail(
                    'Duplicate argument name "{}"'.format(name),
                    self.current().line,
                    self.current().column,
                )
            found.add(name)

    def parse_asterisk_arg(
        self, allow_signature: bool, no_type_checks: bool
    ) -> Argument:
        asterisk = self.skip()
    ...
    def parse_tuple_arg(self, index: int) -> Tuple[Argument, AssignmentStmt, List[str]]:
        ...
        if self.pyversion[0] >= 3:
            self.fail(
                "Tuples in argument lists only supported in Python 2 mode", line, column
            )
    ...
    def verify_tuple_arg(self, paren_arg: Expression) -> None:
        if isinstance(paren_arg, TupleExpr):
            if not paren_arg.items:
                self.fail(
                    "Empty tuple not valid as an argument",
                    paren_arg.line,
                    paren_arg.column,
                )
            ...
            self.fail(
                "Invalid item in tuple argument", paren_arg.line, paren_arg.column
            )
    ...
    def parse_normal_arg(
        self, require_named: bool, allow_signature: bool, no_type_checks: bool
    ) -> Tuple[Argument, bool]:
        name = self.expect_type(Name)
        variable = Var(name.string)
    ...
    def verify_argument_kinds(self, kinds: List[int], line: int, column: int) -> None:
        ...
        for i, kind in enumerate(kinds):
            if kind == nodes.ARG_POS and found & set(
                [nodes.ARG_OPT, nodes.ARG_STAR, nodes.ARG_STAR2]
            ):
    ...
    def construct_function_type(
        self, args: List[Argument], ret_type: Type, line: int, column: int
    ) -> CallableType:
    ...
    def parse_block(self, allow_type: bool = False) -> Tuple[Block, Type]:
        ...
        if not isinstance(self.current(), Break):
            ...
            while not isinstance(self.current(), Dedent) and not isinstance(
                self.current(), Eof
            ):
                try:
                    stmt, is_simple = self.parse_statement()
    ...
    def parse_statement(self) -> Tuple[Statement, bool]:
        ...
        if ts == "if":
            ...
            self.pyversion[0] == 2 and "print_function" not in self.future_options
        ):
    ...
    def parse_expression_or_assignment(
        self,
    ) -> Union[AssignmentStmt, OperatorAssignmentStmt, ExpressionStmt]:
    ...
    def parse_yield_or_yield_from_expr(self) -> Union[YieldFromExpr, YieldExpr]:
        ...
        if not isinstance(self.current(), Break):
            if self.current_str() == "from":
                ...
                expr = (
                    self.parse_expression()
                )  # when yield from is assigned to a variable
    ...
    def parse_for_index_variables(self) -> Expression:
        ...
        while True:
            v = self.parse_expression(
                precedence["in"], star_expr_allowed=True
            )  # Prevent parsing of for stmt 'in'
    ...
    def parse_expression(
        self, prec: int = 0, star_expr_allowed: bool = False
    ) -> Expression:
        ...
        if s == "(":
            ...
            elif isinstance(current, EllipsisToken) and (
                self.pyversion[0] >= 3 or self.is_stub_file
            ):
    ...
    def parse_list_expr(self) -> Union[ListExpr, ListComprehension]:
        ...
        while self.current_str() != "]" and not self.eol():
            items.append(
                self.parse_expression(precedence["<for>"], star_expr_allowed=True)
            )
            if self.current_str() != ",":
                break
            self.expect(",")
    ...
    def parse_comp_for(
        self,
    ) -> Tuple[List[Expression], List[Expression], List[List[Expression]]]:
        indices = []
        sequences = []
    ...
    def parse_dict_or_set_expr(
        self,
    ) -> Union[SetComprehension, SetExpr, DictionaryComprehension, DictExpr]:
    ...
    def parse_dict_comprehension(
        self, key: Expression, value: Expression, colon: Token
    ) -> DictionaryComprehension:
        indices, sequences, condlists = self.parse_comp_for()
    ...
    def parse_tuple_expr(
        self, expr: Expression, prec: int = precedence[","]
    ) -> TupleExpr:
        items = [expr]
        while True:
            self.expect(",")
            if self.current_str() in [")", "]", "=", ":"] or isinstance(
                self.current(), Break
            ):
                break
    ...
    def parse_str_expr(self) -> Union[UnicodeExpr, StrExpr]:
        ...
        while isinstance(self.current(), (StrLit, UnicodeLit)):
            ...
            if isinstance(token, StrLit):
                ...
                is_unicode = True
        if is_unicode or (
            self.pyversion[0] == 2 and "unicode_literals" in self.future_options
        ):
    ...
    def parse_lambda_expr(self) -> FuncExpr:
        ...
        typ = self.build_func_annotation(
            ret_type, args, lambda_tok.line, lambda_tok.column, is_default_ret=True
        )
    ...
    def expect_indent(self) -> Token:
        if isinstance(self.current(), Indent):
            ...
            self.fail(
                "Expected an indented block", self.current().line, self.current().column
            )
            return none
    ...
    def parse_error_at(
        self, tok: Token, skip: bool = True, reason: Optional[str] = None
    ) -> None:
        msg = ""
    ...
    def skip_until_break(self) -> None:
        n = 0
        while not isinstance(self.current(), Break) and not isinstance(
            self.current(), Eof
        ):
            self.skip()
            n += 1
...
def token_repr(tok: Token) -> str:
    ...
    if isinstance(tok, Break):
        ...
        isinstance(tok, IntLit)
        or isinstance(tok, FloatLit)
        or isinstance(tok, ComplexLit)
    ):
