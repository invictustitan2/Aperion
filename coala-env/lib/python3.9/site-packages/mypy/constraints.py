    ...
    for i, actuals in enumerate(formal_to_actual):
        for actual in actuals:
            ...
            actual_type = get_actual_type(
                arg_types[actual], arg_kinds[actual], tuple_counter
            )
...
class ConstraintBuilderVisitor(TypeVisitor[List[Constraint]]):
    ...
    def visit_callable_type(self, template: CallableType) -> List[Constraint]:
        if isinstance(self.actual, CallableType):
            ...
            res.extend(infer_constraints(template.ret_type, AnyType(), self.direction))
            return res
    ...
    def visit_tuple_type(self, template: TupleType) -> List[Constraint]:
        ...
        if isinstance(actual, TupleType) and len(actual.items) == len(template.items):
            ...
            for i in range(len(template.items)):
                res.extend(
                    infer_constraints(
                        template.items[i], actual.items[i], self.direction
                    )
                )
            return res
