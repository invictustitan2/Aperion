...
from lib2to3.main import StdoutRefactoringTool, warn
...
def main(args=None):
    ...
    parser.add_option(
        "-0", "--both-stages", action="store_true", help="Apply both stages 1 and 2"
    )
    parser.add_option(
        "-u",
        "--unicode-literals",
    ...
    for fix in options.nofix:
        if ".fix_" in fix:
            ...
            if len(found) > 1:
                ...
                print(
                    "Unknown fixer. Use --list-fixes or -l for a list.", file=sys.stderr
                )
                return 2
    ...
    if options.all_imports:
        if options.stage1:
            ...
            extra_fixes.add(prefix + "fix_add__future__imports_except_unicode_literals")
        else:
    ...
    if options.fix:
        ...
        for fix in options.fix:
            if fix == "all":
                ...
                if ".fix_" in fix:
                    ...
                    # lib2to3.fixes.fix_blah and libfuturize.fixes.fix_blah):
                    found = [
                        f for f in avail_fixes if f.endswith("fix_{0}".format(fix))
                    ]
                    if len(found) > 1:
                        print(
                            "Ambiguous fixer name. Choose a fully qualified "
                        ...
                        print(
                            "Unknown fixer. Use --list-fixes or -l for a list.",
                            file=sys.stderr,
                        )
                        return 2
        ...
        if len(explicit & unwanted_fixes) > 0:
            print(
                "Conflicting usage: the following fixers have been "
