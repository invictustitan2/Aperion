...
def CheckLineLength(filename, linenumber, clean_lines, errors):
    ...
    if len(line) > _lint_state.linelength:
        return errors(
            ...
            linenumber,
            "linelength",
            "Lines should be <= %d characters long" % (_lint_state.linelength),
        )
...
def IsValidFile(filename):
    return (
        ...
        or os.path.basename(filename).lower() == "cmakelists.txt"
    )
...
def CheckLintPragma(filename, linenumber, line, errors=None):
    ...
    if line.startswith(linter_pragma_start):
        try:
            ...
            print(
                "Exception occurred while processing '{0}:{1}':".format(
                    filename, linenumber
                )
            )
...
def ParseArgs(argv):
    ...
    try:
        if _lint_state.config:
            try:
                ParseOptionFile(
                    OpenTextFile(_lint_state.config).readlines(), ignore_space
                )
            except IOError:
                pass
        _lint_state.SetFilters(filters)
