...
from coala_utils.string_processing import InBetweenMatch
from coala_utils.string_processing.Filters import limit, trim_empty_matches
...
def _split(string, max_split, remove_empty_matches, matching_function, *args, **kwargs):
    """
...
def split(pattern, string, max_split=0, remove_empty_matches=False, use_regex=False):
    """
...
def unescaped_split(
    pattern, string, max_split=0, remove_empty_matches=False, use_regex=False
):
    """
    Splits the given string by the specified pattern. The return character (\\n)
    is not a natural split pattern (if you don't specify it yourself).
...
def search_in_between(
    begin, end, string, max_matches=0, remove_empty_matches=False, use_regex=False
):
    """
...
def unescaped_search_in_between(
    begin, end, string, max_matches=0, remove_empty_matches=False, use_regex=False
):
    """
    Searches for a string enclosed between a specified begin- and end-sequence.
...
def convert_to_raw(string, exceptions=""):
    ...
    while i < length:
        if string[i] == "\\" and i + 1 < length and string[i + 1] not in exceptions:
            output += "\\"
...
def unescaped_rstrip(string):
    ...
    if len(string) > len(stripped) and position_is_escaped(stripped, len(string)):
