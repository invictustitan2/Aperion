...
from git.exc import (AmbiguousObjectName, BadName, BadObject, BadObjectType,
                     CacheError, CheckoutError, CommandError, GitCommandError,
                     GitCommandNotFound, GitError, HookExecutionError,
                     InvalidDBRoot, InvalidGitRepositoryError, NoSuchPathError,
                     ODBError, ParseError, RepositoryDirtyError,
                     UnmergedEntriesError, UnsafeOptionError,
                     UnsafeProtocolError, UnsupportedOperation,
                     WorkTreeRepositoryUnsupported)
...
try:
    ...
    from git.diff import (INDEX, NULL_TREE, Diff, Diffable,  # @NoMove
                          DiffConstants, DiffIndex)
    from git.index import (  # @NoMove; NOTE: This tells type checkers what util resolves to. We delete it, and it is; really resolved by __getattr__, which warns. See below on what to use instead.
        BaseIndexEntry, BlobFilter, CheckoutError, IndexEntry, IndexFile,
        StageType, util)
    from git.objects import (Blob, Commit, IndexObject, Object,  # @NoMove
                             RootModule, RootUpdateProgress, Submodule,
                             TagObject, Tree, TreeModifier, UpdateProgress)
    from git.refs import (HEAD, Head, Reference, RefLog,  # @NoMove
                          RefLogEntry, RemoteReference, SymbolicReference, Tag,
                          TagReference)
    from git.remote import (FetchInfo, PushInfo, Remote,  # @NoMove
                            RemoteProgress)
    from git.repo import Repo  # @NoMove
    from git.util import (Actor, BlockingLockFile, LockFile, Stats,  # @NoMove
                          remove_password_if_present, rmtree)
