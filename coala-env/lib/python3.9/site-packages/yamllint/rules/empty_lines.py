...
def check(conf, line):
    if line.start == line.end and line.end < len(line.buffer):
        ...
        if line.end < len(line.buffer) - 1 and line.buffer[line.end + 1] == "\n":
            return

        blank_lines = 0

        while (
            line.start > blank_lines
            and line.buffer[line.start - blank_lines - 1] == "\n"
        ):
            blank_lines += 1
        ...
        if blank_lines > max:
            yield LintProblem(
                line.line_no, 1, "too many blank lines (%d > %d)" % (blank_lines, max)
            )
