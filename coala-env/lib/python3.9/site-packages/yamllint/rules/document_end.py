...
def check(conf, token, prev, next, nextnext, context):
    if conf["present"]:
        if isinstance(token, yaml.StreamEndToken) and not (
            isinstance(prev, yaml.DocumentEndToken)
            or isinstance(prev, yaml.StreamStartToken)
        ):
            yield LintProblem(token.start_mark.line, 1, 'missing document end "..."')
        elif isinstance(token, yaml.DocumentStartToken) and not (
            isinstance(prev, yaml.DocumentEndToken)
            or isinstance(prev, yaml.StreamStartToken)
        ):
            yield LintProblem(
