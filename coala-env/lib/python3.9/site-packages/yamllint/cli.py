    ...
    for item in items:
        if os.path.isdir(item):
            for root, dirnames, filenames in os.walk(item):
                for filename in [f for f in filenames if f.endswith((".yml", ".yaml"))]:
...
def run(argv=None):
    ...
    parser.add_argument(
        "files", metavar="FILE_OR_DIR", nargs="+", help="files to check"
    )
    parser.add_argument(
        "-c",
        "--config-file",
    ...
    parser.add_argument(
        "-v", "--version", action="version", version="%s %s" % (APP_NAME, APP_VERSION)
    )
    ...
    if args.config_file is not None and args.config_data is not None:
        print(
            "Options --config-file and --config-data cannot be used " "simultaneously.",
            file=sys.stderr,
        )
        sys.exit(-1)
