...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Lint reStructuredText files. Returns 0 if all files pass linting, "
        "1 for an internal error, and 2 if linting failed."
    )
    parser.add_argument("--version", action="version", version=VERSION)
    parser.add_argument(
        "filepaths", metavar="filepath", nargs="+", type=str, help="File to lint"
    )
    parser.add_argument(
        "--format",
        ...
        type=str,
        choices=("text", "json"),
        help='Format of the output (default: "{default}")'.format(
            default=DEFAULT_FORMAT
        ),
    )
    parser.add_argument(
        "--encoding", type=str, help='Encoding of the input file (e.g. "utf-8")'
    )
    parser.add_argument(
        "--level",
        ...
        help='Minimum error level to report (default: "{default}")'.format(
            default=DEFAULT_LEVEL_KEY
        ),
    )
