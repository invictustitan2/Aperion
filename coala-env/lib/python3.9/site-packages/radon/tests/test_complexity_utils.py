...
class TestRank(ParametrizedTestCase):
    ...
    def test_rank(self):
        if hasattr(self.expected_rank, "__call__") and isinstance(
            self.expected_rank(), Exception
        ):
...
class TestAverageComplexity(ParametrizedTestCase):
    ...
    def test_average_complexity(self):
        self.assertEqual(average_complexity(self.blocks), self.expected_average)
...
class TestFlake8Checker(unittest.TestCase):
    ...
    def test_run(self):
        ...
        c.max_cc = 3
        self.assertEqual(
            list(c.run()), [(7, 0, "R701: 'f' is too complex (4)", type(c))]
        )
