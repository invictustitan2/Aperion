...
Module = collections.namedtuple(
    "Module", ["loc", "lloc", "sloc", "comments", "multi", "blank", "single_comments"]
)
...
def is_multiline_string(doc, line_count, quote_type):
    ...
    if (
        line.count("=") > 0
        and line.index("=") < line.index(quote_type)
        or line_count != 0
        and len(previous_line) > 2
...
def remove_python_documentation(doc):
    ...
    for line_count, line in enumerate(doc):
        ...
        quote_type = [
            multi_quo for multi_quo in multi_quos if multi_quo in doc[line_count]
        ]
