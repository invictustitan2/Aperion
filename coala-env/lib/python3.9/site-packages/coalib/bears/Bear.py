...
from coala_utils.decorators import classproperty, enforce_signature, get_public_members
...
class Bear(Printer, LogPrinterMixin):
    ...
    def maintainers_emails(cls):
        ...
        """
        return (
            cls.AUTHORS_EMAILS if cls.MAINTAINERS_EMAILS == set() else cls.MAINTAINERS
        )
    ...
    def __init__(self, section: Section, message_queue, timeout=0):
        ...
        if cp is not True:
            error_string = (
                "The bear " + self.name + " does not fulfill all requirements."
            )
            if cp is not False:
                error_string += " " + cp
    ...
    def run_bear_from_section(self, args, kwargs):
        try:
            ...
            self.warn("The bear {} cannot be executed.".format(self.name), str(err))
            return
    ...
    def execute(self, *args, **kwargs):
        ...
        try:
            ...
            self.warn(
                "Bear {} failed to run. Take a look at debug messages"
    ...
    def __json__(cls):
        ...
        _dict["metadata"] = {
            "desc": metadata.desc,
            "non_optional_params": (
                {param: non_optional_params[param][0]} for param in non_optional_params
            ),
            "optional_params": (
                {param: optional_params[param][0]} for param in optional_params
            ),
        }
        return _dict
    ...
    def download_cached_file(self, url, filename):
        ...
        self.info(
            "Downloading {filename!r} for bear {bearname} from {url}.".format(
                filename=filename, bearname=self.name, url=url
            )
        )
