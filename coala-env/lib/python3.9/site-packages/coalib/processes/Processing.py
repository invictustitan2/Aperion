...
def create_process_group(command_array, **kwargs):
    if platform.system() == "Windows":  # pragma: no cover
        proc = subprocess.Popen(
            command_array, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, **kwargs
        )
    else:
...
def get_default_actions(section):
    ...
    actions = {
        bearname: action_dict[action_name]
...
def autoapply_actions(results, file_dict, file_diff_dict, section, log_printer):
    ...
    for bearname, actionname in invalid_actions.items():
        log_printer.warn(
            "Selected default action {!r} for bear {!r} does "
            "not exist. Ignoring action.".format(actionname, bearname)
        )
    ...
    for result in results:
        ...
        if not action.is_applicable(result, file_dict, file_diff_dict):
            log_printer.warn(
                "Selected default action {!r} for bear {!r} is "
        ...
        try:
            ...
            log_printer.info(
                "Applied {!r} on {} from {!r}.".format(
                    action.get_metadata().name, result.location_repr(), result.origin
                )
            )
...
def print_result(
    ...
    results = list(
        filter(
            lambda result: type(result) is Result
            and result.severity >= min_severity
            and not check_result_ignore(result, ignore_ranges),
            results,
        )
    )

    patched_results = autoapply_actions(
        results, file_dict, file_diff_dict, section, log_printer
    )
...
def get_file_dict(filename_list, log_printer):
    ...
    for filename in filename_list:
        try:
            ...
            log_printer.log_exception(
                "Failed to read file '{}' because of "
...
def instantiate_bears(
    ...
    local_bear_list = [
        ...
        for bear in filter_raising_callables(
            local_bear_list, RuntimeError, section, message_queue, timeout=0.1
        )
    ]
...
def instantiate_processes(
    ...
    if cache:
        ...
        changed_files = (
            cache.get_uncached_files(set(filename_list)) if cache else filename_list
        )
...
def yield_ignore_ranges(file_dict):
    ...
    for filename, file in file_dict.items():
        ...
        for line_number, line in enumerate(file, start=1):
            ...
            if "gnor" in line:
                ...
                if "start ignoring " in line:
                    ...
                    yield (
                        get_ignore_scope(line, "ignore "),
                        SourceRange.from_values(
                            filename, line_number, 1, end_line, len(file[end_line - 1])
                        ),
                    )
        ...
        if stop_ignoring is False and start is not None:
            yield (
                bears,
                SourceRange.from_values(filename, start, 1, len(file), len(file[-1])),
            )
...
def execute_section(
    ...
    try:
        ...
        log_printer.warn(
            "Unable to convert setting 'jobs' into a number. "
