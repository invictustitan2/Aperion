...
STR_GET_VAL_FOR_SETTING = (
    'Please enter a value for the setting "{}" ({}) ' 'needed by {} for section "{}": '
)
STR_LINE_DOESNT_EXIST = (
    "The line belonging to the following result "
...
def format_lines(lines, line_nr=""):
    return "\n".join(
        "|{:>4}| {}".format(line_nr, line) for line in lines.rstrip("\n").split("\n")
    )
...
def print_section_beginning(console_printer, section):
    ...
    """
    console_printer.print("Executing section {name}...".format(name=section.name))
...
def nothing_done(log_printer):
    ...
    """
    log_printer.warn("No existent section was targeted or enabled. " "Nothing to do.")


def acquire_actions_and_apply(
    console_printer, section, file_diff_dict, result, file_dict, cli_actions=None
):
    """
    ...
    while True:
        ...
        if not ask_for_action_and_apply(
            console_printer,
            section,
            metadata_list,
            action_dict,
...
def print_lines(console_printer, file_dict, sourcerange):
    ...
    for i in range(sourcerange.start.line, sourcerange.end.line + 1):
        ...
        console_printer.print(
            format_lines(lines="", line_nr=i), color=FILE_LINES_COLOR, end=""
        )
        ...
        if i == sourcerange.start.line and sourcerange.start.column:
            console_printer.print(
                highlight_text(no_color, line[: sourcerange.start.column - 1], lexer),
                end="",
            )
        ...
        if i == sourcerange.end.line and sourcerange.end.column:
            ...
            console_printer.print(
                highlight_text(no_color, line[sourcerange.end.column - 1 :], lexer),
                end="",
            )
            console_printer.print("")
            ...
            console_printer.print(
                highlight_text(no_color, line[printed_chars:], lexer), end=""
            )
            console_printer.print("")


def print_result(
    console_printer, section, file_diff_dict, result, file_dict, interactive=True
):
    """
    ...
    if not isinstance(result, Result):
        logging.warning(
            "One of the results can not be printed since it is "
    ...
    lexer = TextLexer()
    result.message = highlight_text(
        no_color, result.message, lexer, BackgroundMessageStyle
    )
    ...
    if interactive:
        ...
        if show_patch_action.is_applicable(result, file_dict, file_diff_dict):
            ...
            if diff_size <= DIFF_EXCERPT_MAX_SIZE:
                show_patch_action.apply_from_section(
                    result, file_dict, file_diff_dict, section
                )
                cli_actions = tuple(
                    action
                    for action in cli_actions
        ...
        acquire_actions_and_apply(
            console_printer, section, file_diff_dict, result, file_dict, cli_actions
        )
...
def print_diffs_info(diffs, printer):
    for filename, diff in sorted(diffs.items()):
        ...
        printer.print(
            format_lines(
                "+{additions} -{deletions} in {file}".format(
...
def print_results_formatted(log_printer, section, result_list, *args):
    ...
    default_format = (
        "id:{id}:origin:{origin}:file:{file}:line:{line}:"
    ...
    if "format_str" in section:
        ...
        if _warn_deprecated_format_str:
            log_printer.warn(
                'The setting "format_str" has been deprecated.'
    ...
    for result in result_list:
        ...
        try:
            ...
            log_printer.log_exception(
                "Unable to print the result with the given format string.", exception
            )
    ...
    if len(result.affected_code) == 0:
        ...
        for sourcerange in result.affected_code:
            if sourcerange.file is not None and sourcerange.file not in file_dict:
                log_printer.warn(
                    "The context for the result ({}) cannot "
...
def print_results_no_input(
    log_printer, section, result_list, file_dict, file_diff_dict, console_printer
):
    """
    ...
    for result in result_list:
        ...
        print_affected_files(console_printer, log_printer, result, file_dict)
...
def print_results(
    ...
    for result in sorted(result_list):
        ...
        print_result(console_printer, section, file_diff_dict, result, file_dict)
...
def print_affected_lines(console_printer, file_dict, sourcerange):
    ...
    if sourcerange.start.line is not None:
        if len(file_dict[sourcerange.file]) < sourcerange.end.line:
            console_printer.print(
                format_lines(lines=STR_LINE_DOESNT_EXIST, line_nr=sourcerange.end.line)
            )
        else:
...
def require_setting(setting_name, arr, section):
    ...
    print(
        colored(
            STR_GET_VAL_FOR_SETTING.format(setting_name, arr[0], needed, section.name),
...
def acquire_settings(log_printer, settings_names_dict, section):
    ...
    result = {}
    for setting_name, arr in sorted(
        settings_names_dict.items(), key=lambda x: (join_names(x[1][1:]), x[0])
    ):
    ...
    for param_name in params:
        if param_name not in section or action.name in failed_actions:
            question = format_lines(
                "Please enter a value for the parameter '{}' ({}): ".format(
                    param_name, params[param_name][0]
                )
            )
...
def choose_action(console_printer, actions):
    ...
    """
    while True:
        console_printer.print(format_lines("*0: " + "Do nothing"))
        for i, action in enumerate(actions, 1):
            console_printer.print(format_lines("{:>2}: {}".format(i, action.desc)))
...
def ask_for_action_and_apply(
    ...
    """
    action_name, section = print_actions(
        console_printer, section, metadata_list, failed_actions
    )
    ...
    try:
        ...
        console_printer.print(
            format_lines(chosen_action.SUCCESS_MESSAGE), color=SUCCESS_COLOR
        )
        failed_actions.discard(action_name)
    except Exception as exception:  # pylint: disable=broad-except
        logging.error(
            "Failed to execute the action {} with error: {}.".format(
...
def show_bear(bear, show_description, show_params, console_printer):
    """
...
def print_bears(bears, show_description, show_params, console_printer):
    """
    ...
    if not bears:
        console_printer.print(
            "No bears to show. Did you forget to install "
            "the `coala-bears` package? Try `pip3 install "
            "coala-bears`."
        )
        return

    for bear, sections in sorted(
        bears.items(), key=lambda bear_tuple: bear_tuple[0].name
    ):
        show_bear(bear, show_description, show_params, console_printer)


def show_bears(
    local_bears, global_bears, show_description, show_params, console_printer
):
    """
        ...
        for language, capabilities in language_bears_capabilities.items():
            if capabilities[0]:
                ...
                if capabilities[1]:
                    ...
                    console_printer.print(
                        ", ".join(sorted(capabilities[1])), color=CAPABILITY_COLOR
                    )
            else:
