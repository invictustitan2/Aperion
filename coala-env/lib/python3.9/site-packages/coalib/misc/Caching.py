...
from coalib.misc.CachingUtilities import delete_files, pickle_dump, pickle_load
...
class FileCache:
    ...
    @enforce_signature
    def __init__(
        self, log_printer: LogPrinterMixin, project_dir: str, flush_cache: bool = False
    ):
        """
        ...
        if not flush_cache and last_time > self.current_time:
            log_printer.warn(
                "It seems like you went back in time - your "
    ...
    def get_uncached_files(self, files):
        ...
        if self.data == {}:
            ...
            return {
                ...
                for file in files
                if (
                    file not in self.data
                    or int(os.path.getmtime(file)) > self.data[file]
                )
            }
