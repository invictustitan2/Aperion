...
class LanguageMeta(type, metaclass=LanguageUberMeta):
    ...
    def __call__(cls, *args):
        if cls is Language:
            ...
            forbidden_attributes = list(
                chain(map(itemgetter(0), getmembers(Language)), ("versions", "aliases"))
            )
            ...
            class Sub(cls, metaclass=SubLanguageMeta):
                ...
                _attributes = {
                    name: member
                    for name, member in getmembers(arg)
                    if not name.startswith("_") and not name in forbidden_attributes
                }
    ...
    def __contains__(cls, item):
        ...
        ) and (not versions or all(version in cls.versions for version in versions))
    ...
    **You now know the most important parts for writing a bear using languages.
    ...
    def __getattr__(self, item):
        if len(self.versions) > 1:
            raise AttributeError(
                "You have to specify ONE version of your "
    ...
    def __contains__(self, item):
        item = Language[item]
        return type(self) is type(item) and set(item.versions).issubset(
            set(self.versions)
        )
...
def limit_versions(language, limit, operator):
    ...
    if isinstance(limit, int):
        versions = [
            version for version in language.versions if operator(int(version), limit)
        ]
    else:

        versions = [
            version for version in language.versions if operator(version, limit)
        ]
    if not versions:
