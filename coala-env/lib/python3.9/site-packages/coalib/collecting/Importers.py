...
def object_defined_in(obj, file_path):
    ...
    try:
        ...
        if (
            ...
            and source.lower() == file_path.lower()
            or source == file_path
        ):
            return True
...
def _iimport_objects(file_paths, names, types, supers, attributes, local):
    ...
    for file_path in file_paths:
        ...
        for obj_name, obj in inspect.getmembers(module):
            if (
                (not names or obj_name in names)
                and (not types or isinstance(obj, tuple(types)))
                and (not supers or _is_subclass(obj, supers))
                and (not attributes or _has_all(obj, attributes))
                and (local[0] is False or _is_defined_in(obj, file_path))
            ):
                yield obj
...
def import_objects(
    ...
    """
    return list(
        iimport_objects(file_paths, names, types, supers, attributes, local, verbose)
    )
