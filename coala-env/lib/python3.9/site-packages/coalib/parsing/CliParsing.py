...
def parse_cli(
    ...
    for arg_key, arg_value in sorted(vars(arg_parser.parse_args(arg_list)).items()):
        if arg_key == "settings" and arg_value is not None:
            ...
            append_to_sections(sections, arg_key, arg_value, origin, from_cli=True)
...
def check_conflicts(sections):
    ...
    for section in sections.values():
        if section.get("no_config", False) and (
            section.get("save", False) or section.get("find_config", False)
        ):
            ArgumentParser().error(
                "'no_config' cannot be set together 'save' or 'find_config'."
            )

        if not section.get("json", False) and (
            str(section.get("output", "")) or section.get("relpath", False)
        ):
            ArgumentParser().error(
