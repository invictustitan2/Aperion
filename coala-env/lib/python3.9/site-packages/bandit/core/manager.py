...
class BanditManager(object):
    ...
    def output_results(
        self, lines, sev_level, conf_level, output_file, output_format, template=None
    ):
        ...
        for fname in targets:
            ...
            if os.path.isdir(fname):
                if recursive:
                    ...
                    LOG.warning(
                        "Skipping directory (%s), use -r flag to " "scan contents",
                        fname,
                    )
...
def _find_candidate_matches(unmatched_issues, results_list):
    ...
    for unmatched in unmatched_issues:
        issue_candidates[unmatched] = [i for i in results_list if unmatched == i]
