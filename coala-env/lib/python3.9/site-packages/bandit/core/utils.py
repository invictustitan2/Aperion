...
def warnings_formatter(message, category=UserWarning, filename="", lineno=-1, line=""):
...
def get_module_qualname_from_path(path):
    ...
    if head == "" or tail == "":
        raise InvalidModulePath(
            'Invalid python file path: "%s"' " Missing path or file name" % (path)
        )
...
def parse_ini_file(f_loc):
    ...
    try:
        ...
        LOG.warning(
            "Unable to parse config file %s or missing [bandit] " "section", f_loc
        )
