...
def _output_issue_str(issue, indent, show_lineno=True, show_code=True, lines=-1):
    ...
    bits = []
    bits.append(
        "%s>> Issue: [%s:%s] %s" % (indent, issue.test_id, issue.test, issue.text)
    )
    ...
    bits.append("%s   More Info: %s" % (indent, docs_utils.get_url(issue.test_id)))

    if show_code:
        bits.extend([indent + l for l in issue.get_code(lines, True).split("\n")])
...
def report(manager, fileobj, sev_level, conf_level, lines=-1):
    ...
    if not manager.quiet or manager.results_count(sev_level, conf_level):
        ...
        bits.append("\nCode scanned:")
        bits.append(
            "\tTotal lines of code: %i" % (manager.metrics.data["_totals"]["loc"])
        )
