...
class Value(object):
    ...
    def __init__(self, *args):
        if len(self._fields) != len(args):
            raise ValueError(
                "got {0} arguments for {1} fields for {2}: {3}".format(
...
class Definition(Value):
    ...
    def __str__(self):
        out = "in {0} {1} `{2}`".format(self._publicity, self._human, self.name)
    ...
    def parse_all(self):
        ...
        while self.current.kind != tk.OP or self.current.value not in ")]":
            if self.current.kind in (tk.NL, tk.COMMENT):
                ...
                raise AllError(
                    "Unexpected token kind in  __all__: {0!r}. ".format(
                        self.current.kind
                    )
                )
            self.stream.move()
