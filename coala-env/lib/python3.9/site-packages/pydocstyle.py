...
from collections import Set, defaultdict, namedtuple
...
def is_magic(name):
    return (
        ...
        and name not in VARIADIC_MAGIC_METHODS
    )
...
class Value(object):
    ...
    def __repr__(self):
        kwargs = ", ".join(
            "{0}={1!r}".format(field, getattr(self, field)) for field in self._fields
        )
...
class Parser(object):
    ...
    def parse_all(self):
        ...
        if self.current.value == "[":
            msg = (
                "%s WARNING: __all__ is defined as a list, this means "
        ...
        while self.current.kind != tk.OP or self.current.value not in ")]":
            if self.current.kind in (tk.NL, tk.COMMENT):
                ...
                raise AllError(
                    "Unexpected token kind in  __all__: %r. " % self.current.kind
                )
            self.stream.move()
        self.consume(tk.OP)
        all_content += ")"
    ...
    def parse_definition(self, class_):
        ...
        definition = class_(
            name, self.source, start, end, decorators, docstring, children, None
        )
    ...
    def _parse_from_import_source(self):
        ...
        self.stream.move()
        while (
            self.current.kind in (tk.DOT, tk.NAME, tk.OP)
...
D200 = D2xx.create_error(
    "D200", "One-line docstring should fit on one line " "with quotes", "found %s"
)
D201 = D2xx.create_error(
    "D201", "No blank lines allowed before function " "docstring", "found %s"
)
D202 = D2xx.create_error(
    "D202", "No blank lines allowed after function " "docstring", "found %s"
)
D203 = D2xx.create_error(
    "D203", "1 blank line required before class " "docstring", "found %s"
)
D204 = D2xx.create_error(
    "D204", "1 blank line required after class " "docstring", "found %s"
)
D205 = D2xx.create_error(
    "D205", "1 blank line required between summary line " "and description", "found %s"
)
...
D211 = D2xx.create_error(
    "D211", "No blank lines allowed before class " "docstring", "found %s"
)
...
D300 = D3xx.create_error("D300", 'Use """triple double quotes"""', "found %s-quotes")
...
D401 = D4xx.create_error(
    "D401", "First line should be in imperative mood", "%r, not %r"
)
...
RunConfiguration = namedtuple(
    "RunConfiguration", ("explain", "source", "debug", "verbose", "count")
)
...
class ConfigurationParser(object):
    ...
    def _read_configuration_file(self, path):
        ...
        if parser.read(path) and self._get_section_name(parser):
            option_list = dict(
                [(o.dest, o.type or o.action) for o in self._parser.option_list]
            )
    ...
    def _merge_configuration(self, parent_config, child_options):
        ...
        match = (
            child_options.match
            if child_options.match is not None
            else parent_config.match
        )
        match_dir = (
            child_options.match_dir
            if child_options.match_dir is not None
            else parent_config.match_dir
        )

        return CheckConfiguration(
            checked_codes=error_codes, match=match, match_dir=match_dir
        )
    ...
    def _create_check_config(cls, options, use_dafaults=True):
        ...
        """
        match = (
            cls.DEFAULT_MATCH_RE
            if options.match is None and use_dafaults
            else options.match
        )

        match_dir = (
            cls.DEFAULT_MATCH_DIR_RE
        ...
        return CheckConfiguration(
            checked_codes=checked_codes, match=match, match_dir=match_dir
        )
    ...
    def _validate_options(cls, options):
        ...
        for opt1, opt2 in itertools.permutations(cls.BASE_ERROR_SELECTION_OPTIONS, 2):
            if getattr(options, opt1) and getattr(options, opt2):
                log.error(
                    "Cannot pass both {0} and {1}. They are "
        ...
        if options.convention and options.convention not in conventions:
            log.error(
                "Illegal convention '{0}'. Possible conventions: {1}".format(
    ...
    def _create_option_parser(cls):
        ...
        parser = OptionParser(
            version=__version__, usage="Usage: pydocstyle [options] [<file|dir>...]"
        )
        ...
        option(
            ...
            "existing convention. Possible conventions: {0}".format(
                ", ".join(conventions)
            ),
        )
        option(
...
def check(filenames, select=None, ignore=None):
    ...
    if select is not None and ignore is not None:
        ...
        checked_codes = list(set(ErrorRegistry.get_error_codes()) - set(ignore))
    else:
...
class PEP257Checker(object):
    ...
    def checks(self):
        all = [
            check for check in vars(type(self)).values() if hasattr(check, "_check_for")
        ]
    ...
    def check_docstring_missing(self, definition, docstring):
        ...
        if (
            ...
            codes = {
                ...
                Class: D101,
                NestedClass: D101,
                Method: (lambda: D105() if is_magic(definition.name) else D102()),
                Function: D103,
                NestedFunction: D103,
                Package: D104,
            }
    ...
    def check_newline_after_last_paragraph(self, definition, docstring):
        ...
        if docstring:
            lines = [
                l for l in ast.literal_eval(docstring).split("\n") if not is_blank(l)
            ]
            if len(lines) > 1:
                if docstring.split("\n")[-1].strip() not in ['"""', "'''"]:
                    return D209()
