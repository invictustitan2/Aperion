...
def report(vulns, full=False):
    if vulns:
        ...
        for vuln in vulns:
            table.append(
                "│ {:24} │ {:13} │ {:17} │ {:13} │".format(
                    vuln.name[:24], vuln.version[:13], vuln.spec[:17], vuln.source
                )
            )
            if full:
        ...
        table = "\n".join(table)
        return "\n".join(
            [REPORT_BANNER, REPORT_HEADING, TABLE_HEADING, table, TABLE_FOOTER]
        )
    else:
        report = "│ {:76} │".format("No known security vulnerabilities found.")
        return "\n".join(
            [REPORT_BANNER, REPORT_HEADING, REPORT_SECTION, report, REPORT_FOOTER]
        )
