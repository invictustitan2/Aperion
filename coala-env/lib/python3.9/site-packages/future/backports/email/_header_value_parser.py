...
from future.backports.email import errors, utils
from future.backports.urllib.parse import unquote, unquote_to_bytes
from future.builtins import int, list, range, str, super
...
class TokenList(list):
    ...
    def __repr__(self):
        return "{}({})".format(
            self.__class__.__name__, super(TokenList, self).__repr__()
        )
    ...
    def _fold(self, folded):
        for part in self.parts:
            ...
            try:
                ...
                if any(
                    isinstance(x, errors.UndecodableBytesDefect)
            ...
            try:
                ...
                if last_ew is not None:
                    ...
                    chunk = get_unstructured(
                        "".join(folded.current[last_ew:] + [tstr])
                    ).as_encoded_word(charset)
...
class Terminal(str):
    ...
    def __repr__(self):
        return "{}({})".format(
            self.__class__.__name__, super(Terminal, self).__repr__()
        )
...
_non_atom_end_matcher = re.compile(
    r"[^{}]+".format("".join(ATOM_ENDS).replace("\\", "\\\\").replace("]", "\]"))
).match
_non_printable_finder = re.compile(r"[\x00-\x20\x7F]").findall
_non_token_end_matcher = re.compile(
    r"[^{}]+".format("".join(TOKEN_ENDS).replace("\\", "\\\\").replace("]", "\]"))
).match
_non_attribute_end_matcher = re.compile(
    r"[^{}]+".format("".join(ATTRIBUTE_ENDS).replace("\\", "\\\\").replace("]", "\]"))
).match
_non_extended_attribute_end_matcher = re.compile(
    r"[^{}]+".format(
        "".join(EXTENDED_ATTRIBUTE_ENDS).replace("\\", "\\\\").replace("]", "\]")
    )
).match
...
def get_addr_spec(value):
    ...
    if not value or value[0] != "@":
        addr_spec.defects.append(
            errors.InvalidHeaderDefect("add-spec local part with no domain")
        )
...
def get_section(value):
    ...
    if digits[0] == "0" and digits != "0":
        section.defects.append(
            errors.InvalidHeaderError("section number" "has an invalid leading 0")
        )
