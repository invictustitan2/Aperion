...
from future.builtins import int, list, map, open, range, str
    ...
    if path is None:
        ...
        try:
            ...
            if not quiet:
                raise
            warnings.warn(
                "tests may fail, unable to create temp CWD " + name,
                RuntimeWarning,
                stacklevel=3,
            )
    try:
        ...
        if not quiet:
            raise
        warnings.warn(
            "tests may fail, unable to change the CWD to " + path,
            RuntimeWarning,
            stacklevel=3,
        )
    try:
...
class _MemoryWatchdog(object):
    ...
    def start(self):
        try:
            ...
            warnings.warn(
                "/proc not available for stats: {0}".format(e), RuntimeWarning
            )
            sys.stderr.flush()
            return
