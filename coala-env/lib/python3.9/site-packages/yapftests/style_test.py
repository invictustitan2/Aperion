...
def _LooksLikeChromiumStyle(cfg):
    return cfg["INDENT_WIDTH"] == 2 and cfg["BLANK_LINE_BEFORE_NESTED_CLASS_OR_DEF"]


def _LooksLikeGoogleStyle(cfg):
    return cfg["INDENT_WIDTH"] == 4 and cfg["BLANK_LINE_BEFORE_NESTED_CLASS_OR_DEF"]


def _LooksLikePEP8Style(cfg):
    return cfg["INDENT_WIDTH"] == 4 and not cfg["BLANK_LINE_BEFORE_NESTED_CLASS_OR_DEF"]
...
class StyleFromFileTest(unittest.TestCase):
    ...
    def testErrorNoStyleFile(self):
        with self.assertRaisesRegexp(
            style.StyleConfigError, "is not a valid style or file path"
        ):
    ...
    def testErrorNoStyleSection(self):
        ...
        with _TempFileContents(self.test_tmpdir, cfg) as f:
            with self.assertRaisesRegexp(
                style.StyleConfigError, "Unable to find section"
            ):
