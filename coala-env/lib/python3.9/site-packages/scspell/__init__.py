...
from . import _portable, _util
from ._corpus import CorporaFile
from ._util import VERBOSITY_MAX, VERBOSITY_NORMAL, set_verbosity
...
class MatchDescriptor(object):
    ...
    def get_context(self):
        ...
        self._context = [
            ...
            if (i + 1 - self._line_num)
            in range(-CONTEXT_SIZE // 2, CONTEXT_SIZE // 2 + 1)
        ]
        return self._context
...
def handle_new_filetype(extension, dicts):
    ...
    while True:
        descr = raw_input(
            """\
            Enter a descriptive name for the programming language: """
        ).strip()
        if descr == "":
            print(
                """\
        ...
        while True:
         ...
         (Canceled.)\n"""
                ...
                if dicts.add_by_extension(subtoken, ext):
                    ...
                    if handle_new_extension(ext, dicts) and dicts.add_by_extension(
                        subtoken, ext
                    ):
                        break
            elif ch == "n":
                ...
                print(
                    "New file ID {0} for {1}".format(file_id, filename), file=sys.stderr
                )
...
def handle_failed_check_interactively(
    match_desc, filename, fq_filename, file_id_ref, unmatched_subtokens, dicts, ignores
):
   ...
   show (c)ontext? [i]"""
      ...
      (Canceled.)\n"""
                ...
                tail = re.sub(
                    MATCH_REGEX,
                    replacement,
                    match_desc.get_remainder(),
                    1 if ch == "r" else 0,
                )
                print()
                return (
                    match_desc.get_prefix() + tail,
                    match_desc.get_ofs() + len(replacement),
                )
        elif ch == "a":
            if handle_add(
                unmatched_subtokens, filename, fq_filename, file_id_ref, dicts
            ):
                break
        elif ch == "c":
...
def report_failed_check(match_desc, filename, unmatched_subtokens):
    ...
    if len(unmatched_subtokens) == 1:
        print(
            ...
            % (filename, match_desc.get_line_num(), unmatched_subtokens[0], token),
            file=sys.stderr,
        )
    else:
...
def spell_check_token(
    match_desc, filename, fq_filename, file_id_ref, dicts, ignores, report_only
):
    ...
    if (token.lower() not in ignores) and (HEX_REGEX.match(token) is None):
        ...
        unmatched_subtokens = [
            st
            for st in subtokens
            if len(st) > LEN_THRESHOLD
            and (not dicts.match(st, filename, file_id_ref[0]))
            and (st not in ignores)
        ]
        if unmatched_subtokens:
            unmatched_subtokens = make_unique(unmatched_subtokens)
            if report_only:
                function = getattr(report_only, "__call__", report_failed_check)
                return (function(match_desc, filename, unmatched_subtokens), True)
            else:
                return (
    ...
    return ((match_desc.get_string(), match_desc.get_ofs() + len(token)), False)
...
def spell_check_file(filename, dicts, ignores, report_only, c_escapes):
    ...
    while True:
        ...
        if m_id is not None and m.start() >= m_id.start() and m.start() < m_id.end():
            ...
            pos = m_id.end()
            continue
        result = spell_check_token(
            MatchDescriptor(data, m),
            filename,
            fq_filename,
            file_id_ref,
            dicts,
...
def locate_dictionary():
    ...
    try:
        ...
        if os.path.isabs(loc):
            ...
            print(
                'Error while parsing "%s": dictionary must be an absolute '
                "path." % SCSPELL_CONF
            )
            sys.exit(1)
...
def export_dictionary(filename, base_dicts):
    ...
    """
    if base_dicts:
        raise SystemExit("--export-dictionary doesn't support " + "--base-dict")
        return
...
def find_dict_file(override_dictionary):
    verify_user_data_dir()
    dict_file = (
        locate_dictionary() if override_dictionary is None else override_dictionary
    )
...
def merge_file_ids(
    merge_from, merge_to, override_dictionary=None, base_dicts=[], relative_to=None
):
...
def copy_file(
    copy_from, copy_to, override_dictionary=None, base_dicts=[], relative_to=None
):
...
def rename_file(
    rename_from, rename_to, override_dictionary=None, base_dicts=[], relative_to=None
):
    ...
    with CorporaFile(dict_file, base_dicts, relative_to) as dicts:
        if dictionary_type[0] == "n":
            ...
            if not file_id:
                file_id = get_new_file_id()
                print(
                    "New file ID {0} for {1}".format(file_id, files[0]), file=sys.stderr
                )
                dicts.new_file_and_file_id(fq_filename, file_id)
            ...
            if not dicts.add_by_extension(word, ext):
                print(
                    "Dictionary for file extension '{}' not found.".format(ext),
                    file=sys.stderr,
                )

        else:
            print(
                "Dictionary type '{}' not recognized.".format(dictionary_type),
                file=sys.stderr,
            )


def delete_files(
    delete_files, override_dictionary=None, base_dicts=[], relative_to=None
):
...
def main():
    ...
    dict_group.add_argument(
        "--set-dictionary",
        dest="dictionary",
        help="permanently set location of dictionary to FILE",
        metavar="FILE",
        action="store",
    )
    dict_group.add_argument(
        ...
        help="export current dictionary to FILE",
        metavar="FILE",
        action="store",
    )
    dict_group.add_argument(
    ...
    dict_group.add_argument(
        ...
        dest="base_dicts",
        action="append_const",
        const=SCSPELL_BUILTIN_DICT,
        help="Use scspell's default wordlist as a base dictionary ({0})".format(
            SCSPELL_BUILTIN_DICT
        ),
    )
    dict_group.add_argument(
        "--filter-out-base-dicts",
        action="store_true",
        help="Remove from the dictionary file " "all the words from the base dicts",
    )
    dict_group.add_argument(
        "--relative-to",
        dest="relative_to",
    ...
    test_group.add_argument(
        "--test-input", action="store_true", default=False, help=argparse.SUPPRESS
    )
    ...
    if args.gen_id:
        ...
        print(
            "Exported dictionary to '{}'".format(args.export_filename), file=sys.stderr
        )
    elif args.merge_file_ids is not None:
        merge_file_ids(
            args.merge_file_ids[0],
            args.merge_file_ids[1],
        ...
        delete_files(
            args.files, args.override_filename, args.base_dicts, args.relative_to
        )
        ...
        if dictionary_type in ["p", "programming"] and len(args.files) < 1:
            ...
            parser.error(
                "--relative-to is required in order to use '{}' "
                "dictionary type".format(dictionary_type)
            )
        elif dictionary_type not in ["p", "programming", "f", "file", "n", "natural"]:
            parser.error("Dictionary type '{}' not found.".format(dictionary_type))
