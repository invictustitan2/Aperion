...
class PrefixMatchCorpus(Corpus):
    ...
    def add(self, token):
        ...
        insertion_point = bisect_left(self._tokens, token)
        if (
            insertion_point >= len(self._tokens)
            or self._tokens[insertion_point] != token
        ):
...
class CorporaFile(object):
    ...
    def __init__(self, filename, base_dicts, relative_to):
        ...
        for fn in base_dicts:
            self._base_corpora_files.append(CorporaFile(fn, [], relative_to))
        ...
        try:
            ...
            raise SystemExit(
                "Error while parsing dictionary file '{}': {}".format(filename, e)
            )
        ...
        try:
            with io.open(mapping_file, mode="r", encoding="utf-8") as mf:
                try:
                    ...
                    _util.mutter(
                        ...
                        "got file ID mapping:\n{0}".format(self._file_id_mapping),
                    )
                except ValueError as e:
                    # Error during file creation might leave an empty file
                    # here.  Not necessarily fatal, but report it.
                    _util.mutter(
                        _util.VERBOSITY_NORMAL,
                        "Couldn't load file ID mapping from {0}: {1}".format(
                            mapping_file, e
                        ),
                    )
        except IOError as e:
            if e.errno == errno.ENOENT:
                _util.mutter(
                    _util.VERBOSITY_DEBUG,
                    "No file ID mappings file {0}".format(mapping_file),
                )
            else:
                raise SystemExit(
    ...
    def match(
        ...
        if match_in & MATCH_FILETYPE:
            ...
            try:
                ...
                _util.mutter(
                    _util.VERBOSITY_DEBUG,
                    '(Matching against filetype "%s".)' % corpus.get_name(),
                )
                ...
                _util.mutter(
                    _util.VERBOSITY_DEBUG,
                    '(No filetype match for extension "%s".)' % ext,
                )
        ...
        if match_in & MATCH_FILEID and file_id is not None:
            try:
                ...
                _util.mutter(
                    _util.VERBOSITY_DEBUG, '(Matching against file-id "%s".)' % file_id
                )
                if corpus.match(token):
                    return True
            except KeyError:
                _util.mutter(
                    _util.VERBOSITY_DEBUG, '(No file-id match for "%s".)' % file_id
                )

        return False

    def token_is_in_base_dict(
        self,
        token,
        filename,
        file_id,
    ...
    def filter_out_base_dicts(self):
        ...
        for ext in self._extensions:
            ...
            for t in file_type_corp._tokens:
                if self.token_is_in_base_dict(
                    t, fake_filename, None, MATCH_NATURAL | MATCH_FILETYPE
                ):
    ...
    def add_by_extension(self, token, extension):
        ...
        try:
            ...
            _util.mutter(
                _util.VERBOSITY_DEBUG, '(Adding to filetype "%s".)' % corpus.get_name()
            )
            corpus.add(token)
            return True
        except KeyError:
            _util.mutter(
                _util.VERBOSITY_DEBUG,
                '(No filetype match for extension "%s".)' % extension,
            )
            return False
    ...
    def add_by_file_id(self, token, file_id):
        ...
        try:
            ...
            _util.mutter(
                _util.VERBOSITY_DEBUG,
                '(No file-id match for "%s"; creating new.)' % file_id,
            )
    ...
    def _make_relative_filename(self, fq_filename):
        ...
        if not fq_filename.startswith(self._relative_to):
            raise SystemExit(
                "File {0} not within --relative-to {1}".format(
                    fq_filename, self._relative_to
                )
            )
        ...
        if len(rfn) == 0:
            raise SystemExit(
                "Making {0} relative to {1}: There's nothing "
    ...
    def new_file_and_file_id(self, fq_filename, file_id):
        ...
        if self._relative_to is None:
            raise AssertionError("new_file_and_file_id called without " "--relative-to")
        rel_filename = self._make_relative_filename(fq_filename)
        if rel_filename in self._reverse_file_id_mapping:
            raise AssertionError(
                "{0} already has file_id {1}".format(
        ...
        if rel_filename not in self._file_id_mapping[file_id]:
            ...
            self._file_id_mapping[file_id] = sorted(self._file_id_mapping[file_id])
    ...
    def merge_file_ids(self, merge_from, merge_to):
        if self.file_id_exists(merge_to):
            ...
            if id_to is None:
                raise SystemExit(
                    "Can't find merge_to {0} as file ID or file".format(merge_to)
                )
        ...
        if self.file_id_exists(merge_from):
            ...
            if id_from is None:
                raise SystemExit(
                    "Can't find merge_from {0} as file ID or file" "".format(id_from)
                )
    ...
    def delete_file(self, filename):
        ...
        try:
            ...
            _util.mutter(
                _util.VERBOSITY_NORMAL, "No file ID for {0}".format(report_str)
            )
            return
        _util.mutter(
            _util.VERBOSITY_NORMAL, "Removing {0} <-> {1} mappings".format(filename, id)
        )
    ...
    def copy_file(self, copy_from, copy_to):
        ...
        if from_rel not in self._reverse_file_id_mapping:
            _util.mutter(_util.VERBOSITY_NORMAL, "No file ID for " + copy_from)
            return
        ...
        _util.mutter(
            _util.VERBOSITY_NORMAL,
            "Setting {0} to use {1}'s file ID {2}.".format(to_rel, from_rel, id_from),
        )
    ...
    def rename_file(self, rename_from, rename_to):
        ...
        if from_rel not in self._reverse_file_id_mapping:
            _util.mutter(_util.VERBOSITY_NORMAL, "No file ID for " + rename_from)
            return
        ...
        _util.mutter(
            _util.VERBOSITY_NORMAL,
            "Switching file ID {0} from {1} to {2}".format(id_from, from_rel, to_rel),
        )
    ...
    def is_dirty(self):
        dirty = (
            self._natural_dict.is_dirty() if self._natural_dict is not None else False
        )
        for corpus in self._filetype_dicts:
    ...
    def close(self):
        ...
        if self.is_dirty():
            try:
                ...
                print(
                    "Warning: unable to write dictionary file '{}' "
        ...
        if self._file_id_mapping_is_dirty:
            if self._relative_to is None:
                raise AssertionError(
                    "file ID mapping is dirty but " + "relative_to is None"
                )
            ...
            try:
                with io.open(mapping_file, mode="w", encoding="utf-8") as mf:
                    ...
                    json_str = json.dumps(
                        od, ensure_ascii=False, indent=2, separators=(",", ": ")
                    )
                ...
                print(
                    "Warning: unable to write file ID mapping file '{0}' "
    ...
    def _parse_corpus(self, lines, offset):
        ...
        if dict_type == DICT_TYPE_NATURAL:
            ...
            _util.mutter(
                _util.VERBOSITY_DEBUG,
                "(Loaded natural language dictionary with %u tokens.)" % len(tokens),
            )
            return offset
    ...
    def _parse_header_line(self, line, line_num):
        ...
        if dict_type == DICT_TYPE_FILETYPE:
            ...
            if descr == "":
                raise ParsingError(
                    "File type-description on line %u is empty." % line_num
                )
            for corpus in self._filetype_dicts:
            ...
            for ext in extensions:
                ...
                if ext in self._extensions:
                    raise ParsingError(
                        'Duplicate extension "%s" on line %u.' % (ext, line_num)
                    )
        ...
        if dict_type == DICT_TYPE_FILEID:
            ...
            if metadata in self._file_ids:
                raise ParsingError(
                    'Duplicate file ID string "%s" on line %u.' % (metadata, line_num)
                )
            return (dict_type, metadata)

        raise ParsingError(
            'Unrecognized dictionary type "%s" on line %u.' % (dict_type, line_num)
        )
