...
class VultureBear(GlobalBear):
    ...
    EXECUTABLE = "vulture"
    OUTPUT_REGEX = re.compile(r"(?P<filename>.*):(?P<line>.*):\s*(?P<message>.*)")

    @classmethod
    def check_prerequisites(cls):
        return (
            "Vulture is missing. Make sure to install it using "
    ...
    def run(self):
        ...
        stdout_output, _ = run_shell_command(
            (self.EXECUTABLE,) + tuple(filename for filename in self.file_dict.keys()),
