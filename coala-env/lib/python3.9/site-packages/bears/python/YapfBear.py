...
"""
        ...
        try:
            with prepare_file(options.splitlines(keepends=True), None) as (
                file_,
                fname,
            ):
                corrected = FormatCode("".join(file), style_config=fname)[0].splitlines(
                    True
                )
        ...
        for diff in diffs:
            yield Result(
                self,
                "The code does not comply with the settings " "provided.",
