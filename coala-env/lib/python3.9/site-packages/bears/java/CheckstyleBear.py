...
def check_invalid_configuration(checkstyle_configs, use_spaces, indent_size):
    if checkstyle_configs is "google" and (not use_spaces or indent_size != 2):
        raise ValueError(
            "Google checkstyle config does not support "
...
class CheckstyleBear:
    ...
    def create_arguments(
        ...
        """
        check_invalid_configuration(checkstyle_configs, use_spaces, indent_size)
        ...
        if checkstyle_configs in ("google", "sun"):
            ...
            checkstyle_configs = self.download_cached_file(
                _online_styles[checkstyle_configs], checkstyle_configs + ".xml"
            )

        return ("-jar", self.checkstyle_jar_file, "-c", checkstyle_configs, filename)
