...
class QuotesBear(LocalBear):
    ...
    def correct_single_line_str(self, filename, file, sourcerange, preferred_quotation):
        """
        ...
        replacement = (
            before + preferred_quotation + str_contents + preferred_quotation + after
        )

        diff = Diff(file)
        diff.change_line(
            sourcerange.start.line, file[sourcerange.start.line - 1], replacement
        )
        yield Result(
            self,
    ...
    def run(self, filename, file, dependency_results, preferred_quotation: str = '"'):
        """
        ...
        :param preferred_quotation: Your preferred quotation character, e.g.
                                    ``"`` or ``'``.
        """
        if not isinstance(dependency_results[AnnotationBear.name][0], HiddenResult):
            return
        ...
        for string_range in ranges:
            if (
                file[string_range.start.line - 1][string_range.start.column - 1]
                == preferred_quotation
            ):
                continue
