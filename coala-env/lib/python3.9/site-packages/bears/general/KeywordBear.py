...
def _get_comments(dependency_results):
    ...
    if not annotation_bear_results or not isinstance(annotation_bear_results, list):
        return
...
def generate_diff(comments, file, filename, line, line_number, pos):
    ...
    affected_comment_sourcerange = [c for c in comments if todo_source_range in c]
    ...
    if line_replacement and not in_multi_line_comment:
        ...
        diff.change_line(
            line_number, line, line.replace(text_replacement, "").rstrip() + "\n"
        )
    else:
...
class KeywordBear(LocalBear):
    ...
    def run(
        ...
        keywords_regex = re.compile(
            "(" + "|".join(re.escape(key) for key in keywords) + ")", re.IGNORECASE
        )
        ...
        for line_number, line in enumerate(file, start=1):
            for keyword in keywords_regex.finditer(line):
                diffs = generate_diff(
                    comments, file, filename, line, line_number, keyword.start()
                )
                yield Result.from_values(
                    origin=self,
                    message="The line contains the keyword '{}'.".format(
                        keyword.group()
                    ),
                    file=filename,
                    line=line_number,
