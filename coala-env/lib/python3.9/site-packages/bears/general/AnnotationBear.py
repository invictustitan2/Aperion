...
class AnnotationBear(LocalBear):
    ...
    def run(self, filename, file, language: str, coalang_dir: str = None):
        ...
        multiline_string_delimiters = dict(lang_dict["multiline_string_delimiters"])
        multiline_comment_delimiters = dict(lang_dict["multiline_comment_delimiters"])
        ...
        try:
            ...
            yield Result(
                self, str(e), severity=RESULT_SEVERITY.MAJOR, affected_code=(e.code,)
            )
    ...
    @staticmethod
    def get_range_end_position(
        file, filename, text, annotations, position, func, single_comment=False
    ):
        ...
        for annotation in annotations.keys():
            if text[position:].startswith(annotation):
                if not single_comment:
                    ...
                    ret_val = func(file, filename, text, annotation, position)
                if ret_val:
    ...
    @staticmethod
    def get_multiline(file, filename, text, annotation_start, annotation_end, position):
        """
        ...
        """
        end_end = get_end_position(
            annotation_end, text, position + len(annotation_start) - 1
        )
        if end_end == -1:
    ...
    def get_singleline_strings(
        file, filename, text, string_start, string_end, position
    ):
        """
        ...
        end_position = get_end_position(
            string_end, text, position + len(string_start) - 1
        )
