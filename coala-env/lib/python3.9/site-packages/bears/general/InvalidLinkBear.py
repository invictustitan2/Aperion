...
class InvalidLinkBear(LocalBear):
    ...
    def check_prerequisites(cls):
        code = cls.get_status_code(cls.check_connection_url, cls.DEFAULT_TIMEOUT)
        return "You are not connected to the internet." if code is None else True
        ...
        for line_number, link, code in InvalidLinkBear.find_links_in_file(
            ...
            if code is None:
                yield Result.from_values(
                    origin=self,
                    message=("Broken link - unable to connect to " "{url}").format(
                        url=link
                    ),
                    file=filename,
                    line=line_number,
                ...
                if code in (404, 410) or 500 <= code < 600:
                    yield Result.from_values(
                        origin=self,
                        message=(
                            "Broken link - unable to connect to {url} "
                            "(HTTP Error: {code})"
                ...
                if follow_redirects and 300 <= code < 400:  # HTTP status 30x
                    ...
                    if (matcher.real_quick_ratio() > 0.7 and matcher.ratio()) > 0.7:
                        ...
                        replacement = (
                            current_line[:start] + redirect_url + current_line[end:]
                        )
