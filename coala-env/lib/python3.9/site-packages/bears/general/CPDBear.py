...
class CPDBear(GlobalBear):
    ...
    lowered_lang_dict = {key.lower(): value for key, value in language_dict.items()}
    ...
    def run(
        ...
        if language not in self.lowered_lang_dict:  # pragma: no cover
            self.err(
                "This bear does not support files with the extension "
                "'{}'.".format(language)
            )
            return
        ...
        arguments = (
            ...
            str(minimum_tokens),
            "--language",
            self.lowered_lang_dict[language],
            "--files",
            files,
            "--format",
            "xml",
        ...
        arguments += tuple(
            option for option, enable in options.items() if enable is True
        )
        ...
        if stdout_output:
            ...
            for duplication in root.findall("duplication"):
                ...
                for xml_file in duplication.findall("file"):
                    ...
                    end_line = min(
                        start_line + length - 1, len(self.file_dict[filename])
                    )
