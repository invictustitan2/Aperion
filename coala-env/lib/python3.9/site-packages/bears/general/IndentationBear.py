...
class IndentationBear(LocalBear):
    ...
    def run(
        ...
        :param coalang_dir:
            ...
            file for language.
        """
        lang_settings_dict = LanguageDefinition(language, coalang_dir=coalang_dir)
        ...
        encapsulators = (
            dict(lang_settings_dict["encapsulators"])
            if "encapsulators" in lang_settings_dict
            else {}
        )
        ...
        comments.update(dict(lang_settings_dict["multiline_comment_delimiters"]))

        try:
            indent_levels = self.get_indent_levels(
                file, filename, indent_types, annotation_dict, encaps_pos, comments
            )
        ...
        absolute_indent_levels = self.get_absolute_indent_of_range(
            file, filename, encaps_pos, annotation_dict
        )

        insert = " " * indent_size if use_spaces else "\t"

        no_indent_file = self._get_no_indent_file(file)
        new_file = self._get_basic_indent_file(no_indent_file, indent_levels, insert)
        new_file = self._get_absolute_indent_file(
            new_file, absolute_indent_levels, indent_levels, insert
        )
    ...
    def _get_no_indent_file(self, file):
        no_indent_file = [
            line.lstrip() if line.lstrip() else "\n"
    ...
    def _get_basic_indent_file(self, no_indent_file, indent_levels, insert):
        ...
        for line_nr, line in enumerate(no_indent_file):
            new_file.append(
                insert * indent_levels[line_nr] + line if line is not "\n" else "\n"
            )

        return new_file

    def _get_absolute_indent_file(
        self, indented_file, absolute_indent_levels, indent_levels, insert
    ):
        for _range, indent in absolute_indent_levels:
            prev_indent = get_indent_of_line(
                indented_file, _range.start.line - 1, length=False
            )
    ...
    def get_absolute_indent_of_range(self, file, filename, encaps_pos, annotation_dict):
        """
    ...
    def get_indent_levels(
        self, file, filename, indent_types, annotation_dict, encapsulators, comments
    ):
        """
        ...
        for line in range(0, len(file)):
            ...
            for _range in ranges:
                ...
                if _range.end.line == line + 1 and first_ch in indent_types.values():
                    indent -= 1
                    next_indent -= 1
    ...
    def get_specified_block_range(
        self, file, filename, open_specifier, close_specifier, annotation_dict
    ):
        """
        ...
        open_pos = list(self.get_valid_sequences(file, open_specifier, annotation_dict))
        close_pos = list(
            self.get_valid_sequences(file, close_specifier, annotation_dict)
        )
    ...
    def get_unspecified_block_range(
        self, file, filename, indent_specifier, annotation_dict, encapsulators, comments
    ):
        """
        ...
        for specifier in specifiers:
            ...
            unindent_line = get_first_unindent(
                indent, file, current_line, annotation_dict, encapsulators, comments
            )
            ...
            _range.append(
                SourceRange.from_values(
                    filename,
                    start_line=specifier.line,
                    start_column=None,
                    end_line=unindent_line,
                    end_column=None,
                )
            )
    ...
    def get_valid_sequences(
        file, sequence, annotation_dict, encapsulators=None, check_ending=False
    ):
        """
        ...
        for sequence_match in unescaped_search_for(sequence, file_string):
            ...
            for string in annotation_dict["strings"]:
                if gt_eq(sequence_position, string.start) and lt_eq(
            ...
            for comment in annotation_dict["comments"]:
                if gt_eq(sequence_position, comment.start) and lt_eq(
                    sequence_position, comment.end
                ):
                    valid = False

                if (
                    comment.start.line == sequence_position.line
                    and comment.end.line == sequence_position.line
                    and check_ending
                ):
                    sequence_line_text = (
                        sequence_line_text[: comment.start.column - 1]
                        + sequence_line_text[comment.end.column - 1 :]
                    )

            if encapsulators:
                for encapsulator in encapsulators:
                    if gt_eq(sequence_position, encapsulator.start) and lt_eq(
                        sequence_position, encapsulator.end
                    ):
                        valid = False
...
def get_indent_of_specifier(file, current_line, encapsulators):
    ...
    _range = 0
    while (
        _range < len(encapsulators) and encapsulators[_range].end.line <= current_line
    ):
...
def get_first_unindent(
    indent, file, start_line, annotation_dict, encapsulators, comments
):
    """
    ...
    while line_nr < len(file):
        ...
        for comment in annotation_dict["comments"]:
            if comment.start.line < line_nr + 1 and comment.end.line >= line_nr + 1:
                valid = False

            first_char = file[line_nr].lstrip()[0] if file[line_nr].strip() else ""
            if first_char in comments:
                valid = False

        for encapsulator in encapsulators:
            if (
                encapsulator.start.line < line_nr + 1
                and encapsulator.end.line >= line_nr + 1
            ):
                valid = False
...
class UnmatchedIndentError(Exception):
    ...
    def __init__(self, open_indent, close_indent):
        Exception.__init__(
            self, "Unmatched " + open_indent + ", " + close_indent + " pair"
        )
