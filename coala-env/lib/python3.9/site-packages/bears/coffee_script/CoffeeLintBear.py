...
@linter(executable="coffeelint", use_stdin=True)
class CoffeeLintBear:
    ...
    @deprecate_settings(
        ...
        allow_unnecessary_double_quotes=("no_unnecessary_double_quotes", negate),
    ...
    def generate_config(
        ...
        if spaces_before_and_after_colon:
            coffee_configs["colon_assignment_spacing"] = {
                "level": "error",
                "spacing": {"left": spaces_before_colon, "right": spaces_after_colon},
            }
        ...
        coffee_configs["no_backticks"] = {
            "level": "error" if prohibit_embedding_javascript_snippet else "ignore"
        }
        if force_braces:
            coffee_configs["no_implicit_braces"] = {"level": "error", "strict": True}
        if not allow_implicit_parentheses:
            coffee_configs["no_implicit_parens"] = {"strict": True, "level": "error"}
        coffee_configs["no_interpolation_in_single_quotes"] = {
            "level": "error" if not allow_interpolation_in_single_quotes else "ignore"
        }
        if not allow_stand_alone_at_sign:
            coffee_configs["no_stand_alone_at"] = {"level": "error"}
        if use_spaces:
            coffee_configs["no_tabs"] = {"level": "error"}
        coffee_configs["indentation"] = {"value": indent_size, "level": "error"}
        ...
        if enforce_parentheses_on_non_empty_constructors:
            coffee_configs["non_empty_constructor_needs_parens"] = {"level": "error"}
        if consistent_line_endings_style:
            coffee_configs["line_endings"] = {
                "level": "error",
                "value": consistent_line_endings_style,
            }
    ...
    def process_output(self, output, filename, file):
        ...
        for item in tuple(output.values())[0]:
            yield Result.from_values(
                ...
                additional_info=item.get("description", item.get("context", "")),
            )
