...
def generate_spacing_diff(
    file, filename, line, line_number, match_object, required_spacing
):
    """
    ...
    replacement = "{before}{open}{spacing}{content}{spacing}{close}{after}".format(
...
def generate_label_diff(
    file, filename, line, line_number, match_object, expected_label
):
    """
    ...
    replacement = "{before}{label}{after}".format(
        before=content_before, after=content_after, label=expected_label
    )
    diff.change_line(line_number, line, replacement)
    return {filename: diff}
...
def has_required_spacing(string, required_spacing):
    ...
    return leading_spaces == required_spacing and trailing_spaces == required_spacing
...
class Jinja2Bear(LocalBear):
    ...
    VARIABLE_REGEX = re.compile(r"(?P<open>{{)(?P<content>.*?)(?P<close>}})")
    ...
    def handle_control_spacing_issue(
        self, file, filename, line, line_number, control_spacing, match_object
    ):
        """
        ...
        return Result.from_values(
            ...
            "`{0}` spaces on each side.".format(control_spacing),
            file=filename,
            line=line_number,
    ...
    def check_for_variable_spacing_issues(
        self, file, filename, line, line_number, variable_spacing
    ):
        """
        ...
        for m in self.VARIABLE_REGEX.finditer(line):
            ...
            if not has_required_spacing(match, variable_spacing):
                diff = generate_spacing_diff(
                    file, filename, line, line_number, m, variable_spacing
                )
                yield Result.from_values(
                    origin=self,
                    message="Variable blocks should be spaced with "
                    "`{0}` spaces on each side.".format(variable_spacing),
                    file=filename,
                    line=line_number,
                    column=m.start(0) + 1,
                    end_line=line_number,
    ...
    def check_control_start_tags(
        self, file, filename, line, line_number, control_spacing
    ):
        """
        ...
        for m in self.CONTROL_START_REGEX.finditer(line):
            ...
            end_label = "{{#{spacing}{content}{spacing}#}}".format(
                content=m.group("content").strip(), spacing=" " * control_spacing
            )
    ...
    def run(self, filename, file, variable_spacing: int = 1, control_spacing: int = 1):
        """
