...
class GitCommitBear(GlobalBear):
    ...
    def get_shortlog_checks_metadata(cls):
        return FunctionMetadata.from_function(
            cls.check_shortlog, omit={"self", "shortlog"}
        )

    @classmethod
    def get_body_checks_metadata(cls):
        return FunctionMetadata.from_function(cls.check_body, omit={"self", "body"})
    ...
    def run(self, allow_empty_commit_message: bool = False, **kwargs):
        ...
        yield from self.check_shortlog(
            stdout[0], **self.get_shortlog_checks_metadata().filter_parameters(kwargs)
        )
    ...
    def check_shortlog(
        ...
        if diff > 0:
            yield Result(
                ...
                "the limit ({} > {}).".format(
                    len(shortlog), diff, len(shortlog), shortlog_length
                ),
            )
        ...
        if (shortlog[-1] != ".") == shortlog_trailing_period:
            yield Result(
                ...
                (
                    ...
                    if shortlog_trailing_period
                    else "Shortlog of HEAD commit contains a period at end."
                ),
            )
    ...
    def check_imperative(self, paragraph):
        ...
        try:
            ...
            ):  # Handle special case for VBG
                return (word, tag)
            else:
                return None
        except LookupError as error:  # pragma: no cover
            self.err(
                "NLTK data missing, install by running following "
    ...
    def check_body(
        ...
        if any(
            (
                len(line) > body_line_length
                and not any(regex.search(line) for regex in ignore_regexes)
            )
            for line in body[1:]
        ):
            yield Result(
