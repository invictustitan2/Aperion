...
def create_file_structure_for_packages(root_folder, file_to_copy, object_name):
    ...
    shutil.copyfile(
        file_to_copy, os.path.join(upload_package_folder, object_name + ".py")
    )
...
def perform_register(path, file_name):
    ...
    """
    subprocess.call([sys.executable, "setup.py", "sdist", "bdist_wheel"], cwd=path)
    subprocess.call(
        [
            "twine",
            "register",
            "-r",
            "pypi",
            os.path.join(path, "dist", file_name + ".tar.gz"),
        ]
    )
    subprocess.call(
        [
            ...
            "-r",
            "pypi",
            os.path.join(path, "dist", file_name + "-py3-none-any.whl"),
        ]
    )
...
def perform_upload(path):
    ...
    """
    subprocess.call(["twine", "upload", path + "/dist/*"])
...
def create_upload_parser():
    ...
    """
    parser = argparse.ArgumentParser(description="Generates PyPi packages from bears.")
    parser.add_argument(
        "-r", "--register", help="Register the packages on PyPi", action="store_true"
    )
    parser.add_argument(
        "-u", "--upload", help="Upload the packages on PyPi", action="store_true"
    )
    return parser
...
def main():
    ...
    for bear_file_name in sorted(set(glob("bears/**/*Bear.py"))):
        bear_object = next(
            iimport_objects(bear_file_name, attributes="kind", local=True), None
        )
        if bear_object:
            ...
            if bear_object.REQUIREMENTS:
                for requirement in bear_object.REQUIREMENTS:
                    if isinstance(requirement, PipRequirement):
                        with open(
                            os.path.join(
                                "bears", "upload", bear_name, "requirements.txt"
                            ),
                            "a",
                        ) as reqtxt:
                            reqtxt.write(
                ...
                if os.path.exists(
                    os.path.join("bears", "upload", bear_name, "requirements.txt")
                ):
                    with open(
                        os.path.join("bears", "upload", bear_name, "MANIFEST.in"), "w"
                    ) as manifest:
            ...
            substitution_dict = {
                ...
                "AUTHORS_EMAILS": str(bear_object.AUTHORS_EMAILS),
                "MAINTAINERS": str(bear_object.maintainers),
                "MAINTAINERS_EMAILS": str(bear_object.maintainers_emails),
            ...
            create_file_from_template(
                os.path.join("bears", "setup.py.in"),
                os.path.join("bears", "upload", bear_name, "setup.py"),
                substitution_dict,
            )
            ...
            if args.register:
                perform_register(
                    os.path.join("bears", "upload", bear_name), bear_dist_name
                )
            if args.upload:
