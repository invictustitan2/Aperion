...
class GNUIndentBear:
    ...
    def create_arguments(
        ...
        indent_options += (
            ...
            if delete_optional_blank_lines
            else ("-nsob",)
        )
        indent_options += (
            ("--blank-lines-after-declarations",)
            if blank_lines_after_declarations
            else ("-nbad",)
        )
        indent_options += (
            ("--blank-lines-after-commas",) if blank_lines_after_commas else ("-nbc",)
        )
        indent_options += (
            ("--blank-lines-after-procedures",)
            if blank_lines_after_procedures
            else ("-nbap",)
        )
        indent_options += (
            ("-di" + str(declaration_indent),) if declaration_indent != 0 else ()
        )
        indent_options += (
            ("--case-indentation" + str(case_indentation),)
            if case_indentation != 0
            else ()
        )
        indent_options += (
        ...
        indent_options += (
            ("--braces-on-func-def-line",) if braces_on_func_def_line else ("-blf",)
        )
        indent_options += (
            ((("-ce",) if cuddle_else else ("-nce",)) + ("-br",))
        ...
        indent_style_option += (
            ("--k-and-r-style",) if k_and_r_style and indent_style_option is () else ()
        )
        indent_style_option += (
            ("--linux-style",) if linux_style and indent_style_option is () else ()
        )
        # If a style is chosen the other configs aren't passed to `indent`
        return (
            indent_style_option if indent_style_option is not () else indent_options
