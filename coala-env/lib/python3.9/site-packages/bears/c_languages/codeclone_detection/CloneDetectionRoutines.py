...
def exclude_function(count_matrix):
    ...
    return (
        all(
            (cv.category is CountVector.Category.literal or sum(cv.unweighted) < 10)
            for cv in count_matrix.values()
        )
        or variable_sum < 11
        or var_count < 2
...
def get_count_matrices(
    count_vector_creator, filenames, progress_callback, base_path, extra_include_paths
):
    """
    ...
    for i, filename in enumerate(filenames):
        ...
        count_dict = count_vector_creator.get_vectors_for_file(filename, include_paths)
        for function in count_dict:
            if not exclude_function(count_dict[function]):
                result[(filename, function[0], function[1])] = count_dict[function]
...
def get_difference(
    matching_iterator, average_calculation, poly_postprocessing, exp_postprocessing
):
    """
    ...
    if average_calculation:
        difference = average(
            [relative_difference(diff, norm) for diff, norm in matching_iterator]
        )
    else:
        difference = relative_difference(
            sum(diff for diff, norm in matching_iterator), norm_sum
        )
...
def compare_functions(
    ...
    diff_table = [
        (cv1, [(cv2, cv1.difference(cv2), cv1.maxabs(cv2)) for cv2 in cm2.values()])
    ...
    cost_matrix = [
        [relative_difference(difference, maxabs) for cv2, difference, maxabs in lst]
    ...
    return get_difference(
        [(diff_table[x][1][y][1], diff_table[x][1][y][2]) for x, y in matching],
        average_calculation,
        poly_postprocessing,
        exp_postprocessing,
    )
