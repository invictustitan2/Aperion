...
class ClangCountVectorCreator:
    ...
    def _get_vector_for_function(self, cursor, child_num=0):
        ...
        if is_reference(cursor):
            self.count_identifier(
                get_identifier_name(cursor), CountVector.Category.reference
            )
        if is_literal(cursor):
            tokens = list(cursor.get_tokens())
            if tokens:
                self.count_identifier(tokens[0].spelling, CountVector.Category.literal)
