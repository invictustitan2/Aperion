...
def _get_position_in_for_tokens(tokens, position):
    ...
    for token in tokens:
        ...
        if next_state is not state:
            token_position = (token.extent.start.line, token.extent.start.column)
...
def _get_binop_operator(cursor):
    ...
    for token in cursor.get_tokens():
        if operator_min_begin < (
            token.extent.start.line,
            token.extent.start.column,
        ) and operator_max_end >= (token.extent.end.line, token.extent.end.column):
            return token
...
def is_condition(stack):
    ...
    """
    return (
        _is_nth_child_of_kind(stack, [0], CursorKind.WHILE_STMT) != 0
        or _is_nth_child_of_kind(stack, [0], CursorKind.IF_STMT) != 0
        or _is_nth_child_of_kind(stack, [0], CursorKind.SWITCH_STMT) != 0
        or _is_nth_child_of_kind(stack, [0], CursorKind.CASE_STMT) != 0
        or FOR_POSITION.COND in _get_positions_in_for_loop(stack)
    )
...
def in_condition(stack):
    ...
    return _is_nth_child_of_kind(stack, [1, 2], CursorKind.IF_STMT) == 1 or (
        _is_nth_child_of_kind(stack, [1], CursorKind.SWITCH_STMT) == 1
        and _is_nth_child_of_kind(stack, [0], CursorKind.CASE_STMT) == 0
    )
...
def is_assignee(stack):
    ...
    for elem, _ in stack:
        if (
            elem.kind == CursorKind.BINARY_OPERATOR
            or elem.kind == CursorKind.COMPOUND_ASSIGNMENT_OPERATOR
        ):
            for token in elem.get_tokens():
                token_pos = (token.extent.start.line, token.extent.start.column)
                # This needs to be an assignment and cursor has to be on LHS
                if token.spelling in ASSIGNMENT_OPERATORS and cursor_pos <= token_pos:
                    return True
...
def is_assigner(stack):
    ...
    """
    cursor_pos = (stack[-1][0].extent.start.line, stack[-1][0].extent.start.column)
    for elem, _ in stack:
        if (
            elem.kind == CursorKind.BINARY_OPERATOR
            or elem.kind == CursorKind.COMPOUND_ASSIGNMENT_OPERATOR
            ...
            for token in elem.get_tokens():
                ...
                if token.spelling in ASSIGNMENT_OPERATORS and (
                    token_pos <= cursor_pos or token.spelling != "="
                ):
                    return True
...
def _loop_level(stack):
    ...
    return (
        positions_in_for.count(FOR_POSITION.INC)
...
def is_called(stack):
    ...
    are counted too.)
    """
    return _stack_contains_kind(stack, CursorKind.CALL_EXPR) and is_function(stack)
...
def is_call_param(stack):
    ...
    """
    return _stack_contains_kind(stack, CursorKind.CALL_EXPR) and not is_function(stack)
