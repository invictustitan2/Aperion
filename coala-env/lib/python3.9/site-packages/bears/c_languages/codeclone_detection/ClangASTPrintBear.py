    ...
    def print_node(self, cursor, filename, before="", spec_before=""):
        ...
        if file is not None and file.name == filename:
            self.debug(
                ...
                str(cursor.kind),
                "Lines",
                str(cursor.extent.start.line) + "-" + str(cursor.extent.end.line),
                "("
                + " ".join(str(token.spelling) for token in cursor.get_tokens())
                + ")",
            )
        ...
        if len(children) > 0:
            for child in children[:-1]:
                self.print_node(child, filename, before + len(spec_before) * " " + "|")

            self.print_node(
                children[-1], filename, before + len(spec_before) * " ", "`"
            )
    ...
    def run(self):
        ...
        for filename, file in sorted(self.file_dict.items()):
            root = (
                Index.create()
                .parse(
                    filename, options=TranslationUnit.PARSE_DETAILED_PROCESSING_RECORD
                )
                .cursor
            )
