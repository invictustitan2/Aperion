...
class TailorBear:
    ...
    severity_map = {"warning": RESULT_SEVERITY.NORMAL, "error": RESULT_SEVERITY.MAJOR}
    ...
    def process_output(self, output, filename, file):
        ...
        for item in output["files"][0]["violations"]:
            column = (
                ...
                if "column" in item["location"].keys()
                else None
            )
    ...
    def create_arguments(
        ...
        args = (
            ...
            str(max_line_length),
            "--max-class-length",
            str(max_class_length),
            "--max-closure-length",
            str(max_closure_length),
            "--max-file-length",
            str(max_file_length),
            "--max-function-length",
            str(max_function_length),
            "--max-name-length",
            str(max_name_length),
            "--max-struct-length",
            str(max_struct_length),
            "--min-name-length",
            str(min_name_length),
        )
        if tailor_config:
