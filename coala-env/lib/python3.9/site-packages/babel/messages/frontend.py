            ...
            if self.locale:
                ...
                for locale in os.listdir(self.directory):
                    po_file = os.path.join(
                        self.directory, locale, "LC_MESSAGES", f"{domain}.po"
                    )
            ...
            if self.mapping_file.endswith(".toml"):
                ...
                with open(self.mapping_file) as fileobj:
                    method_map, options_map = parse_mapping_cfg(
                        fileobj, filename=self.mapping_file
                    )
            for path in self.input_paths:
...
def _parse_config_object(config: dict, *, filename="(unknown)"):
    ...
    for idx, entry in enumerate(mappings_read):
        ...
        method = extractors.get(
            method, method
        )  # Map the extractor name to the callable now
...
def _parse_mapping_toml(
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "tomli or tomllib is required to parse TOML files"
            ) from ie
