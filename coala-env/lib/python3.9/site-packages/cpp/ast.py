...
class Using(Node):
    ...
    def __str__(self):
        return self._string_helper(self.__class__.__name__, unicode(self.names))
...
class Function(_GenericDeclaration):
    ...
    def is_exportable(self):
        if self.return_type:
            if (
                "static" in self.return_type.modifiers
                or "constexpr" in self.return_type.modifiers
            ):
                return False
...
class ASTBuilder(object):
    ...
    def __init__(
        self, token_stream, filename, in_class=None, namespace_stack=None, quiet=False
    ):
    ...
    def _generate_one(self, token):
        if token.token_type == tokenize.NAME:
            if keywords.is_keyword(token.name) and not keywords.is_builtin_type(
                token.name
            ):
            ...
            temp_tokens, last_token = self._get_var_tokens_up_to(True, "(", ";", "{")
            ...
            if last_token.name == "(" or last_token.name == "{":
                ...
                if temp_tokens[-1].name == "__attribute__":
                    list(self._get_parameters())
                    new_temp, last_token = self._get_var_tokens_up_to(
                        True, "(", ";", "{"
                    )
                    del temp_tokens[-1]
                ...
                for i, elt in reversed(list(enumerate(temp_tokens))):
                    if elt.name == "=" and temp_tokens[i - 1].name != "operator":
                        ...
                        new_temp, last_token = self._get_var_tokens_up_to(False, ";")
                        temp_tokens.extend(new_temp)
                        break
    ...
    def _get_tokens_up_to(self, expected_token):
        return self._get_var_tokens_up_to(False, expected_token)[0]
    ...
    def _get_var_tokens_up_to(self, skip_bracket_content, *expected_tokens):
        ...
        while (
            count1 != 0
            or count2 != 0
            or last_token.token_type != tokenize.SYNTAX
    ...
    def get_method(self, modifiers, templated_types):
        ...
        return self._get_method(return_type_and_name, modifiers, templated_types, False)

    def _get_method(self, return_type_and_name, modifiers, templated_types, get_paren):
        specializations = []
        if get_paren:
            token = self._get_next_token()
        ...
        if (
            len(return_type_and_name) > 2
            and return_type_and_name[-1].name == ">"
            and return_type_and_name[-2].name == "operator"
            and (name.name == ">=" or name.name == ">")
        ):
            n = return_type_and_name.pop()
            name = tokenize.Token(
                tokenize.SYNTAX, n.name + name.name, n.start, name.end
            )

        if len(return_type_and_name) > 1 and (
            return_type_and_name[-1].name == "operator"
            or return_type_and_name[-1].name == "~"
        ):
            op = return_type_and_name.pop()
            name = tokenize.Token(
                tokenize.NAME, op.name + name.name, op.start, name.end
            )
        # Handle templatized ctors.
            ...
            name = tokenize.Token(
                tokenize.NAME, name_seq.name + "[]", name_seq.start, name.end
            )
        ...
        if name.name == "operator" and not parameters:
            ...
            assert_parse(token.name == "(", token)
            name = tokenize.Token(
                tokenize.NAME, "operator()", name.start, last_token.end
            )
            parameters = list(self._get_parameters())
        ...
        while (
            ...
            or token.token_type == tokenize.PREPROCESSOR
        ):
            if (
                token.name == "const"
                or token.name == "override"
                or token.name == "final"
            ):
        ...
        if token.name == "{":
            ...
            assert_parse(token.name == ";", (token, return_type_and_name, parameters))

        # Looks like we got a method, not a function.
        if len(return_type) > 1 and return_type[-1].name == "::":
            return_type, in_class = self._get_return_type_and_class_name(return_type)
            return Method(
                indices.start,
                indices.end,
                name.name,
    ...
    def handle_enum(self):
        ...
        if token.token_type == tokenize.NAME:
            if self._handling_typedef:
                self._add_back_token(token)
                return Enum(token.start, token.end, name, None, self.namespace_stack)
        ...
        if token.token_type == tokenize.SYNTAX and token.name == "{":
            ...
            new_type = Enum(token.start, token.end, name, fields, self.namespace_stack)
    ...
    def handle_virtual(self):
        ...
        return self._get_method(return_type_and_name, FUNCTION_VIRTUAL, None, False)
    ...
    def handle_typedef(self):
        token = self._get_next_token()
        if token.token_type == tokenize.NAME and keywords.is_builtin_other_modifiers(
            token.name
        ):
        ...
        return Typedef(name.start, name.end, name.name, new_type, self.namespace_stack)
    ...
    def handle_template(self):
        ...
        if token.token_type == tokenize.NAME:
            if token.name == "class":
                return self._get_class(Class, templated_types)
            elif token.name == "struct":
                return self._get_class(Struct, templated_types)
            elif token.name == "union":
                return self._get_class(Union, templated_types)
    ...
    def _get_bases(self):
        ...
        while True:
            token = self._get_next_token()
            if token.name in specifier or token.token_type == tokenize.PREPROCESSOR:
                continue
            self._add_back_token(token)
            ...
            if (
                ...
                and next_token.name not in specifier
            ):
                self._add_back_token(next_token)
    ...
    def _get_class(self, class_type, templated_types):
        ...
        if class_token.token_type != tokenize.NAME:
            assert_parse(class_token.token_type == tokenize.SYNTAX, class_token)
            token = class_token
        else:
        ...
        if token.token_type == tokenize.SYNTAX:
            ...
            if token.name in "*&":
                ...
                if next_token.name == ";":
                    ...
                    modifiers = ["class"]
                    return self._create_variable(
                        class_token, name_token.name, class_name, modifiers, token.name
                    )
                else:
        ...
        if token.token_type == tokenize.SYNTAX and token.name == "{":
            ...
            if not self._handling_typedef:
                ...
                if token.token_type != tokenize.NAME:
                    ...
                    return self._create_variable(
                        class_token, token.name, new_class, modifiers, token.name
                    )
        else:
            if not self._handling_typedef:
...
def builder_from_source(source, filename, quiet=False):
    ...
    """
    return ASTBuilder(tokenize.get_tokens(source), filename, quiet=quiet)
