...
from . import ast, headers, keywords, metrics, symbols, tokenize, utils
...
class WarningHunter(object):
    ...
    def _get_module(self, node):
        ...
        if source is None:
            ...
            ast_list = None
            try:
                builder = ast.builder_from_source(source, filename, quiet=self.quiet)
                ...
                if not self.quiet:
                    print(
                        "Exception while processing '{}': {}".format(filename, error),
                        file=sys.stderr,
                    )
        ...
        for node in self.ast_list:
            if isinstance(node, ast.Include):
                ...
                if filename == self.filename:
                    self._add_warning(
                        "'{}' #includes itself".format(node.filename), node
                    )
    ...
    def _verify_forward_declarations_used(
        self, forward_declarations, decl_uses, file_uses
    ):
        ...
        for cls in forward_declarations:
            if cls in file_uses:
                if not decl_uses[cls] & USES_DECLARATION:
                    ...
                    msg = "'{}' forward declared, " "but needs to be #included".format(
                        cls
                    )
                    self._add_warning(msg, node)
            else:
    ...
    def _determine_uses(self, included_files, forward_declarations):
        ...
        def _add_reference(name, namespace):
            ...
            if file_use_node[1].ast_list is None:
                ...
                if isinstance(file_use_node[0], ast.Enum) or isinstance(
                    file_use_node[0], ast.Typedef
                ):
        ...
        def _add_variable(node, namespace, reference=False):
            ...
            _add_template_use(node.name, node.templated_types, namespace, reference)
            ...
            for p in function.parameters:
                ...
                if node.name not in templated_types:
                    if function.body and p.name:
                        ...
                        p.default
                        and p.default[0].name != "0"
                        and p.default[0].name != "NULL"
                        and p.default[0].name != "nullptr"
                    ):
                        _add_use(node.name, namespace)
                        ...
                        _add_use(node.name, namespace)
                    _add_template_use(
                        node.name, node.templated_types, namespace, reference
                    )
        ...
        def _add_template_use(name, types, namespace, reference=False):
            for cls in types or ():
                ...
                _add_template_use(cls.name, cls.templated_types, namespace, reference)
    ...
    def _find_unused_warnings(
        self, included_files, forward_declarations, primary_header=None
    ):
        file_uses, decl_uses = self._determine_uses(
        ...
        self._verify_forward_declarations_used(
            forward_declarations, decl_uses, file_uses
        )
        for node in forward_declarations.values():
            try:
                file_use_node = self.symbol_table.lookup_symbol(
            ...
            name = file_use_node[1].filename
            if (
                file_use_node[1].ast_list is not None
                and name in file_uses
                and file_uses[name] & USES_DECLARATION
            ):
                msg = "'{}' forward declared, " "but already #included in '{}'".format(
                    node.name, name
                )
                self._add_warning(msg, node)
    ...
    def _find_incorrect_case(self, included_files):
        for filename, node_and_module in included_files.items():
            ...
            correct_filename = get_correct_include_filename(base_name, candidates)
            if correct_filename:
                self._add_warning(
    ...
    def _find_public_function_warnings(self, node, name, primary_header, all_headers):
        ...
        for _, header in all_headers.values():
            if name in header.public_symbols:
                ...
                if primary_header:
                    msg = "expected to find '{}' in '{}', " "but found in '{}'".format(
            ...
            if name != "main" and name != name.upper():
                self._add_warning("'{}' not found {}".format(name, where), node)
    ...
    def _check_public_functions(self, primary_header, all_headers):
        ...
        for node in self.ast_list:
            ...
            if name in public_symbols:
                ...
                self._find_public_function_warnings(
                    node, name, primary_header, all_headers
                )
    ...
    def _find_source_warnings(self):
        ...
        for node in forward_declarations.values():
            ...
            # to check for now.
            msg = "'{}' forward declaration not expected in source file".format(
                node.name
            )
            self._add_warning(msg, node)
...
def run(filename, source, entire_ast, include_paths, quiet):
    hunter = WarningHunter(
        filename, source, entire_ast, include_paths=include_paths, quiet=quiet
    )
