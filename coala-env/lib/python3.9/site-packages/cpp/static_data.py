...
def _find_warnings(filename, lines, ast_list, static_is_optional):
    def print_warning(node):
        for name in node.name.split(","):
            print(
                "{}:{}: static data '{}'".format(
                    filename, lines.get_line_number(node.start), name
                )
            )
    ...
    def find_static(function_node):
        ...
        for node in function_node.body:
            ...
            if static_found:
                tokens.append(node)
                if node.name == ";":
                    body = list(ast.ASTBuilder(iter(tokens), filename).generate())
    ...
    for node in ast_list:
        if isinstance(node, ast.VariableDeclaration):
            ...
            is_not_const = (
                "const" not in node.type.modifiers
                and "constexpr" not in node.type.modifiers
            )
...
def _get_static_declarations(ast_list):
    for node in ast_list:
        if (
            ...
            and "static" in node.type.modifiers
        ):
            for name in node.name.split(","):
                yield (name, node)
...
def _find_unused_static_warnings(filename, lines, ast_list):
    ...
    count = 0
    for name, _ in sorted(static_declarations.items(), key=lambda x: x[1].start):
...
def run(filename, source, entire_ast, include_paths, quiet):
    ...
    return _find_warnings(
        filename, lines, entire_ast, True
