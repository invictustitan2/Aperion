...
def isvarname(s):
    return viml_eqregh(
        s, "^[vgslabwt]:$\\|^\\([vgslabwt]:\\)\\?[A-Za-z_][0-9A-Za-z_#]*$"
    )
...
class VimLParser:
    ...
    def check_missing_endfunction(self, ends, pos):
        if self.context[0].type == NODE_FUNCTION:
            raise VimLParserException(
                Err(viml_printf("E126: Missing :endfunction:    %s", ends), pos)
            )

    def check_missing_endif(self, ends, pos):
        if (
            self.context[0].type == NODE_IF
            or self.context[0].type == NODE_ELSEIF
            or self.context[0].type == NODE_ELSE
        ):
            raise VimLParserException(
                Err(viml_printf("E171: Missing :endif:    %s", ends), pos)
            )

    def check_missing_endtry(self, ends, pos):
        if (
            self.context[0].type == NODE_TRY
            or self.context[0].type == NODE_CATCH
            or self.context[0].type == NODE_FINALLY
        ):
            raise VimLParserException(
                Err(viml_printf("E600: Missing :endtry:    %s", ends), pos)
            )

    def check_missing_endwhile(self, ends, pos):
        if self.context[0].type == NODE_WHILE:
            raise VimLParserException(
                Err(viml_printf("E170: Missing :endwhile:    %s", ends), pos)
            )

    def check_missing_endfor(self, ends, pos):
        if self.context[0].type == NODE_FOR:
            raise VimLParserException(
                Err(viml_printf("E170: Missing :endfor:    %s", ends), pos)
            )
        ...
        while TRUE:
            ...
            if viml_stridx("aboveleft", k) == 0 and viml_len(k) >= 3:
                ...
                if d != "":
                    viml_add(
                        modifiers,
                        AttributeDict({"name": "tab", "count": viml_str2nr(d, 10)}),
                    )
                else:
                ...
                if d != "":
                    viml_add(
                        modifiers,
                        AttributeDict({"name": "verbose", "count": viml_str2nr(d, 10)}),
                    )
                else:
    ...
    def parse_command(self):
        ...
        if self.ea.cmd is NIL:
            ...
            raise VimLParserException(
                Err(
                    viml_printf(
                        "E492: Not an editor command: %s", self.reader.peekline()
                    ),
                    self.ea.cmdpos,
                )
            )
        if (
            self.reader.peekn(1) == "!"
            and self.ea.cmd.name != "substitute"
            and self.ea.cmd.name != "smagic"
            and self.ea.cmd.name != "snomagic"
        ):
            self.reader.getn(1)
        ...
        if (
            not viml_eqregh(self.ea.cmd.flags, "\\<BANG\\>")
            and self.ea.forceit
            and not viml_eqregh(self.ea.cmd.flags, "\\<USERCMD\\>")
        ):
    ...
    def _parse_command(self, parser):
        if parser == "parse_cmd_append":
            ...
            raise VimLParserException(
                viml_printf("unknown parser: %s", viml_string(parser))
            )
    ...
    def find_command(self):
        ...
        if c == "k":
            self.reader.getn(1)
            name = "k"
        elif c == "s" and viml_eqregh(
            self.reader.peekn(5), "\\v^s%(c[^sr][^i][^p]|g|i[^mlg]|I|r[^e])"
        ):
            self.reader.getn(1)
            name = "substitute"
        elif viml_eqregh(c, "[@*!=><&~#]"):
        ...
        if (cmd is NIL or cmd.name == "Print") and viml_eqregh(name, "^[A-Z]"):
            ...
            del cmd
            cmd = AttributeDict(
                {"name": name, "flags": "USERCMD", "parser": "parse_cmd_usercmd"}
            )
    ...
    def parse_argopt(self):
        while self.reader.p(0) == "+" and self.reader.p(1) == "+":
            ...
            if viml_eqregh(s, "^++bin\\>"):
                ...
                raise VimLParserException(
                    Err("E474: Invalid Argument", self.reader.getpos())
                )
            else:
                break
            self.reader.skip_white()
    ...
    def parse_comment(self):
        ...
        if c != '"':
            raise VimLParserException(
                Err(viml_printf("unexpected character: %s", c), npos)
            )
    ...
    def parse_cmd_common(self):
        ...
        if viml_eqregh(self.ea.cmd.flags, "\\<TRLBAR\\>") and not self.ea.usefilter:
            ...
            self.ea.cmd.name == "!"
            or self.ea.cmd.name == "global"
            or self.ea.cmd.name == "vglobal"
            or self.ea.usefilter
        ):
            while TRUE:
                end = self.reader.getpos()
    ...
    def separate_nextcmd(self):
        if (
            self.ea.cmd.name == "vimgrep"
            or self.ea.cmd.name == "vimgrepadd"
            or self.ea.cmd.name == "lvimgrep"
            or self.ea.cmd.name == "lvimgrepadd"
        ):
        ...
        while TRUE:
            ...
            if c == "<EOF>" or c == "<EOL>":
                ...
                if c == "<EOF>" or c == "<EOL>":
                    break
                self.reader.get()
            elif self.reader.peekn(2) == "`=" and viml_eqregh(
                self.ea.cmd.flags, "\\<\\(XFILE\\|FILES\\|FILE1\\)\\>"
            ):
                self.reader.getn(2)
                self.parse_expr()
                c = self.reader.peekn(1)
                ...
                or c == "\n"
                or c == '"'
                and not viml_eqregh(self.ea.cmd.flags, "\\<NOTRLCOM\\>")
                and (
                    self.ea.cmd.name != "@"
                    and self.ea.cmd.name != "*"
                    or self.reader.getpos() != self.ea.argpos
                )
                and (
                    self.ea.cmd.name != "redir"
                    or self.reader.getpos().i != self.ea.argpos.i + 1
                    or pc != "@"
                )
            ):
                has_cpo_bar = FALSE
                ...
                if (
                    not has_cpo_bar
                    or not viml_eqregh(self.ea.cmd.flags, "\\<USECTRLV\\>")
                ) and pc == "\\":
                    self.reader.get()
                else:
                    break
        ...
        if tokenizer.peek().type == TOKEN_PCLOSE:
            ...
            while TRUE:
                ...
                if token.type == TOKEN_IDENTIFIER:
                    if (
                        not isargname(token.value)
                        or token.value == "firstline"
                        or token.value == "lastline"
                    ):
                        raise VimLParserException(
                            Err(
                                viml_printf("E125: Illegal argument: %s", token.value),
                                token.pos,
                            )
                        )
                        ...
                        raise VimLParserException(
                            Err(
                                viml_printf(
                                    "E853: Duplicate argument name: %s", token.value
                                ),
                                token.pos,
                            )
                        )
                    ...
                    if (
                        iswhite(self.reader.p(0))
                        and tokenizer.peek().type == TOKEN_COMMA
                    ):
                        raise VimLParserException(
                            Err(
                    ...
                    raise VimLParserException(
                        Err(viml_printf("unexpected token: %s", token.value), token.pos)
                    )
        while TRUE:
            self.reader.skip_white()
            ...
            if key == "":
                ...
                raise VimLParserException(
                    Err(viml_printf("unexpected token: %s", key), epos)
                )
        self.add_node(node)
    ...
    def parse_cmd_endfunction(self):
        ...
        if self.context[0].type != NODE_FUNCTION:
            raise VimLParserException(
                Err("E193: :endfunction not inside a function", self.ea.cmdpos)
            )
        self.reader.getn(-1)
    ...
    def parse_cmd_return(self):
        if self.find_context(NODE_FUNCTION) == -1:
            raise VimLParserException(
                Err("E133: :return not inside a function", self.ea.cmdpos)
            )
    ...
    def parse_cmd_call(self):
        ...
        if self.ends_excmds(c):
            raise VimLParserException(
                Err("E471: Argument required", self.reader.getpos())
            )
        node.left = self.parse_expr()
    ...
    def parse_cmd_endif(self):
        if (
            self.context[0].type != NODE_IF
            and self.context[0].type != NODE_ELSEIF
            and self.context[0].type != NODE_ELSE
        ):
    ...
    def parse_cmd_endwhile(self):
        if self.context[0].type != NODE_WHILE:
            raise VimLParserException(
                Err("E588: :endwhile without :while", self.ea.cmdpos)
            )
    ...
    def parse_cmd_continue(self):
        if self.find_context(NODE_WHILE) == -1 and self.find_context(NODE_FOR) == -1:
            raise VimLParserException(
                Err("E586: :continue without :while or :for", self.ea.cmdpos)
            )
    ...
    def parse_cmd_break(self):
        if self.find_context(NODE_WHILE) == -1 and self.find_context(NODE_FOR) == -1:
            raise VimLParserException(
                Err("E587: :break without :while or :for", self.ea.cmdpos)
            )
    ...
    def parse_cmd_catch(self):
        if self.context[0].type == NODE_FINALLY:
            raise VimLParserException(
                Err("E604: :catch after :finally", self.ea.cmdpos)
            )
    ...
    def parse_cmd_finally(self):
        if self.context[0].type != NODE_TRY and self.context[0].type != NODE_CATCH:
            raise VimLParserException(
                Err("E606: :finally without :try", self.ea.cmdpos)
            )
        if self.context[0].type != NODE_TRY:
            self.pop_context()
    ...
    def parse_cmd_endtry(self):
        if (
            self.context[0].type != NODE_TRY
            and self.context[0].type != NODE_CATCH
            and self.context[0].type != NODE_FINALLY
        ):
    ...
    def parse_lvalue_func(self):
        ...
        if (
            ...
            or node.type == NODE_CURLYNAME
            or node.type == NODE_SUBSCRIPT
            or node.type == NODE_DOT
            or node.type == NODE_OPTION
            or node.type == NODE_ENV
            or node.type == NODE_REG
        ):
            return node
    ...
    def parse_lvalue(self):
        ...
        if node.type == NODE_IDENTIFIER:
            if not isvarname(node.value):
                raise VimLParserException(
                    Err(
                        viml_printf("E461: Illegal variable name: %s", node.value),
                        node.pos,
                    )
                )
        if (
            ...
            while TRUE:
                ...
                if token.type == TOKEN_SQCLOSE:
                    ...
                    if token.type == TOKEN_SQCLOSE:
                        ...
                        raise VimLParserException(
                            Err(
                                viml_printf("E475 Invalid argument: %s", token.value),
                                token.pos,
                            )
                        )
                else:
    ...
    builtin_commands = [
        ...
        AttributeDict(
            {
                "name": "mzscheme",
                "minlen": 2,
                "flags": "RANGE|EXTRA|DFLALL|NEEDARG|CMDWIN|SBOXOK",
        ...
        AttributeDict(
            {
                "name": "perl",
                "minlen": 2,
                "flags": "RANGE|EXTRA|DFLALL|NEEDARG|SBOXOK|CMDWIN",
        ...
        AttributeDict(
            {
                "name": "perldo",
                "minlen": 5,
                "flags": "RANGE|EXTRA|DFLALL|NEEDARG|CMDWIN",
                "parser": "parse_cmd_common",
            }
        ),
        AttributeDict(
        ...
        AttributeDict(
            {
                "flags": "BANG|NEEDARG|EXTRA|NOTRLCOM|RANGE|NOTADR|DFLALL",
                "minlen": 3,
                "name": "cdo",
                "parser": "parse_cmd_common",
            }
        ...
        AttributeDict(
            {
                "flags": "BANG|FILE1|NEEDARG|TRLBAR|SBOXOK|CMDWIN",
                "minlen": 2,
                "name": "packadd",
                "parser": "parse_cmd_common",
            }
...
class ExprTokenizer:
    ...
    def get2(self):
        ...
        if c == "<EOF>":
            ...
            raise VimLParserException(
                Err(viml_printf("unexpected character: %s", c), self.reader.getpos())
            )
    ...
    def get_dstring(self):
        ...
        while TRUE:
            ...
            if c == "<EOF>" or c == "<EOL>":
                ...
                if c == "<EOF>" or c == "<EOL>":
                    raise VimLParserException(
                        Err("ExprTokenizer: unexpected EOL", self.reader.getpos())
                    )
                self.reader.seek_cur(1)
                s += c
            else:
...
class ExprParser:
        ...
        while TRUE:
            ...
            if not iswhite(c) and token.type == TOKEN_SQOPEN:
                ...
                if viml_len(node.rlist) > MAX_FUNC_ARGS:
                    ...
                    raise VimLParserException(
                        Err("E740: Too many arguments for function", node.pos)
                    )
                left = node
                del node
    ...
    def parse_expr9(self):
        ...
        if token.type == TOKEN_NUMBER:
            ...
            if not lambda_ and not (
                token.type == TOKEN_SQUOTE or token.type == TOKEN_DQUOTE
            ):
                    ...
                    if token.type == TOKEN_ARROW:
                        ...
                        if not isargname(token.value):
                            raise VimLParserException(
                                Err(
                                    viml_printf(
                                        "E125: Illegal argument: %s", token.value
                                    ),
                                    token.pos,
                                )
                            )
                        ...
                        if (
                            iswhite(self.reader.p(0))
                            and self.tokenizer.peek().type == TOKEN_COMMA
                        ):
                            raise VimLParserException(
                                Err(
                                    "E475: Invalid argument: White space is not allowed before comma",
            ...
            token.type == TOKEN_IS
            or token.type == TOKEN_ISCS
            or token.type == TOKEN_ISNOT
            or token.type == TOKEN_ISNOTCS
        ):
            self.reader.seek_set(pos)
            node = self.parse_identifier()
    ...
    def parse_dot(self, token, left):
        if (
            ...
            and left.type != NODE_CURLYNAME
            and left.type != NODE_DICT
            and left.type != NODE_SUBSCRIPT
            and left.type != NODE_CALL
            and left.type != NODE_DOT
        ):
            return NIL
        ...
        while TRUE:
            ...
            if isnamec(c):
                ...
                if c != "}":
                    raise VimLParserException(
                        Err(
                            viml_printf("unexpected token: %s", c), self.reader.getpos()
                        )
                    )
                self.reader.seek_cur(1)
            else:
    ...
    def __init__(self, lines):
        ...
        while lnum < viml_len(lines):
            ...
            for c in viml_split(lines[lnum], "\\zs"):
                ...
                col += viml_len(c)
                offset += viml_len(c)
            while lnum + 1 < viml_len(lines) and viml_eqregh(
                lines[lnum + 1], "^\\s*\\\\"
            ):
                skip = TRUE
                col = 0
...
class Compiler:
    ...
    def compile(self, node):
        if node.type == NODE_TOPLEVEL:
            ...
            raise VimLParserException(
                viml_printf("Compiler: unknown node: %s", viml_string(node))
            )
        return NIL
    ...
    def compile_or(self, node):
        return viml_printf(
            "(|| %s %s)", self.compile(node.left), self.compile(node.right)
        )
    ...
    def compile_is(self, node):
        return viml_printf(
            "(is %s %s)", self.compile(node.left), self.compile(node.right)
        )
    ...
    def compile_isnot(self, node):
        return viml_printf(
            "(isnot %s %s)", self.compile(node.left), self.compile(node.right)
        )
    ...
    def compile_concat(self, node):
        return viml_printf(
            "(concat %s %s)", self.compile(node.left), self.compile(node.right)
        )
    ...
    def compile_subscript(self, node):
        return viml_printf(
            "(subscript %s %s)", self.compile(node.left), self.compile(node.right)
        )
    ...
    def compile_dot(self, node):
        return viml_printf(
            "(dot %s %s)", self.compile(node.left), self.compile(node.right)
        )
    ...
    def compile_call(self, node):
        ...
        if viml_empty(rlist):
            ...
            return viml_printf(
                "(%s %s)", self.compile(node.left), viml_join(rlist, " ")
            )
    ...
    def compile_dict(self, node):
        value = [
            "(" + self.compile(vval[0]) + " " + self.compile(vval[1]) + ")"
            for vval in node.value
        ]
        if viml_empty(value):
            return "(dict)"
        else:
    ...
    def compile_lambda(self, node):
        rlist = [self.compile(vval) for vval in node.rlist]
        return viml_printf(
            "(lambda (%s) %s)", viml_join(rlist, " "), self.compile(node.left)
        )
...
class RegexpParser:
    ...
    def parse_regexp(self):
        ...
        if self.reader.peekn(4) == "\\%#=":
            ...
            if token != "\\%#=0" and token != "\\%#=1" and token != "\\%#=2":
                raise VimLParserException(
                    Err("E864: \\%#= can only be followed by 0, 1, or 2", epos)
                )
            viml_add(ret, token)
        ...
        if c == "\\":
            ...
            if c == "d":
                ...
                if r != "":
                    return ["\\%U" + r, "\\%U" + r]
            raise VimLParserException(
                Err("E678: Invalid character after \\%[dxouU]", epos)
            )
        return ["\\" + c, c]
    ...
    def get_token_brace(self, pre):
        ...
        if self.reader.p(0) != "}":
            raise VimLParserException(
                Err("E554: Syntax error in \\{...}", self.reader.getpos())
            )
        self.reader.get()
    ...
    def get_token_sq(self, pre):
        ...
        while TRUE:
            ...
            if (
                startc != 0
                and self.reader.p(0) == "-"
                and not self.isend(self.reader.p(1))
                and not (self.reader.p(1) == "\\" and self.reader.p(2) == "n")
            ):
                self.reader.seek_cur(1)
                r += "-"
                c = self.reader.p(0)
                ...
                if startc > endc or endc > startc + 256:
                    raise VimLParserException(
                        Err("E16: Invalid range", self.reader.getpos())
                    )
    ...
    def get_token_sq_coll_element(self):
        if (
            self.reader.p(0) == "["
            and self.reader.p(1) == "."
            and not self.isend(self.reader.p(2))
            and self.reader.p(3) == "."
            and self.reader.p(4) == "]"
        ):
    ...
    def get_token_percent_sq(self, pre):
        ...
        while TRUE:
            ...
            if self.isend(c):
                raise VimLParserException(
                    Err("E69: Missing ] after \\%[", self.reader.getpos())
                )
            elif c == "]":
                if r == "":
                    raise VimLParserException(
                        Err("E70: Empty \\%[", self.reader.getpos())
                    )
                self.reader.seek_cur(1)
                break
            self.reader.seek_cur(1)
    ...
    def get_token_mlvc(self, pre):
        ...
        raise VimLParserException(
            Err("E71: Invalid character after %", self.reader.getpos())
        )
