...
class ComposedDecodingStrategy(DecodingStrategy):
    ...
    def decode(self, bytes_seq, debug_hint):
        ...
        debug_hint["composed_strategies"] = [
            type(strategy).__name__ for strategy in self.strategies
        ]
...
class DecodingStrategyByScriptencoding(DecodingStrategy):
    def decode(self, bytes_seq, debug_hint):
        # type: (bytes, Dict[str, Any]) -> Optional[str]
        encoding_part = DecodingStrategyByScriptencoding.parse_script_encoding(
            bytes_seq, debug_hint
        )
    ...
    def parse_script_encoding(cls, bytes_seq, debug_hint):
        ...
        try:
            ...
            try:
                encoding_part_end_index_candidate_by_line_break = bytes_seq.index(
                    LF, encoding_part_start_index
                )
            ...
            encoding_part_candidate = bytes_seq[
                encoding_part_start_index:encoding_part_end_index
            ]
