...
from vint.ast.traversing import SKIP_CHILDREN, traverse
...
class ScopeLinker(object):
    ...
    class ScopeTreeBuilder(object):
        ...
        def handle_new_index_parameters_found(self, params_number):
            ...
            for variadic_index in range(20 - params_number):
                ...
                variadic_param = self._create_virtual_identifier(
        ...
        def handle_new_variable_found(self, node):
            current_scope = self.get_current_scope()
            scope_visibility_hint = detect_scope_visibility(node, current_scope)
            ...
            self._add_variable(
                objective_scope, node, is_function=is_function, is_implicit=is_implicit
            )
        ...
        def _add_variable(
            self, objective_scope, node, is_implicit=False, is_function=False
        ):
            current_scope = self.get_current_scope()
        ...
        def _add_symbol_table_variables(self, objective_scope):
            ...
            for symbol_table_variable_name in symbol_table_variable_names:
                virtual_node = self._create_virtual_identifier(
                    symbol_table_variable_name
                )

                self._register_variable(
                    objective_scope, symbol_table_variable_name, virtual_node
                )
        ...
        def _register_variable(
            ...
            variable = self._create_variable(
                is_implicit=is_implicit, is_builtin=is_builtin
            )

            objective_variable_list = objective_scope[
                "functions" if is_function else "variables"
            ]
    ...
    def process(self, ast):
        ...
        traverse(
            attached_ast, on_enter=self._enter_handler, on_leave=self._leave_handler
        )
