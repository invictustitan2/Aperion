...
class ConfigFileSource(ConfigSource):
    ...
    def _warn_invalid_severity(self, severity_name):
        possible_severities = ", ".join(
            [possible_severity.name.lower() for possible_severity in list(Level)]
        )

        logging.warning(
            (
                "Severity `{severity_name}` is invalid. "
                + "Possible: {possible_severities}"
            ).format(
                severity_name=severity_name, possible_severities=possible_severities
            )
        )
