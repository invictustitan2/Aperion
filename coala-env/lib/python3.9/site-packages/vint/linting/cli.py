...
class CLI(object):
    ...
    def _build_argparser(self):
        ...
        parser.add_argument(
            "-v", "--version", action="version", version=self._get_version()
        )
        parser.add_argument(
            "-V",
            "--verbose",
        ...
        parser.add_argument(
            "-e", "--error", action="store_const", const=True, help="report only errors"
        )
        parser.add_argument(
            "-w",
            "--warning",
            action="store_const",
            const=True,
            help="report errors and warnings",
        )
        parser.add_argument(
            "-s",
            "--style-problem",
            action="store_const",
            const=True,
            help="report errors, warnings and style problems",
        )
        parser.add_argument(
            "-m", "--max-violations", type=int, help="limit max violations count"
        )
        parser.add_argument(
            "-c",
            "--color",
            action="store_const",
            const=True,
            help="colorize output when possible",
        )
        parser.add_argument(
            "--no-color",
            action="store_const",
        ...
        parser.add_argument(
            "-j", "--json", action="store_const", const=True, help="output json style"
        )
        parser.add_argument(
            "-t",
            "--stat",
