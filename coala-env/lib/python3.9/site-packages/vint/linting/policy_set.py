...
class PolicySet(object):
    ...
    def _create_all_policies_map(self):
        ...
        policy_map = {
            ...
            for policy_name, PolicyClass in policy_class_map.items()
        }
    ...
    def _warn_unexistent_policy(self, policy_name):
        logging.warning("Policy `{name}` is not defined".format(name=policy_name))
    ...
    def _log_policy_status(self, policy_name, enabled):
        ...
        logging.debug(
            "{status}: {policy_name}".format(status=status, policy_name=policy_name)
        )
