...
from ipython_genutils.py3compat import cast_bytes, cast_unicode, unicode_type
...
class NotebookNotary(LoggingConfigurable):
    ...
    secret_file = Unicode(help="""The file where the secret key is stored.""").tag(
        config=True
    )
    ...
    secret = Bytes(help="""The secret key with which notebooks are signed.""").tag(
        config=True
    )
