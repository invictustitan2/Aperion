...
def _CanPlaceOnSingleLine(uwline):
    ...
    last = uwline.last
    last_index = -1
    if last.is_comment and re.search(
        r"^#+\s+pylint:", last.value.strip(), re.IGNORECASE
    ):
...
def _FormatFirstToken(first_token, indent_depth, prev_uwline, final_lines):
    ...
    first_token.AddWhitespacePrefix(
        _CalculateNumberOfNewlines(first_token, indent_depth, prev_uwline, final_lines),
