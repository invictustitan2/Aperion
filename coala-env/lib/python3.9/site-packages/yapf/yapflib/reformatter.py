...
from yapf.yapflib import (format_decision_state, format_token, line_joiner,
                          pytree_utils, style, verifier)
...
def _CanPlaceOnSingleLine(uwline):
  ...
  last = uwline.last
  last_index = -1
  if last.is_comment and re.search(r'^#+\s+pylint:',
                                   last.value.strip(), re.IGNORECASE):
