...
from yapf.yapflib import format_token, py3compat, pytree_utils, pytree_visitor, style
...
class _TreePenaltyAssigner(pytree_visitor.PyTreeVisitor):
    ...
    def Visit_import_as_names(self, node):  # pyline: disable=invalid-name
        ...
        for child in node.children:
            if (
                prev_child
                and isinstance(prev_child, pytree.Leaf)
    ...
    def Visit_funcdef(self, node):  # pylint: disable=invalid-name
        ...
        while colon_idx < len(node.children):
            if (
                isinstance(node.children[colon_idx], pytree.Leaf)
    ...
    def Visit_trailer(self, node):  # pylint: disable=invalid-name
        ...
        if node.children[0].value == ".":
            self._SetUnbreakableOnChildren(node)
            pytree_utils.SetNodeAnnotation(
                node.children[1], pytree_utils.Annotation.SPLIT_PENALTY, DOTTED_NAME
            )
    ...
    def Visit_atom(self, node):  # pylint: disable=invalid-name
        ...
        if node.children[0].value == "(":
            ...
            if len(node.children) == 2:
                ...
                and rbracket.column - lbracket.column < style.Get("COLUMN_LIMIT")
            ):
                self._SetStronglyConnected(*node.children[1:])
