...
from yapf.yapflib import format_token, split_penalty, style, unwrapped_line
...
class FormatDecisionState(object):
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and (
                self.ignore_stack_for_comparison
    ...
    def MustSplit(self):
        ...
        if (
            format_token.Subtype.DICTIONARY_KEY in current.subtypes
        ...
        if (
            style.Get("SPLIT_BEFORE_NAMED_ASSIGNS")
            and not current.is_comment
            and format_token.Subtype.DEFAULT_OR_NAMED_ASSIGN_ARG_LIST
            in current.subtypes
        ):
            if (
                previous.value not in {"=", ":", "*", "**"}
                ...
                if not _IsFunctionDefinition(previous):
                    ...
                    opening_column = (
                        len(pptoken.value) if pptoken else 0 - indent_amt - 1
                    )
                    if previous.value == "(":
        ...
        if (
            previous.OpensScope()
            and not current.OpensScope()
            and format_token.Subtype.SUBSCRIPT_BRACKET not in previous.subtypes
        ):
        ...
        if previous.value == "{":
            ...
            if (
                not self._FitsOnLine(previous, closing)
        ...
        if (
            ...
            length = _GetLengthOfSubtype(
                current, format_token.Subtype.COMP_FOR, format_token.Subtype.COMP_IF
            )
            if length + self.column > self.column_limit:
                return True

        if (
            format_token.Subtype.COMP_IF in current.subtypes
        ...
        if (
            current.is_name
            and previous_previous_token
            and previous_previous_token.is_name
            and previous.value == "("
        ):
            if not self._FitsOnLine(previous, previous.matching_bracket):
    ...
    def _AddTokenOnCurrentLine(self, dry_run):
        ...
        if previous.OpensScope():
            if not current.is_comment:
                ...
                self.stack[-1].closing_scope_indent = self.stack[-1].indent - style.Get(
    ...
    def _AddTokenOnNewline(self, dry_run, must_split):
        ...
        if previous.OpensScope() or (
            previous.is_comment
            and previous.previous_token is not None
    ...
    def _GetNewlineColumn(self):
        ...
        if (
            previous
            and previous.is_string
            and current.is_string
        ...
        if self.line.first.value in _COMPOUND_STMTS and (
            ...
            token_indent = len(
                self.line.first.whitespace_prefix.split("\n")[-1]
    ...
    def _EachDictEntryFitsOnOneLine(self, opening):
        ...
        while current and current != closing:
            ...
            if current.OpensScope():
                if (
                    current.value == "{"
                    or (current.is_pseudo_paren and current.next_token.value == "{")
                ) and format_token.Subtype.DICTIONARY_VALUE in current.subtypes:
...
def _GetLengthOfSubtype(token, subtype, exclude=None):
    ...
    while (
        current.next_token
        and subtype in current.subtypes
...
def _IsFunctionDefinition(current):
    ...
    return (
        current.value == "(" and prev and format_token.Subtype.FUNC_DEF in prev.subtypes
    )
