            ...
            if isinstance(child, pytree.Node):
                ...
                if child.prefix.lstrip().startswith("#"):
                    ...
                    if child.type == token.NEWLINE:
                        ...
                        comment_column += len(comment_prefix) - len(
                            comment_prefix.lstrip()
                        )
                        ...
                        if ancestor_at_indent.type == token.DEDENT:
                            ...
                            while index < len(comments):
                                cmt = comments[index]
                                if not cmt.strip() or cmt.startswith(
                                    prefix_indent + "#"
                                ):
                                    before.append(cmt)
                                else:
                        ...
                        for leaf_in_parent in stmt_parent.leaves():
                            if leaf_in_parent.type == token.NEWLINE:
                                ...
                                node_with_line_parent = (
                                    _FindNodeWithStandaloneLineParent(child)
                                )
                                pytree_utils.InsertNodesBefore(
                                    _CreateCommentsFromPrefix(
                                        comment_prefix,
                                ...
                                if comment_lineno == prev_leaf[0].lineno:
                                    ...
                                    if value.rstrip("\n"):
                                        ...
                                        comment_column += len(comment_lines[0]) - len(
                                            comment_lines[0].lstrip()
                                        )
                                ...
                                comment_column = len(comment_prefix[rindex:]) - len(
    ...
    while index < len(lines):
        ...
        if comment_block:
            ...
            comment_node = (
                comment_leaf
                if not standalone
                else pytree.Node(pygram.python_symbols.simple_stmt, [comment_leaf])
            )
