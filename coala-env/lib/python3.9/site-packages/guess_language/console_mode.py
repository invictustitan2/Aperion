...
if getattr(sys.stdout, "errors", "") == "strict" and not getattr(
    sys.stdout, "encoding", ""
    ...
    try:
        ...
        def simple_translit_error_handler(error):
            ...
            repl = simplify(chunk)
            repl = repl.encode(error.encoding, "backslashreplace").decode(
                error.encoding
            )
        ...
        class SimpleTranslitStreamFilter:
            ...
            def writelines(self, lines):
                self.target.writelines([self.downgrade(line) for line in lines])

            def downgrade(self, s):
                return s.encode(self.target.encoding, self.errors).decode(
                    self.target.encoding
                )

        codecs.register_error(
            SimpleTranslitStreamFilter.errors, simple_translit_error_handler
        )
