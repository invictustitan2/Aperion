...
import setuptools
from pbr import extra_files, git, options, testr_command, version
...
def generate_script(group, entry_point, header, template):
    ...
    if not entry_point.attrs or len(entry_point.attrs) > 2:
        raise ValueError(
            "Script targets must be of the form " "'func' or 'Class.class_method'."
        )
...
class LocalSDist(sdist.sdist):
    ...
    def checking_reno(self):
        ...
        if not os.path.exists(os.path.join(conf.reporoot, conf.notespath)):
            log.info(
                "[pbr] reno does not appear to be configured. Skipping " "release notes"
            )
            self._has_reno = False
            return False
