...
class TestInstallWithoutPbr(base.BaseTestCase):
    ...
    def test_install_without_pbr(self):
        ...
        venv = self.useFixture(
            test_packaging.Venv("nopbr", ["pip", "wheel", "setuptools"])
        )
        python = venv.python
        ...
        self.useFixture(
            base.CapturedSubprocess("nopbr", [pbr_cmd] + ["freeze"], cwd=test_pkg_dir)
        )
...
class TestMarkersPip(base.BaseTestCase):
    ...
    scenarios = [("pip-latest", {"modules": ["pip", "setuptools"]})] + lts_scenarios
