...
def _get_member(obj, member):
    ...
    if callable(attribute) and hasattr(attribute, "__self__"):
        ...
        try:
            ...
            raise TypeError(
                "Given bound method '" + member + "' must be "
...
def _construct_repr_string(obj, members):
    ...
    values = ", ".join(
        member + "=" + func(_get_member(obj, member)) for member, func in members
    )
    return "<" + type(obj).__name__ + " object(" + values + ") at " + hex(id(obj)) + ">"
...
def get_public_members(obj):
    ...
    return {
        ...
        for attr in dir(obj)
        if not attr.startswith("_") and not hasattr(getattr(obj, attr), "__call__")
    }
    ...
    if members:
        ...
        for i, member in enumerate(members_to_print):
            if isinstance(member, tuple):
                ...
                length = len(member)
                if length == 2:
                    members_to_print[i] = (member[0], member[1] if member[1] else repr)
                else:
                    raise ValueError(
                        "Passed tuple "
                        + repr(member)
        ...
        def __repr__(self):
            ...
            member_repr_list = (
                (member, repr) for member in sorted(members_to_print, key=str.lower)
            )
...
def generate_eq(*members):
    ...
    def decorator(cls):
        def eq(self, other):
            ...
            return all(
                getattr(self, member) == getattr(other, member) for member in members
            )
...
def generate_ordering(*members):
    ...
    def decorator(cls):
        def lt(self, other):
            if not isinstance(other, cls):
                raise TypeError("Comparison with unrelated classes is " "unsupported.")
...
def assert_right_type(value, types, argname):
    ...
    raise TypeError(
        "{} must be an instance of one of {} (provided value: "
...
def generate_consistency_check(*members):
    ...
    def decorator(cls):
        cls.check_consistency = lambda self: all(
            getattr(self, member) for member in members
        )
