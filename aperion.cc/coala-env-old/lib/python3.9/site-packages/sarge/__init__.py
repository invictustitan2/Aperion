    ...
    def reader(self, stream, ready):
        ...
        while not self._done:
            ...
            if chunk:
                ...
                logger.debug(
                    "queued chunk of length %d to %s: %r",
                    len(chunk),
                    self.buffer,
                    chunk[:30],
                )
...
class CommandLineParser(object):
            ...
            while tt in ("|", "|&"):
                ...
                if (tt == "|" and 1 in last_part.redirects) or (
                    ...
                    if last_part.redirects != SWAP_OUTPUTS:
                        raise ValueError(
                            "semantics: cannot redirect and pipe the " "same stream"
                        )
                self.consume(tt)
    ...
    def parse_command(self):
        ...
        if node.redirects != SWAP_OUTPUTS:
            ...
            if d:
                raise ValueError(
                    "semantics: can only redirect stdout and "
    ...
    def run_command_node(self, node, input, async_):
        ...
        if node != self.last:
            ...
            if self.stdout and stdout:
                raise ValueError("You cannot redirect one stream to two " "places")
