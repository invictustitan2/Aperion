...
from coalib.parsing.Globbing import fnmatch, glob_escape, iglob
...
def _import_bears(file_path, kinds):
    # recursive imports:
    for bear_list in iimport_objects(
        file_path, names="__additional_bears__", types=list
    ):
...
def collect_files(
    file_paths, log_printer, ignored_file_paths=None, limit_file_paths=None
):
    """
    Evaluate globs in file paths and return all matching files
    ...
    """
    limit_fnmatch = (
        functools.partial(fnmatch, globs=limit_file_paths)
        if limit_file_paths
        else lambda fname: True
    )

    valid_files = list(
        filter(
            lambda fname: os.path.isfile(fname[0]),
...
def collect_dirs(dir_paths, ignored_dir_paths=None):
    ...
    valid_dirs = list(
        filter(
            lambda fname: os.path.isdir(fname[0]),
...
def icollect_bears(bear_dir_glob, bear_globs, kinds, log_printer):
    ...
    for bear_dir, dir_glob in filter(
        lambda x: os.path.isdir(x[0]), icollect(bear_dir_glob)
    ):
                ...
                try:
                    ...
                    log_printer.log_exception(
                        (
                            ...
                            "version."
                        ).format(file=matching_file, pkg=exception.req),
                        exception,
...
def filter_capabilities_by_languages(bears, languages):
    ...
    for section_bears in bears.values():
        for bear in section_bears:
            ...
            capabilities = (
                language_bears_capabilities[language] if language else tuple()
            )
            language_bears_capabilities.update(
                {
                    language: (
                        capabilities[0] | bear.can_detect,
...
def collect_registered_bears_dirs(entrypoint):
    ...
    for ep in pkg_resources.iter_entry_points(entrypoint):
        ...
        collected_dirs.append(
            os.path.abspath(os.path.dirname(registered_package.__file__))
        )
