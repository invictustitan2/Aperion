...
from coala_utils.string_processing import (convert_to_raw, position_is_escaped,
                                           unescape, unescaped_rstrip)
...
class LineParser:
    ...
    def parse(self, line):
        ...
        """
        logging.warning(
            "The parse method of LineParser is deprecated and will"
    ...
    def _parse(self, line):
        ...
        for separator in self.comment_separators:
            if re.match("[^ ]" + separator, line) or re.match(separator + "[^ ]", line):
                logging.warning(
                    "This comment does not have whitespace"
                    + " before or after "
                    + separator
                    + " in: "
                    + repr(line.replace("\n", ""))
    ...
    @staticmethod
    def __separate_by_first_occurrence(
        string, delimiters, strip_delim=False, return_second_part_nonempty=False
    ):
        """
    ...
    def __get_section_name(self, line):
        for begin, end in self.section_name_surroundings.items():
            if (
                line[0 : len(begin)] == begin
                and line[len(line) - len(end) : len(line)] == end
            ):
    ...
    def __extract_keys_and_value(
        self, line, delimiters, return_second_part_nonempty=False
    ):
        """
        ...
        keys = list(
            StringConverter(key_part, list_delimiters=self.key_delimiters).__iter__(
