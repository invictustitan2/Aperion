...
class Bear:
    ...
    def get_metadata(cls):
        ...
        """
        return FunctionMetadata.from_function(cls.analyze, omit={"self"})
        ...
        if not_installed_requirements:
            return "Following requirements are not installed: " + ", ".join(
                "{} (installable via `{}`)".format(
                    requirement.package, " ".join(requirement.install_command())
                )
    ...
    def download_cached_file(cls, url, filename):
        ...
        logging.info("{}: Downloading {} into {!r}.".format(cls.name, url, filename))
