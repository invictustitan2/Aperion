...
def filter_results(
    original_file_dict, modified_file_dict, original_results, modified_results
):
    """
    ...
    for file in original_file_dict:
        diffs_dict[file] = Diff.from_string_arrays(
            original_file_dict[file], modified_file_dict[renamed_files.get(file, file)]
        )

    orig_result_diff_dict_dict = remove_result_ranges_diffs(
        original_results, original_file_dict
    )

    mod_result_diff_dict_dict = remove_result_ranges_diffs(
        modified_results, modified_file_dict
    )
...
def basics_match(original_result, modified_result):
    ...
    * debug_msg
    ...
    return all(
        getattr(original_result, member) == getattr(modified_result, member)
        for member in ["origin", "message", "severity", "debug_msg"]
    )


def source_ranges_match(
    original_file_dict,
    diff_dict,
    ...
    for file_name in original_file_dict:
        ...
        modified_total_file = modified_result_diff_dict[
            renamed_files.get(file_name, file_name)
        ].modified
...
def remove_range(file_contents, source_range):
    ...
    if source_range.start.line == source_range.end.line:
        ...
        newfile[source_range.start.line - 1] = newfile[source_range.start.line - 1][
            : source_range.start.column - 1
        ]

        # cut away before end
        newfile[source_range.end.line - 1] = newfile[source_range.end.line - 1][
    ...
    for original_result in result_list:
        ...
        for file_name in file_dict:
            diff_dict[file_name] = Diff.from_string_arrays(
                file_dict[file_name], mod_file_dict[file_name]
            )
...
def ensure_files_present(original_file_dict, modified_file_dict):
    ...
    renamed_files_dict = {}
    for file in filter(
        lambda filter_file: filter_file not in original_files, affected_files
    ):
    ...
    for file in filter(
        lambda filter_file: filter_file not in modified_files, affected_files
    ):
