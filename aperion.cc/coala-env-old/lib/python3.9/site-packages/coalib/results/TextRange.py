...
class TextRange:
    ...
    @classmethod
    def from_values(
        cls, start_line=None, start_column=None, end_line=None, end_column=None
    ):
        """
        Creates a new TextRange.
    ...
    def join(cls, a, b):
        ...
        if not isinstance(a, cls) or not isinstance(b, cls):
            raise TypeError("only instances of {} can be joined".format(cls.__name__))

        if not a.overlaps(b):
            raise ValueError("{}s must overlap to be joined".format(cls.__name__))
    ...
    def expand(self, text_lines):
        ...
        end_column = (
            len(text_lines[end_line - 1])
            if self.end.column is None
