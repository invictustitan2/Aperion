...
from coala_utils.decorators import (enforce_signature, generate_ordering,
                                    generate_repr, get_public_members)
...
class Result:
    ...
    def from_values(
        ...
        :param aspect:
            ...
            belongs to.)
        """
        range = SourceRange.from_values(file, line, column, end_line, end_column)
    ...
    def location_repr(self):
        ...
        return ", ".join(
            repr(relpath(range_path)) for range_path in sorted(range_paths)
        )
    ...
    def __json__(self, use_relpath=False):
        ...
        if use_relpath and _dict["diffs"]:
            _dict["diffs"] = {
                relpath(file): diff for file, diff in _dict["diffs"].items()
            }
