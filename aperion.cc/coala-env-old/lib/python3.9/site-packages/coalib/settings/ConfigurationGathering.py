...
from coalib.parsing.CliParsing import check_conflicts, parse_cli
        ...
        if not silent:
            if os.path.basename(filename) == Constants.default_coafile:
                ...
                log_printer.err(
                    ...
                    "suppress any use of config files with `-I`.".format(filename)
                )
                sys.exit(2)
...
def warn_config_absent(sections, argument, log_printer):
    ...
    if all(argument not in section for section in sections.values()):
        log_printer.warn(
            "coala will not run any analysis. Did you forget "
...
def load_configuration(arg_list, log_printer, arg_parser=None):
    ...
    if (
        bool(cli_sections["cli"].get("find_config", "False"))
    ...
    if bool(cli_sections["cli"].get("no_config", "False")):
        ...
        user_sections = load_config_file(
            Constants.user_coafile, log_printer, silent=True
        )
...
def get_filtered_bears(languages, log_printer, arg_parser=None):
    ...
    :return:            Tuple containing dictionaries of local bears
                        and global bears.
    """
    sections, _ = load_configuration(
        arg_list=None, log_printer=log_printer, arg_parser=arg_parser
    )
    local_bears, global_bears = collect_all_bears_from_sections(sections, log_printer)
    if languages:
...
def gather_configuration(acquire_settings, log_printer, arg_list=None, arg_parser=None):
    """
    ...
    local_bears, global_bears = fill_settings(sections, acquire_settings, log_printer)
