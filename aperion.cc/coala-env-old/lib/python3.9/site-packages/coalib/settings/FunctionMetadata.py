...
class FunctionMetadata:
    ...
    def _filter_out_omitted(self, params):
        ...
        """
        return OrderedDict(
            filter(lambda p: p[0] not in self.omit, tuple(params.items()))
        )
    ...
    def _get_param(param, section, annotation):
        ...
        try:
            ...
            raise ValueError(
                "Unable to convert parameter {!r} into type "
        ...
        for i, arg in enumerate(args):
            ...
            if i < num_non_defaults:
                ...
                optional_params[arg] = (
                    doc_comment.param_dict.get(arg, cls.str_nodesc)
                    + " ("
                    + cls.str_optional.format(defaults[i - num_non_defaults])
                    + ")",
                    argspec.annotations.get(arg, None),
    ...
    def filter_parameters(self, dct):
        ...
        return {
            key: dct[key]
            for key in (self.non_optional_params.keys() | self.optional_params.keys())
            if key in dct
        }
    ...
    def merge(cls, *metadatas):
        ...
        merged_name = (
            "<Merged signature of "
            + ", ".join(repr(metadata.name) for metadata in metadatas)
            + ">"
        )
