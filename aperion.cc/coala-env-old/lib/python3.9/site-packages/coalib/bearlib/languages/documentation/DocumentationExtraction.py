...
def _extract_doc_comment_from_line(
    content, line, column, regex, marker_dict, docstyle_definition
):
    ...
    if begin_match:
        ...
        for marker in marker_dict[begin_match.group()]:
            ...
            if doc_comment is not None:
                ...
                rng = TextRange.from_values(
                    line + 1, begin_match.start() + 1, end_line + 1, end_column + 1
                )
                doc = DocumentationComment(
                    documentation, docstyle_definition, indent, marker, rng
                )
...
def extract_documentation_with_markers(content, docstyle_definition):
    ...
    while line < len(content):
        line, column, doc = _extract_doc_comment_from_line(
            content, line, column, begin_regex, marker_dict, docstyle_definition
        )
        if doc:
            yield doc
