...
def to_camelcase(string):
    ...
    string = re.sub(
        "^(_*)(.)", lambda match: match.group(1) + match.group(2).lower(), string
    )
...
def to_pascalcase(string):
    ...
    string = re.sub("(\s)", lambda match: "_", string)
    string = re.sub(
        "^(_*)(.)", lambda match: match.group(1) + match.group(2).upper(), string
    )
    return re.sub("(?<=[^_])_+([^_])", lambda match: match.group(1).upper(), string)
...
def to_snakecase(string):
    ...
    string = re.sub("(\s)", lambda match: "_", string)
    string = re.sub(
        "^(_*)([^_])", lambda match: match.group(1) + match.group(2).lower(), string
    )
    string = re.sub(
        "(\w*)([.]+)([A-Z])",
        lambda match: (match.group(1) + match.group(2) + match.group(3).lower()),
        string,
    )
    string = re.sub(
        "(?<=[^_])_+([^_])", lambda match: "_" + match.group(1).lower(), string
    )
    return re.sub("[A-Z]", lambda match: "_" + match.group(0).lower(), string)
...
def to_spacecase(string):
    ...
    string = re.sub("^(\s*)(.)", lambda match: match.group(2).upper(), string)
    string = re.sub("(\s*)$", "", string)
    string = re.sub(
        "(?<=[^\s])\s+([^\s])", lambda match: " " + match.group(1).upper(), string
    )
    return re.sub("(?<=[^\s])([A-Z])", lambda match: " " + match.group(1), string)
...
def to_kebabcase(string):
    ...
    string = re.sub("(\s)", lambda match: "-", string)
    string = re.sub("(\_)", lambda match: "-", string)
    string = re.sub(
        "^(-*)([^-])", lambda match: match.group(1) + match.group(2).lower(), string
    )
    string = re.sub(
        "(\w*)([.]+)([A-Z])",
        lambda match: (match.group(1) + match.group(2) + match.group(3).lower()),
    ...
    string = re.sub(
        "(?<=[^-])-+([^-])", lambda match: "-" + match.group(1).lower(), string
    )
    string = re.sub("[A-Z]", lambda match: "-" + match.group(0).lower(), string)
    string = re.sub(r"^-+|-+$", lambda match: "" * len(match.group()), string)
