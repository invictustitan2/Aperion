...
class LocalBearTestHelper(unittest.TestCase):  # pragma: no cover
    ...
    def check_validity(
        ...
        self.assertIsInstance(
            local_bear, LocalBear, msg="The given bear is not a local bear."
        )
        self.assertIsInstance(
            lines, (list, tuple), msg="The given lines are not a list."
        )
        ...
        ) as (file, fname), execute_bear(local_bear, fname, file) as bear_output:
            if valid:
                msg = (
                    "The local bear '{}' yields a result although it "
                    "shouldn't.".format(local_bear.__class__.__name__)
                )
                self.assertEqual(bear_output, [], msg=msg)
            else:
                msg = (
                    "The local bear '{}' yields no result although it "
    ...
    def check_results(
        ...
        with prepare_file(
            ...
            create_tempfile=create_tempfile,
            tempfile_kwargs=tempfile_kwargs,
        ) as (file, fname), execute_bear(
            local_bear, fname, file, **settings
        ) as bear_output:
            msg = (
                "The local bear '{}' doesn't yield the right results. Or "
                "the order may be wrong.".format(local_bear.__class__.__name__)
            )
            if not check_order:
