...
def add_car():
    ...
    if action == "add":
        c.execute(
            "INSERT INTO package_cars (car_number, weight, location, status, complete) VALUES (?, ?, ?, 'Not on site', 0)",
            (car_number, weight, location),
        )
    elif action == "shift_start":
        c.execute(
            "INSERT INTO package_cars (car_number, weight, location, status, complete) VALUES ('Shift Start', 'N/A', 'N/A', 'Shift Start', 0)"
        )
    elif action == "shift_end":
        c.execute(
            "INSERT INTO package_cars (car_number, weight, location, status, complete) VALUES ('Shift End', 'N/A', 'N/A', 'Shift End', 0)"
        )
    conn.commit()
    conn.close()
