...
def append_to_sections(
    sections, key, value, origin, section_name=None, from_cli=False, to_append=False
):
    """
    ...
    sections[section_name.lower()].append(
        Setting(key, str(value), origin, from_cli=from_cli, to_append=to_append)
    )
...
class Section:
    ...
    def add_or_create_setting(self, setting, custom_key=None, allow_appending=True):
        """
    ...
    def __str__(self):
        value_list = ", ".join(
            key + " : " + repr(str(self[key])) for key in self.contents
        )
    ...
    def set_default_section(self, sections, section_name=None):
        ...
        :param section_name: Optional section name argument to find the default
                             ...
                             name.
        """
        default_section = ".".join((section_name or self.name).split(".")[:-1]).lower()
