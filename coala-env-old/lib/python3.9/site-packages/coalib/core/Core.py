...
def cleanup_bear(bear, running_tasks, event_loop):
    """
...
def schedule_bears(bears, result_callback, event_loop, running_tasks, executor):
    """
    ...
    for bear in bears:
        ...
        logging.debug("Scheduled {!r} (tasks: {})".format(bear, len(tasks)))
...
def finish_task(bear, result_callback, running_tasks, event_loop, executor, task):
    """
    ...
    if results is not None:
        for result in results:
            try:
                ...
                logging.error(
                    "An exception was thrown during result-handling.", exc_info=ex
                )
...
def run(bears, result_callback):
    ...
    executor = concurrent.futures.ProcessPoolExecutor(max_workers=_get_cpu_count())
