...
def _prepare_options(options, bear_class):
    ...
    if (
        options["output_format"] == "corrected"
        or options["output_format"] == "unified-diff"
    ):
        if (
            "diff_severity" in options
            and options["diff_severity"] not in RESULT_SEVERITY.reverse
        ):
            raise TypeError(
                "Invalid value for `diff_severity`: " + repr(options["diff_severity"])
            )
        ...
        if "severity_map" in options:
            ...
            for key, value in options["severity_map"].items():
                ...
                try:
                    ...
                    raise TypeError(
                        ...
                        "severity-map is no valid severity value.".format(value, key)
                    )
            ...
            options["severity_map"] = {
                key.lower(): value for key, value in options["severity_map"].items()
            }
...
def _create_linter(klass, options):
    ...
    class LinterBase(metaclass=LinterMeta):
        ...
        def check_prerequisites(cls):
            ...
            if shutil.which(cls.get_executable()) is None:
                return (
                    repr(cls.get_executable())
                    + " is not installed."
                    + (
                        " " + options["executable_check_fail_info"]
                        if options["executable_check_fail_info"]
                        else ""
                    )
                )
            else:
                if options["prerequisite_check_command"]:
                    try:
                        check_call(
                            options["prerequisite_check_command"],
                            stdout=DEVNULL,
                            stderr=DEVNULL,
                        )
                        return True
        ...
        def _get_create_arguments_metadata(cls):
            return FunctionMetadata.from_function(
                cls.create_arguments, omit={"self", "filename", "file", "config_file"}
            )

        @classmethod
        def _get_generate_config_metadata(cls):
            return FunctionMetadata.from_function(
                cls.generate_config, omit={"filename", "file"}
            )
        ...
        def _get_process_output_metadata(cls):
            ...
            if options["output_format"] is None:
                ...
                omitted = set(
                    chain(metadata.non_optional_params, metadata.optional_params)
                )
        ...
        def _convert_output_regex_match_to_result(
            self, match, filename, severity_map, result_message
        ):
            """
            ...
            for variable in ("line", "column", "end_line", "end_column"):
                groups[variable] = (
                    None
                    if groups.get(variable, None) is None
                    else int(groups[variable])
                )

            if "origin" in groups:
                groups["origin"] = "{} ({})".format(
                    klass.__name__, groups["origin"].strip()
                )
        ...
        def process_diff(
            self, diff, filename, diff_severity, result_message, diff_distance
        ):
            """
            ...
            for splitted_diff in diff.split_diff(distance=diff_distance):
                yield Result(
                    self,
                    result_message,
                    affected_code=splitted_diff.affected_code(filename),
                    diffs={filename: splitted_diff},
                    severity=diff_severity,
                )

        def process_output_corrected(
            self,
            output,
            filename,
            file,
            ...
            return self.process_diff(
                Diff.from_string_arrays(file, output.splitlines(keepends=True)),
                filename,
                diff_severity,
                result_message,
                diff_distance,
        ...
        def process_output_unified_diff(
            self,
            output,
            filename,
            file,
            ...
            """
            return self.process_diff(
                Diff.from_unified_diff(output, file),
                filename,
                diff_severity,
                result_message,
                diff_distance,
        ...
        if options["output_format"] is None:
            ...
            if hasattr(klass, "process_output"):
                raise ValueError(
                    ...
                    "specified.".format(klass.__name__, options["output_format"])
                )
        ...
        def run(self, filename=None, file=None, **kwargs):
            ...
            with self._create_config(
                filename, file, **generate_config_kwargs
            ) as config_file:
                ...
                if isinstance(self, LocalBear):
                    args = self.create_arguments(
                        filename, file, config_file, **create_arguments_kwargs
                    )
                else:
                ...
                try:
                    ...
                    self.err(
                        "The given arguments " "{!r} are not iterable.".format(args)
                    )
                    return
                ...
                output = tuple(
                    compress(output, (options["use_stdout"], options["use_stderr"]))
                )
                if len(output) == 1:
                    output = output[0]
                ...
                return self.process_output(
                    output, filename, file, **process_output_kwargs
                )
    ...
    # default version.
    result_klass = type(
        klass.__name__, (klass, LinterBaseClass), {"__module__": klass.__module__}
    )
