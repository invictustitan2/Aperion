...
from coalib.results.LineDiff import ConflictError, LineDiff
...
class Diff:
        ...
        for change_group in matcher.get_grouped_opcodes(1):
            for tag, a_index_1, a_index_2, b_index_1, b_index_2 in change_group:
                if tag == "delete":
                    ...
                    result.change_line(
                        a_index_1 + 1, file_array_1[a_index_1], file_array_2[b_index_1]
                    )
                    result.add_lines(
                        a_index_1 + 1, file_array_2[b_index_1 + 1 : b_index_2]
                    )
                        ...
                        if source_code != original_line:
                            raise RuntimeError(
                                ...
                                "Original line #{!r}: {!r}".format(
                                    source_code, file_line, original_line
                                )
                            )
    ...
    def from_clang_fixit(cls, fixit, file):
        ...
        oldvalue = "\n".join(file[fixit.range.start.line - 1 : fixit.range.end.line])
        endindex = fixit.range.end.column - len(file[fixit.range.end.line - 1]) - 1

        newvalue = (
            oldvalue[: fixit.range.start.column - 1] + fixit.value + oldvalue[endindex:]
        )
        new_file = (
            file[: fixit.range.start.line - 1]
    ...
    def range(self, filename):
        ...
        return SourceRange.from_values(
            filename, start_line=max(1, start), end_line=max(1, end)
        )
    ...
    def __bool__(self):
        ...
        """
        return self.rename is not False or self.delete is True or len(self._changes) > 0
    ...
    def add_lines(self, line_nr_before, lines):
        ...
        if linediff.add_after is not False:
            raise ConflictError(
                "Cannot add lines after the given line since "
    ...
    def change_line(self, line_nr, original_line, replacement):
        logging.debug(
            "Use of change_line method is deprecated. Instead "
    ...
    def replace(self, range, replacement):
        ...
        first_part = self.original[range.start.line - 1][: range.start.column - 1]
