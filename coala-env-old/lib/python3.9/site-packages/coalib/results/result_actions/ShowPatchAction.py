...
def format_line(
    ...
    return "|{:>4}{}{:>4}|{:1}{}".format(
        real_nr, sign, mod_nr, symbol, line.rstrip("\n")
    )
...
def print_beautified_diff(difflines, printer):
    ...
    for line in difflines:
        if line.startswith("@@"):
            ...
            printer.print(
                format_line(line[1:], mod_nr=current_line_added, symbol="+"),
                color="green",
            )
            current_line_added += 1
        elif line.startswith("-"):
            printer.print(
                format_line(line[1:], real_nr=current_line_subtracted, symbol="-"),
                color="red",
            )
    ...
    def is_applicable(result: Result, original_file_dict, file_diff_dict):
        ...
        try:
            ...
            for filename, diff in result.diffs.items():
                if diff and (
                    filename not in file_diff_dict
                    or diff + file_diff_dict[filename] != file_diff_dict[filename]
                ):
            ...
            return "Two or more patches conflict with " "each other: {}".format(str(ce))
            ...
            if tuple(current_file) != tuple(new_file):
                print_beautified_diff(
                    difflib.unified_diff(
                        current_file, new_file, fromfile=filename, tofile=to_filename
                    ),
                    printer,
                )
