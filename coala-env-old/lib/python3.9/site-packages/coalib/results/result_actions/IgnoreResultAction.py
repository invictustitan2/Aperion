...
class IgnoreResultAction(ResultAction):
    ...
    def is_applicable(result: Result, original_file_dict, file_diff_dict):
        ...
        if any(exists(filename) for filename in filenames):
            return True
        return (
            "The result is associated with source code that doesn't " "seem to exist."
        )
    ...
    def get_ignore_comment(self, origin, language):
        ...
        try:
            comment_delimiter = (
                Language[language].get_default_version().comment_delimiter
            )
            ...
            try:
                ...
                start_comment, end_comment = next(
                    iter(multiline_comment_delimiter.items())
                )
                ignore_comment = (
                    str(start_comment)
                    + " Ignore "
