...
KNOWN_EDITORS = {
    # non-gui editors
    "vim": {"file_arg_template": "{filename} +{line}", "gui": False},
    ...
    "gedit": {"file_arg_template": "{filename} +{line}", "args": "-s", "gui": True},
    "gvim": {"file_arg_template": "{filename} +{line}", "gui": True},
    "kate": {
...
class OpenEditorAction(ResultAction):
    ...
    def is_applicable(result: Result, original_file_dict, file_diff_dict):
        ...
        filenames = set(
            src.renamed_file(file_diff_dict) for src in result.affected_code
        )
        ...
        try:
            ...
            logging.warning(
                ...
                " Supported editors are: {supported}".format(
                    editor=editor, supported=", ".join(sorted(KNOWN_EDITORS.keys()))
                )
            )
