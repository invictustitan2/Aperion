...
class ConfWriter(ClosableObject):
    ...
    def __init__(
        ...
        (self.__section_name_surrounding_beg, self.__section_name_surrounding_end) = (
    ...
    def write_section(self, section):
        ...
        try:
            while True:
                ...
                if (
                    str(setting) == val
                    and not self.is_comment(setting.key)
                    and (
                        (setting.key not in self.__unsavable_keys)
                        or (not setting.from_cli)
                    )
                ):
    ...
    def __write_key_val(self, keys, val, defaults):
        ...
        for key in keys:
            if defaults and (
                key in defaults and val.startswith(str(defaults[key]) + ",")
            ):
    ...
    def __write_keys_val_to_file(self, append_keys, other_keys, val, defaults):
        ...
        if append_keys:
            append_keys = sorted(
                append_keys, key=lambda key: len(defaults[str(key)]), reverse=True
            )
            write_keys = []
            def_val = None
            for key in append_keys:
                if str(defaults[key]) == def_val:
                    ...
                    if write_keys:
                        ...
                        self.__write_value(
                            write_keys, append_val, self.__key_value_append_delimiter
                        )
                    write_keys = [key]
    ...
    def __get_append_val(self, val, def_val):
        ...
        append_val = (self.__key_delimiter + " ").join(
            [
                v.strip()
                for v in val.split(self.__key_delimiter)
    ...
    def __write_value(self, keys, val, delimiter):
        self.__file.write(
            (self.__key_delimiter + " ").join(keys) + " " + delimiter + " " + val + "\n"
        )
