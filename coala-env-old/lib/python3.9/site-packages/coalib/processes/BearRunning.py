    ...
    for result in result_list:
        if not isinstance(result, Result):
            send_msg(
                ...
                "{kwarglist}".format(bear=name, arglist=args, kwarglist=kwargs),
            )
            send_msg(
                message_queue,
                timeout,
                ...
                "Result".format(bear=name, ret=result.__class__),
            )
...
def run_local_bear(
    message_queue, timeout, local_result_list, file_dict, bear_instance, filename
):
    """
    ...
    """
    if (
        not isinstance(bear_instance, LocalBear)
        or bear_instance.kind() != BEAR_KIND.LOCAL
    ):
        send_msg(
            message_queue,
            timeout,
    ...
    kwargs = {
        "dependency_results": get_local_dependency_results(
            local_result_list, bear_instance
        )
    }
    return run_bear(
        message_queue, timeout, bear_instance, filename, file_dict[filename], **kwargs
    )


def run_global_bear(message_queue, timeout, global_bear_instance, dependency_results):
    """
    ...
    """
    if (
        not isinstance(global_bear_instance, GlobalBear)
        ...
        send_msg(
            ...
            "out...".format(global_bear_instance.__class__.__name__),
            Constants.THIS_IS_A_BUG,
        )
...
def run_local_bears_on_file(
    ...
    if filename not in file_dict:
        ...
        send_msg(
            ...
            timeout,
            LOG_LEVEL.DEBUG,
            "The given file through the queue is not in the file " "dictionary.",
        )
...
def get_next_global_bear(
    timeout, global_bear_queue, global_bear_list, global_result_dict
):
    """
...
def run_global_bears(
    ...
    try:
        while True:
            bear, dep_results = get_next_global_bear(
                timeout, global_bear_queue, global_bear_list, global_result_dict
            )
