...
from coala_utils.string_processing import unescape, unescaped_split, unescaped_strip
...
class StringConverter:
    ...
    def __get_raw_list(self):
        pattern = "(?:" + "|".join(re.escape(v) for v in self.__list_delimiters) + ")"

        return list(unescaped_split(pattern, self._value, use_regex=True))
    ...
    def __prepare_list(self):
        ...
        if self.__strip_whitespaces:
            self.__escaped_list = [
                unescaped_strip(elem) for elem in self.__escaped_list
            ]

        self.__unescaped_list = [unescape(elem) for elem in self.__escaped_list]
